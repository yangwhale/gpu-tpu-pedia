...
| 精度 | **fp8_mx** | 对齐 DSV3/Qwen3 GB300 recipe |
...
| 模型 | 规模 | GB300 实测 TFLOP/s/GPU | 备注 |
|---|---|---|---|
| DeepSeek V3 | 671B / 61L | **~1658**（256 GPU） | 官方 1670 的 99.3% |
| DeepSeek V3 scale-in | 671B / 31L | ~1550（128 GPU） | 层数减半 → 固定开销占比升 |
| **Hy3** | **295B / 80L** | **待测** | 见下方预期 |
...
- **粗判**：若稳态落在 **1200–1500 TFLOP/s/GPU** 属正常；低于 1000 说明配置有问题（优先查 EP / dispatcher / graph 是否真生效）。
...
| 4 | torchrun 直跑不自动设 perf env | 全部手动 export（§5） |
