...
<!-- ⛔ 2026-09-08：这里原先并排着两个 note ——&nbsp;一个讲「2026-09-04 从第七节提到
     这里」的编排理由，一个讲「这次为什么折叠」。**两个都是给作者看的**，而且互相重复。
     ⭐ 按规矩（教材只留读者要的、讲述进讲义）合并成下面一句，
       编排理由与现场原话搬进 topic03-build-lecture.py 的「讲稿 · 3」。
     📌 保留的判据没变：**不先说清「那个大家伙根本不落地」，
       后面所有「怎么少算」的讨论都建在恐慌上** ——&nbsp;所以这一节不能整节拿掉，
       只能把硬件细节折起来。 -->
<div class="note ok"><p>⭐ 上一节那个 <b>488 GiB</b> 是<b>假想的最坏情况</b>。
  实际上，那个 n × n 的注意力矩阵从来没有被完整存进显存过 ——&nbsp;
  FlashAttention 让它<b>只在片上一块一块地过</b>，算完就扔。<br>
  📌 <b>这一节只要带走这一句</b>，外加下面那张「它跟三个旋钮不是一类东西」的对照表。
  硬件细节（块该开多大、为什么融合完还是只跑到 35%）折在本节末尾，
  <b>跳过不影响主线。</b></p></div>
...
<details class="aside"><summary>🔬 <b>FlashAttention 深潜</b>
——&nbsp;它到底怎么做到不落地、块该开多大、以及
<b>为什么融合完还是只跑到 35%</b><em>（这一段是硬件细节，跳过不影响主线）</em></summary>
