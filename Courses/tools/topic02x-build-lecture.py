...
lec("X78", "五", "补两样：SparseCore 与二维环面", 2, "五：补两样（可跳）",
    ...
    land="**它们不影响主线，但少了会让人觉得芯片没拆干净**", opt=True,
    html='''
  <div class="goal"><span class="k">这一讲要留下什么</span>
    <b>把芯片拆干净。</b>这两张<b>时间紧就可以整段跳过</b> ——
    主线从 X-6 直接接到 X-5 完全成立。
  </div>

  <h3 class="sec">讲稿</h3>
  <div class="say">
    <span class="board">滚到<b>图 X-7</b>，快过。</span>

    <p>芯片上除了那个大核，还蹲着两个 SparseCore。
      <b>它们不做矩阵乘，做的是「在一张很大的表里到处乱查」</b> ——
      官方原话说，主用途是加速重 embedding 的推荐模型。</p>

    <span class="pause">⭐ <b>这一句值得原样讲给学员听。</b>
      我这张图的初版写的是「扩散模型用不到它」——&nbsp;<b>那句比出处强。</b>
      官方说的是 <b>a primary use case</b>，主用途，不是唯一用途。
      <em>所以现在图上摆的是推导链，不是结论：没有大 embedding 表 →
      没有稀疏聚合 → 这颗为「散着取」造的东西在这条链路上基本闲着。</em>
      ——&nbsp;<b>「闲着」不等于「用不到」，我们没实测，就不下断言。</b></span>

    <span class="board">滚到<b>图 X-8</b>。</span>

    <p>4 个 ICI 口，连上下左右四个邻居，<b>边缘绕回对面 —— 所以叫环面，不叫网格。</b>
      16 乘 16 就是一个 Pod 的 256 颗。</p>

    <p><b>最远 16 跳</b>，这个数能当场推：环面每一维最多绕半圈，16 除以 2 是 8，
      两维相加 16。<em>对照 v7 的 4×4×4，每维最多 2 跳，三维相加是 6 跳。</em></p>

    <p>16 跳听着很多。<b>但要先问一句：谁会走这条路？</b>
      ——&nbsp;<em>模型一颗装得下，多卡就是各生成各的，
      八颗卡同时出八张图，卡与卡之间几乎不用说话。</em>
      <b>通信量接近零的时候，跳数是多少就不重要了。</b></p>
  </div>

  <h3 class="sec">别讲什么</h3>
  <div class="dont">
    <b>SparseCore 的内部数据流一个字都不要讲。</b>gather / scatter 怎么排是半小时的东西。<br>
    <b>时间紧就整段跳过这一节。</b>它是补完整性的，不是主线。
  </div>
''')
