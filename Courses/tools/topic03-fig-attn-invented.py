...
⭐⭐⭐ 2026-09-13 **整张重画**（现场：「图是图，字是字 ——&nbsp;把原理画出来」）。
   ...
   ② **2015 第一刀 ——&nbsp;打分函数太慢。**
      ⛔ 2026-09-15 更正：原写「2017 第一刀」，记错了年份和团队。
      点积打分出自 **Luong 等 2015**（arXiv 1508.04025，dot/general/concat 三种）；
      Vaswani 2017 加的是 **1/√d_k 那个缩放**，不是点积本身 ——
      §3.2.1 原话自己承认：「identical to our algorithm, except for the scaling factor」。
      ⭐ 而这一格是暗线①（硬件决定用哪个公式）的**第一站**，§9.2 收尾点过它的名。
...
def main():
    ...
    # ══════════ ② 2015 第一刀：打分函数 ══════════════════════════
    x, pw = PX[1], PW[1]
    py = f.panel(x, y0, pw, PH, "② 2015 第一刀　打分太慢", GR,
                 sub="加性 → 点积")
    ...
    yy = f.src(yy + 14,
               ...
               "② 点积打分出自 <tspan font-weight=\"700\">Luong 等 arXiv 1508.04025（2015）</tspan>，该文同时给了 dot / general / concat 三种打分",
               "⛔ Vaswani 等 arXiv 1706.03762 §3.2.1 加的是 <tspan font-weight=\"700\">1/√d_k 那个缩放</tspan>，不是点积本身 ——&#160;原话是「identical to our algorithm, except for the scaling factor」；③ 同文 §3.2.2（多头补偿分辨率）",
