...
SECTIONS = [
    # ⭐ 2026-09-07：整表**后移了一位**，因为最前面插进了新的 §零「起点：RNN」。
    #   现场定的新讲法：「从头到尾一个一个讲那个 Attention 的变体，从头开始讲，
    #   讲完一个是一个深入的。……不对，应该先讲 RNN，讲完 RNN 再讲。」
    # ⛔ 重编号是脚本一次性做的（含 <h3>N.M 和正文里的 §N.M），
    #   靠 topic02-lint-xref.py 兜底验收。手改一定会漏，而漏了不报错。
    ("s零", "零", "起点：RNN —— 被注意力补的那个东西"),
    ("s一", "一", "一切从长上下文说起 —— 两条独立的动机"),
    ("s二", "二", "先解决一个误会：那个矩阵从来没被存下来过 —— FlashAttention"),
    ("s三", "三", "分类学：只有三个旋钮"),
    ("s四", "四", "旋钮①：每个 token 存多少"),
    ("s五", "五", "旋钮②：每个 query 看多少"),
    ("s六", "六", "旋钮③：换一套数学 —— 线性注意力"),
    ("s七", "七", "旋钮④（其实是元旋钮）：混合"),
    ("s八", "八", "代价：没有免费的午餐"),
    ("s九", "九", "落到硬件（本专题的落点）"),
...
# ══════════════════════════════════════════════════════════════════
BODY = '''<section id="x1"><div class="wrap"><div class="stn"><h2>这个专题要回答的一个问题</h2></div>
...
__FIG_CHRONICLE__
__TABLE_MODELS__
<!-- ⛔ 2026-09-07：这里原先是一段五行的说明，讲「上半是编年史、下半是各家配比」。
     两个毛病：① 按新立的规矩，正文不留大段解释；
     ② **「下半」早就不在这张图里了** —— 39 行的表已经拆成上面那张可排序 HTML 表。
     ⭐ 又一次同一个病：**描述图的文字是图的下游，图改了它不会跟着动，而且读起来通顺。**
     现在只留一句落点。 -->
...
<section id="s二"><div class="wrap"><div class="stn"><span class="badge">第 二 节</span><h2>先解决一个误会：那个矩阵从来没被存下来过 —— FlashAttention</h2></div>
...
<h3>2.2b ⭐ 那张经典图：内循环、外循环 —— 以及 FA2 为什么把它掉了个个儿</h3>
...
<h3>2.2c 同一个原则，在 warp 这一层又用了一遍</h3>
...
<h3>2.3 ⚠️ 在线 softmax 不是免费的 —— 它是后面 35% 那个数的根</h3>
...
<h3>2.5 ⭐ TPU 那边是三堵墙 —— 而且「块越大越好」是错的</h3>
...
<h3>2.6 ⭐ 融合之后，它还是只跑到 35% —— 三层原因，都不是配置问题</h3>
...
<section id="s三"><div class="wrap"><div class="stn"><span class="badge">第 三 节</span><h2>分类学：只有三个旋钮</h2></div>
...
<section id="s四"><div class="wrap"><div class="stn"><span class="badge">第 四 节</span><h2>旋钮①：每个 token 存多少</h2></div>
__FIG_TX_K1__
<h3>3.1 MHA → MQA → GQA：一个连续旋钮</h3>
...
<h3>3.2 MLA：不砍头，改成低秩压缩</h3>
<ul><li>KV 不再按头存，而是压成<b>一个 512 维的隐向量</b>（<code>d_c = 512</code>）， 用的时候再用上投影矩阵升回 128 个头</li><li>位置信息<b>单独走 64 维一路 RoPE</b>（<code>d_h^R = 64</code>）</li><li>所以每 token 每层只存 <b>512 + 64 = 576</b> 个数 —— 对照 MHA 的 32,768 个，<b>56.9 倍</b></li><li>出处：V3 论文 §5.2 超参一节，<code>n_h=128, d_h=128, d_c=512, d_h^R=64, 61 层</code></li></ul>
<p><b>代价：用计算换显存。</b> 多了一对降维/升维的矩阵乘。 这句话在<a href="专题04-反向与优化器.md">专题四</a> §2.2 会被再打一个折扣 —— ⚠️ <b>MLA 的压缩在训练前向里其实不生效</b>（K/V 会被解压出来算）， 它省的是<b>推理时的 KV cache</b>，不是训练时的激活。这是一个非常常见的误解。</p>
...
<h3>3.4 Gated MLA</h3>
<ul><li>在 MLA 的输出端加一个<b>门控</b>：<code>gate = σ(W_g x)</code>，逐元素乘在注意力输出上</li><li>Kimi K3 用的是<b>全秩</b>门控矩阵（K2 那代是低秩的）</li><li>出处：K3 技术报告 §3.1.2</li></ul>
<p>它的作用不是省显存 —— <b>门控不减少任何 KV</b> —— 而是让模型能学会「这一层这个位置，注意力的输出干脆不要」。 放在旋钮 ① 里是因为它改的是 MLA 这一支的形状，但要讲清楚它<b>省的不是显存</b>。</p>
<hr>
</div></section>
<section id="s五"><div class="wrap"><div class="stn"><span class="badge">第 五 节</span><h2>旋钮②：每个 query 看多少</h2></div>
__FIG_TX_K2__
<h3>4.1 SWA（滑动窗口）</h3>
...
<h3>4.2 Attention sink —— 一个现象，不是一个方案</h3>
...
<h3>4.4 DSA（DeepSeek-V3.2）—— Lightning Indexer</h3>
...
<h3>4.5 ⭐ CSA + HCA（DeepSeek-V4）—— 压缩率按距离分层</h3>
...
<h3>4.5b ⭐ 第二阶段：索引本身成了开销 —— IndexShare 与 IndexCache</h3>
<p>前面四个方案（NSA / DSA / CSA+HCA）都有一个共同的零件：<b>一个决定"该看哪几块"的索引器</b>。 §4.4 讲 DSA 的时候它叫 Lightning Indexer。到这一步为止，所有心思都花在<b>让每个 query 少看几块</b>上。</p>
...
<section id="s六"><div class="wrap"><div class="stn"><span class="badge">第 六 节</span><h2>旋钮③：换一套数学 —— 线性注意力</h2></div>
__FIG_TX_K3__
<h3>5.1 基本换法</h3>
...
<h3>5.2 谱系：四步，每一步都在修上一步的一个具体毛病</h3>
<table>
...
<ul><li><b>delta rule 不是 2024 年的东西</b>，是 Schlag 等 2021 年那篇 <em>Linear Transformers Are Secretly Fast Weight Programmers</em>（可上溯到 1990 年代的 fast weight programmer）。2024 年那篇（arXiv 2406.06484）做的是 <b>把它并行化</b>，这是另一件事、也是很关键的一件事 —— 见 §5.4</li><li><b>KDA 是 2025 年 10 月的 Kimi Linear</b>，不是 2026 年</li></ul>
...
<h3>5.2b ⭐⭐ Mamba 在哪？——&nbsp;它不是另一支，它是这一支的祖宗</h3>
...
<p>把它们放到同一个式子里看，一切就清楚了。上面 §5.1 那个递推稍微写全一点：</p>
...
<p>⭐ 所以这张表不是「表达力越来越强」的单调故事。<b>每一步都在「A 能多复杂」和「还算不算得动」之间重新划线</b>—— 这正是 §5.2 那句「表达力和可算性是一起设计的」的完整版。</p></div>
...
<h3>5.3 ⚠️ 它不是「更快的 attention」，是另一个模型</h3>
...
<section id="s七"><div class="wrap"><div class="stn"><span class="badge">第 七 节</span><h2>旋钮④（其实是元旋钮）：混合</h2></div>
<h3>6.1 为什么混合几乎是唯一的答案</h3>
...
<h3>6.2 配比：3:1 是怎么来的，以及它不是定律</h3>
<table>
...
<h3>6.3 ⭐ K3 的 NoPE —— 混合带来的一个意外红利</h3>
...
<p>三个后果，一个比一个实在：</p>
<ol><li><b>不用调 RoPE 外推。</b> 模型直接外推到 1M，不需要任何位置编码的重标定 —— 长上下文扩展里最烦人的一块调参，直接消失了</li><li><b>MLA 层在推理时可以退化成纯 MQA。</b> 位置编码没了， §3.3 里那条"不可吸收的 64 维"也就不存在了 —— <b>上投影可以完全吸收</b></li><li><b>KV cache 最多降 75%</b>（Kimi Linear 的数字）， 1M 上下文下 TPOT 从 11.48 ms 降到 1.84 ms，<b>6.3×</b></li></ol>
...
<h3>6.4 ⭐ 各家速查：你日常在用的那些模型，注意力到底是什么</h3>
...
<p>这跟"腾讯评估过线性注意力但最终没上"的说法对得上，而且是一手证据。 然后 Hy4 一步跨到全层稀疏 —— <b>它整个跳过了线性这一支。</b> 对照 §6.2 那张配比表：<b>混合不是唯一解，只是最多人选的那个解。</b></p></div>
...
<tr><td>GLM-5.2（744B）</td><td>2026-06-16</td><td>MLA ＋ DSA ＋ <b>IndexShare</b></td><td>每四个稀疏层共用一个索引器（见 §4.5b），1M 下省 <b>2.9×</b> FLOPs</td></tr>
...
<p>而且配比正好落回 <b>3 : 1</b>，跟 Kimi Linear、Qwen3.5 一样 —— <b>换了一家公司、换了搭档层的类型，配比还是那个区间。</b> 这是 §6.2 那句"3:1 到 6:1 这个量级"到目前为止最强的一个旁证。</p></div>
<hr>
</div></section>
<section id="s八"><div class="wrap"><div class="stn"><span class="badge">第 八 节</span><h2>代价：没有免费的午餐</h2></div>
<p>一张表把所有方案摆在一起：</p>
<table>
...
<section id="s九"><div class="wrap"><div class="stn"><span class="badge">第 九 节</span><h2>落到硬件（本专题的落点）</h2></div>
...
<section id="s十"><div class="wrap"><div class="stn"><span class="badge">第 十 节</span><h2>收尾：把谱系放回时间线</h2></div>
...
<section id="s十一"><div class="wrap"><div class="stn"><span class="badge">第 十一 节</span><h2>这个专题明确不讲</h2></div>
...
<tr><td>MLA</td><td>DeepSeek-V3, arXiv <b>2412.19437</b> §3.1 + §5.2（超参那段给了 <code>n_h/d_h/d_c/d_h^R</code> 的准确值）</td></tr>
<tr><td>Gated MLA / K3 全貌</td><td>Kimi K3, arXiv <b>2607.24653</b> §3.1.2、表 1（93 层 / 69 KDA + 24 MLA / 2.78T-104.2B）</td></tr>
...
<tr><td>CSA / HCA</td><td>DeepSeek-V4, arXiv <b>2606.19348</b> §3.3 + §3.3.4（m=4 / m′=128 / top-k / 27%·10% / 2%）</td></tr>
...
<tr><td>§1.1 那张 KV cache 对照表</td><td><b>自己按公式推的</b>：<code>2·n_h·d_h·L</code> 与 <code>(d_c+d_h^R)·L</code>，输入全部来自 V3 论文 §5.2。<b>口径（K/V 都按 d_h=128）要在讲的时候声明</b></td></tr>
</tbody></table>
<hr>
</div></section>
