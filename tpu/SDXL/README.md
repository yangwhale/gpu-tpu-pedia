...
| 场景 | 推荐配置 | 说明 |
|------|---------|------|
| **单图生成** | **v6e-1** | ✅ 最佳选择，延迟最低，性价比最高 |
| 已有 v6e-4/v6e-8 | 数据并行 | 使用 `--batch_size` 批量生成，充分利用多卡 |

> **重要**：SDXL (3.5B) 模型可完整放入单张 TPU 芯片。如果你有配额选择权，**v6e-1 是最佳选择**。如果你已经有 v6e-4/v6e-8 机器，请使用数据并行模式批量生成图片。

### 各配置性能对比

| TPU 配置 | 模式 | 50 步总时间 | 图片数 | 吞吐量 |
|----------|------|-------------|--------|--------|
| **v6e-1** | 单图 | **3s** | 1 | 0.33 img/s |
| v6e-4 | 数据并行 | 3.31s | 4 | 1.21 img/s |
| v6e-8 | 数据并行 | 3.33s | 8 | 2.40 img/s |
