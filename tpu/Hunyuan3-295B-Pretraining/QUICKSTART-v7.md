...
| 5 | ~~FP8 换 tile 入口~~ **已撤回** | 7.85 s | ~~1,014.8~~ | — | ❌ 漏算 189/192 专家 | 补齐后 **637.0**，低于第 4 行 |
...
### ⛔ 64 芯片 FP8 → 1,014.8 **已撤回，不要用**

> [!caution] 2026-08-16：这个配方触发 megablox 的静默漏算 bug
> `shard_exp_on_fsdp=True` + `fixed` 校准 + **native（非 tokamax）** 三者同时成立时，
> 权重按**专家维**切成 64 份（每卡 3 个完整专家），而 native 分支
> **没有执行 all-gather** —— kernel 只对本地 3 个专家建表，
> **其余 189 个专家的贡献完全没算**，不报错、loss 照常下降。
>
> | | 每步 | per-chip |
> |---|---|---|
> | 漏算（曾报为 1,014.8） | 7.847 s | ~~1,014.8~~ |
> | **补齐 all-gather 后** | **12.501 s** | **637.0** |
>
> **修正后比原配方（677）还低 6%。**
> **生产请继续用下面的 tokamax + QAG 配方（677）。**
> 根因与补丁见 [TUNING-v7 §3.4.10](TUNING-v7.md)。
