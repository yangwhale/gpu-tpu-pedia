...
# ══════════════════════════════════════════════════════════════════
lec("X0", "开场", "这十八分钟要回答两件事", 1, "开场：两件事",
  ...
  <div class="say">
    ...
    <p>开始之前先说清一件事：<b>这十八分钟我不给你们比 benchmark，一个快慢数都没有。</b>
...
lec("X1", "一", "一把尺子量四颗，再把两颗拆开", 5, "一：那条线 ＋ 两颗芯片",
  ...
  <div class="say">
    ...
    <span class="board">滚到 <b>图 X-3</b>，一颗 H100 —— <b>同一种画法</b>。</span>

    <p>H100：算力<b>摊成 528 个</b>小单元；片上多了一整层<b>硬件自动管</b>的 L2；门宽一倍。</p>

    <span class="pause">⭐ <b>这里指一下那道门</b>：两张图里门的宽度是<b>按带宽等比画的</b>，
      可以直接叠着比 —— <em>台下会自己看出来那是一半。</em></span>
...
lec("X9X4", "二", "那你的活落在哪一边", 3, "二：算一遍，放回轴上",
  ...
  <div class="dont">
    ...
    主动说这条，客户才信你前面那句。<br>
    <b>② 落在算力侧不等于算力吃满了：</b>我们实测 Wan 的 MFU 约 <b>37%</b>，
    卡在 head_dim 128 对 MXU 的 256。<b>屋顶线说瓶颈在哪一侧，不说那一侧用得好不好。</b>
  </div>
""")

lec("X6", "三", "一颗装得下吗", 1, "三：装得下吗",
    what="把权重体积摆到 32 GB 这条线上，给出「摊开还是多放几路」的判据",
    land="**装不下就把一个模型摊开，装得下就多放几路**",
    grp="判据 · 10 分钟", html="""
  <h3 class="sec">讲稿</h3>
  <div class="say">
  ...
  <h3 class="sec">别讲什么</h3>
  <div class="dont">
    <b>别把它讲成「扩散都装得进一颗」</b> —— 图上那两条踩线和超线的是特意画的。
    而且这根轴<b>只算权重</b>，不含激活。<br>
    <b>别展开 Pod 拓扑。</b>那三条「减配」为什么配套，在 L200。
  </div>
""")

lec("X10", "四", "它的流水线又胖又瘦", 3, "四：最细的那道腰",
    what="把一次生成的数据体积摊在对数轴上，指出 latent 那道腰",
    land="**跨机只传 19.4 MB，切口成本是计算量的万分之零点七 → 「能不能切」这个问题不成立**",
  ...
  <div class="say">
    ...
    <p>实际部署时，一次生成是<b>三件事</b>：文本编码、五十步去噪、VAE 解码。
      这张图把它们之间的<b>数据体积</b>画在一根对数轴上。</p>

    <p>进去是两百多字节的 prompt。到 DiT 内部，随手一个激活张量就是 <b>774 兆</b>；
      注意力分数矩阵要是显式摊开，<b>457 GB</b>。
      出了 VAE 一展开，又变成 <b>448 兆</b>的像素。</p>

    <p><b>但 DiT 吐出来的那份 latent，只有 19.4 MB。</b></p>

    <span class="pause">⭐ <b>这是全场最值得停的一处。</b><br>
      切开流水线的代价是切口上的数据要搬一趟 —— 所以切在哪，取决于<b>哪儿的数据最少</b>。
      而扩散的形状<b>天然把这个位置摆在了明处</b>。</span>

    <p>算一下就知道有多便宜：19.4 兆走 100G 网卡是 <b>1.5 毫秒</b>，
      而被切开的那一段本身要算 <b>229 秒</b> ——&nbsp;
      <b>切口成本是计算量的万分之零点七。</b></p>

    <p>所以<b>「能不能切」这个问题在这里根本不成立，只剩「要不要切」。</b></p>

    <p>顺带说一句我们怎么确认这个数：那三个绿点是<b>仓库里真实存在的文件</b>。
      latent 那个 —— 16 乘 21 乘 90 乘 160，每个数四字节，
      加上 safetensors 的 272 字节文件头，<b>正好等于文件大小，一个字节不差。</b></p>
  </div>

  <h3 class="sec">⛔ 两处极易被读反</h3>
  <div class="dont">
    <b>① 这张图画的是体积，不是时间。</b>腰细不代表那一段轻松 ——
    产出那 19.4 兆的第二段，恰恰占了全程 <b>98%</b> 的时间。<br>
    <b>② 那根 457 GB 从不真的存在。</b>Splash Attention 分块算，一块算完即弃。
    画它是为了说明「不分块就没法跑」，不是说 HBM 里真有这么多。
  </div>
""")
...
''')
    ...
    assert total == 18, "主线合计 %d 分钟 —— 目标 18，超了就砍，少了就补" % total
