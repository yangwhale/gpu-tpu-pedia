...
# ⛔⛔ 2026-09-12 二轮学生审稿：「做到 1M 以上的有几家」全讲有两个答案 ——
#   骨架图写 13、编年史写 12，差在 RWKV-7 那行的「无限（理论）」算不算。
#   ⭐ 而这门课自己就在教口径问题，所以规则必须写下来，且只有一处：
#     **「做到 1M」＝ 上下文那一列的声明值 ≥ 1M；「无限（理论）」不计入** ——
#     它不是一个声明的长度，是一个架构性质。
#   ⚠️ 算上它是 13 家，**结论不变**（RWKV-7 是纯 RNN，本来就在旋钮③ 上）。
def over_1m(rows):
    """按上面那条规则数「做到 1M 以上」的行。⛔ 别在别处再数一遍。"""
    return [r for r in rows if r[3].endswith("M")]
