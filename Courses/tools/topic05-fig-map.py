...
TOPO = [("1P1D，TP4 decode", "原始脚本（并发 256）", 14563, 8),
        ...
        ("3P ＋ DEP8 decode", "decode 卡 4→8 张，并发 512", 55153, 20)]
...
PANO = [
    ("数据并行", "切 batch", BL, [("DP", "AllReduce", 1), ("ZeRO-1", "RS ＋ AG", 1), ("FSDP", "AG ×2 ＋ RS", 0),
                                 ("HSDP", "机间那一维可跨慢线", 1), ("Attention DP", "AllToAll（随 EP 走）", 0)]),
    ("序列并行", "切一条样本", GR, [("Megatron SP", "AG ＋ RS（TP 的搭档）", 0), ("Ring Attention", "Send／Recv 沿环", 0),
                                  ("Ulysses", "AllToAll", 0), ("DCP", "推理：AG ＋ 合并，每层 3 次", 0), ("PCP", "推理：切长 prompt", 0)]),
...
def fig_panorama():
    ...
    y0 = f.header("全景：四类刀法，每个名字挂两样东西　——　<tspan font-weight=\"700\">用哪种通信，能不能跨慢线</tspan>",
                  "切权重和切专家都归「模型并行」。● 可以跨慢线　○ 要快线（AFD、Wide-EP 跨机但每层都走，也要快网；有例外见第七节）",
