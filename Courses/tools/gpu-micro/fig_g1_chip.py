...
def build():
    ...
    f.legend([(BL, "计算：SM"), (RD, "启用/禁用边界"), (GN, "片上 SRAM"),
              (YL, "片外内存"), (TL, "片间互联"), (GREY, "非 NVIDIA 官方")])
...
def _die(f, dx, k):
    ...
    f.t(dx + 138, HBM_T - 8,
        "全片 8 堆栈 ＝ 186 GB · <tspan class=\"num\" fill=\"#b06000\">8.0 TB/s</tspan>"
        "　（HGX B200 那版是 180 GB）", "xs")
    sw = (DIEW - 24 - 3 * 13) / 4
    for i in range(4):
        x = dx + 12 + i * (sw + 13)
    ...
    f.t(dx + 22, L2_T + 19, f"L2 分区 {k}　63 MB", "box", GN)
    f.t(dx + 190, L2_T + 19,
        "全片 126 MB —— H100 是 50，A100 是 40", "xs")
    f.t(dx + 22, L2_T + 37,
        "<tspan fill=\"#9aa0a6\">实测本分区 21 TB/s；跨到对面 die 掉到 16.8 TB/s，延迟也变高（第三方 Vulkan 压测）</tspan>", "xs")
