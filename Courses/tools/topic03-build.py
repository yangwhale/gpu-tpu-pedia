...
<p>⭐ 图给不了的是这条判据的<b>一般形式</b>：
  <b>看到模型里一个「毫无道理却极其稳定」的现象，先去找是不是某个守恒或归一化约束逼出来的。</b>
  <em>attention sink 不是 bug、也不是谁设计的特性 ——&nbsp;
  你规定每一行注意力必须加起来等于 1，模型就<b>一定</b>会找个地方倒掉多余的那部分。
  量化里那批总也压不下去的 outlier，跟它是同一件事。</em></p>
...
<p>⭐ 图给不了的是这条<b>提问法</b>：
  <b>拿到任何一个新注意力方案，先问它动了哪几个轴</b>
  ——&nbsp;<em>竖着压（每个 token 存多少）、横着压（几个 token 合一条）、
  读哪几条（稀疏）。三个轴互不相干，所以可以同时拧。
...
<p>⭐ 图给不了的是这条<b>提问法</b>：
  <b>看到这一支的任何一个新名字，只问一句 ——&nbsp;「它的『擦』是怎么决定的？」</b>
  <em>三代之间，<b>读的方式几乎没变，全部的差别都在「写之前擦不擦、擦多少」</b>。
