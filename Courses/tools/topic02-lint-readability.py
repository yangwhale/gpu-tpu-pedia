...
def scan(path):
    ...
    # ⛔⛔ 2026-09-17：style / script 也必须整块挖掉，理由跟下面 tag_balance()
    #    那段注释一模一样 —— 而我当时**只在那个函数里挖了**。
    #    实际后果：某页的 CSS 注释里写着字面量 `<p class="q">`（在解释一条折叠规则），
    #    下面这个 `<p\b...>(.*?)</p>` 就从那儿一路匹配到正文里下一个 </p>，
    #    拼出一个 774 字的「段落墙」。段落不存在，告警编号却是真的。
    #    ⭐⭐⭐ 判据：**一条判据写进注释，它只在那一个函数里生效。**
    #       同一个文件里的另一个函数不会因为「旁边写着」就自动遵守它。
    #       ——&nbsp;要么抽成共用函数，要么每处都写。这里选了后者并互相指名。
...
# ⛔⛔ 2026-09-04 立。触发事件：上一轮把 3.3b 那个 624 字的「记分板」框整块折起来，
#     理由（它回答「怎么实现」不是「为什么要有」）到现在仍然成立 ——&nbsp;
#     **但那个框里装的不只是机制，还装着「记分板」这个词唯一的定义**。
#     折完之后主线剩下的是：「这门课欠了一笔账……先把这笔还上」，然后账没还。
#     ⭐ 判据一句话：**折叠是用来放「怎么做到的」的，不是用来放「它是什么」的。**
#        一个词的定义永远留在主线，机制随便折。
DEF_VERB = r'(?:就是|＝|=|叫做|叫|指的是|是指|也就是|量的是|这名字|定义为)'


def fold_defines(path):
    """揪出「主线用了这个词、而它唯一的定义在折叠里」的词。

    做法：把页面切成 **折叠内** 和 **折叠外** 两堆文本，
    对每个候选术语问三句话 ——&nbsp;
      ① 主线上出现过吗？没有就不管（折叠里自成一体，随便写）。
      ② 主线上有定义句吗？有就放行。
      ③ 折叠里有定义句吗？有 → **这就是那个坑**：读者关着折叠读，
         会撞见一个从没被解释过的词。

    ⚠️ 候选词是手工列的，不做自动抽词 ——&nbsp;自动抽会把「带宽」「延迟」
       这种不需要定义的常用词全抓进来，误报一多这条检查就废了
       （零容忍那版自指计数就是这么废过一次的）。
    """
    TERMS = ('记分板', '双缓冲', 'mbarrier', '占用率', '屋顶线', '脉动阵列',
             '收缩维', '算术强度', '算子融合', 'warp', 'lane', 'sublane',
             'MFU', 'HFU', 'TMA', 'BlockSpec', 'CTA', '累加器')
    h = io.open(path, encoding='utf-8').read()
    h = re.sub(r'<!--.*?-->', '', h, flags=re.S)
    # ⛔ 这里**故意不挖掉 <svg>**（本文件别处的检查都挖）。
    #    理由：这是一门以画图为主的课，**术语的定义大量就写在图上**
    #    （「线程块（CTA）」「双缓冲：搬下一块的同时算这一块」都只在图里）。
    #    第一版照抄了别处那句挖 svg 的代码，于是把图当成不存在 ——&nbsp;
    #    两条误报，而且方向最坏：**它指着已经解释清楚的词说「你没解释」**。
    h = re.sub(r'<(script|style)\b.*?</\1>', '', h, flags=re.S)
    folds = re.findall(r'<details\b.*?</details>', h, flags=re.S)
    inside = re.sub(r'<[^>]+>', '', '\n'.join(folds))
    outside = re.sub(r'<[^>]+>', '', re.sub(r'<details\b.*?</details>', '', h, flags=re.S))
    out = []
    for w in TERMS:
        if w not in outside:
            continue
        # ⚠️ 定义不止「X 就是 Y」一种句式，漏掉别的写法就会误报：
        #    · 同位语　线程块（CTA）／（CTA）线程块      ← CTA 是这么定义的
        #    · 冒号　　双缓冲：搬下一块的同时算这一块      ← 图上普遍这么写
        #    · 破折号　记分板 —— 一张表
        #    第一版只认「就是／叫／＝」，四条里误报了两条。**误报会把真问题淹掉。**
        e = re.escape(w)
        pat = re.compile(
            r'(?:%s)\s*.{0,8}%s' % (DEF_VERB, e)          # 就是 X
            + r'|%s.{0,14}(?:%s)' % (e, DEF_VERB)          # X 是 / X 量的是
            + r'|[\u4e00-\u9fa5]{2,8}[（(]\s*%s\s*[)）]' % e   # 线程块（CTA）
            + r'|%s\s*[（(][^)）]{2,20}[)）]' % e               # CTA（线程块）
            + r'|%s\s*[：:]\s*\S' % e                          # 双缓冲：…
            + r'|%s\s*(?:——|&mdash;|-{2,})\s*\S' % e)         # 记分板 —— …
...
def nested_a(path):
    ...
    ⛔ 2026-09-13 真出过：正文里手写了 `<a href="#s六">§6.5b</a>`，
...
def undefined_note_class(path):
    ...
    ⛔ 2026-09-13 真出过：专题三用了 4 处 `note bad`，
    ...
    for p in paths:
        ...
        for t, o, c in tag_balance(p):
            ...
            print('    —— 这类错**不报错也不难看**，但会让后面的样式一路串下去。'
                  '先修它，再看别的。')
            bad += 1
        for w in fold_defines(p):
            print('\n⛔ %s 「%s」的定义只在折叠里，而主线上已经用了它'
                  % (os.path.basename(p), w))
            print('    —— 读者默认是关着折叠读的，他会撞见一个从没被解释过的词。')
            print('    ⭐ 折叠是放「怎么做到的」的，不是放「它是什么」的：'
                  '定义留主线，机制随便折。')
            bad += 1
...
if __name__ == '__main__':
    ...
    main(sys.argv[1:] or [os.path.join(W, f) for f in
                          ('topic-02-L300.html', 'topic-02.html', 'topic-01.html',
                           'topic-03.html', 'topic-03-L300.html', 'topic-04.html', 'topic-08.html',
