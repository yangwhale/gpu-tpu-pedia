...
<div class="hero"><div class="wrap">
  ...
  <div class="chips">
    ...
    <span class="chip">⏱ <b>讲约 52 分钟</b></span>
  </div>
...
''' + sec("s三", "三", "第二刀：切权重") + '''
  ...
  <p>换回砌墙的说法：FSDP 送的是「工具」（权重），一件工具送到，这一批每个 token 都要拿它算一遍，token 越多，同一趟送货够师傅忙得越久。</p>
__FIG_INTENSITY__
...
<figcaption>通信只在最后那一下；可每一层都有这一下。
  ...
  <p>流水线并行把模型按层切成几段，每张卡负责一段，段与段之间只在边界上用一对一收发传激活，
    是所有刀里通信最少的，所以它<b>能跨到慢线上</b>。代价是<b>气泡</b>：</p>
__FIG_PP_BUBBLE__
