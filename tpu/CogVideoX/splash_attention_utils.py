...
def tpu_splash_attention(query, key, value, mesh, scale=None):
    ...
    分片策略与 stage2_transformer.py 保持一致：
    - Self-attention (q_seq_len == kv_seq_len): head parallel P('dp', 'tp', None, None)
    - Cross-attention: sequence parallel P('dp', None, 'tp', None)
    """
    ...
    # 分片策略：与 stage2_transformer.py 保持一致
    # 使用字符串而不是元组，确保 PartitionSpec 正确
    if num_heads < mesh.size:
        ...
        if q_seq_len == kv_seq_len:
            # Self-attention: head parallel（按 head 维度分片）
            q_spec = P('dp', 'tp', None, None)
            kv_spec = P('dp', 'tp', None, None)
        else:
            # Cross-attention: sequence parallel（按序列维度分片）
    ...
    # 输出 sharding constraint - 与输入保持一致，避免额外通信
    out = jax.lax.with_sharding_constraint(out, q_spec)
