...
> - **唯一稳定路径**: 5-shot Q/A completion pattern + `enable_thinking:false`（GSM8K 93–97% 就是用这个）
...
# 输出 ≥1 = 已 patch；输出 0 = 需要 patch（见 Step 2）
```
...
**生产 workaround**（按可靠性）：
1. ⭐ Chat + 5-shot Q/A pattern + `enable_thinking:false`（**唯一稳定**, GSM8K 实测 93–97%）
...
|---|---|---|---|---|---|---|
| **1. 单机** | 1 × v7x-8 (8 chips, TP=8) | low-latency / batch eval / GSM8K / **大多数生产用例** ⭐ | ~7 min | 1 (PR #2366) | 低 | 🟢 全验证 (perf + GSM8K) |
| **2. PD 分离 (1P1D)** | 2 × v7x-8 (TP=8 P + TP=8 D) | 长 prompt 高吞吐 / **TPOT 优化** / 大并发 | ~10 min | 2 (PR #2366 + HMA) | 中 | 🟡 部署+smoke ✅, perf 待补 |
| **3. Multi-host TP=16** | 2 × v7x-8 (16 chips, TP=16, LWS) | **超大模型 (>500B) / 验证 multi-host capability** | ~11-30 min | 3 (PR #2366 + 2 mrope bypass) | 高 | 🟡 部署+smoke ✅, perf 待补 |
...
# 期望: 94  <newline>  ≥1   (94 shards + PR #2366 markers; ≥1 = 已 patch)
...
kubectl exec $POD -- grep -c '_hybrid_uniform_page_size_bytes' \
  /workspace/tpu_inference/tpu_inference/runner/kv_cache_manager.py
# 输出 ≥1 = 已 patch, 跳过；输出 0 = 走下面 patch
```
...
| 症状 | 根因 | 修复 |
|---|---|---|
| **多并发输出乱码 / OOM `vmem 86MB > 64MB` / `HBM 95G > 94.75G` / EngineCore silent crash** | 缺 PR #2366 (KV cache 状态损坏) | 走 [Step 2](#step-2-应用-pr-2366-patch如未-patch)，grep 应输出 ≥1 |
