...
**⚠️ 出处口径。** sm_120a 跟 sm_100a 的区别出自 PTX ISA 的 Target ISA Notes；
...
def _gates(f):
    ...
    cards = [
        ...
        (PU, "TPU 那道门槛：这批数据的取行重复度够不够高",
         ...
         # ⛔ 2026-09-06 改：错在拿序列长度当不重复行数（详见 topic02-build-L200.py 同处注释）。
         "<r>而语言模型的取行重复度不够高</r>"
         "（<b>不是没有重复</b>：十几万 token 里不同的词只有几千个，绝大部分命中最高频那一两千行；"
         "只是跟搜广推差着量级）—— 落在最不划算的一端，"
...
def _src(f):
    ...
    y = para(f, 38, SRC_Y + 50, 1324,
             ...
             #    「出自我自己前面那张图」不是出处。
             "<b>查到的</b>：sm_120a 跟 sm_100a 的区别出自 PTX ISA 的 Target ISA Notes；"
