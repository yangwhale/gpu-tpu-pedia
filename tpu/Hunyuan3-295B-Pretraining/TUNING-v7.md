...
> ⛔ **本节的 FP8 +50%（1,014.8）已作废** —— 那是漏算 189/192 个专家换来的，见 §3.4.10。
> **BF16 +26%（662.3）不受影响**，它走的是按 embed 维切的安全路径。
...
| FP8+QAG | ⛔ **(512,2048,1536) 不开 gmm** —— 该行作废，漏算 189/192 专家（§3.4.10）；补齐后 637.0，低于 tokamax 路径的 **677.0** | — | ~~1,014.8~~ | — |
...
#### 3.4.7 ~~profile 对拍：那 49.9% 省在哪~~ ⛔ 前提已失效

> 本节分析的「49.9% 提升」不存在（§3.4.10）。profile 方法本身可参考，结论不要引用。
...
#### 3.4.8 ~~已定案：native 没有算少~~ ⛔ 本节结论错误，已删除

> 此处原有一节（2026-08-15）断言「native 路径没有算少，FP8 配方可以用」，
> 依据是 KL 散度与 32 步 loss 对拍。**结论是错的** —— 那两个判据对分片错误
> 都不敏感（原因见 §3.4.10 末）。真相见 **§3.4.10**。
>
> 那一轮摸索出的**方法**仍然有效，已抽成独立文档：
> [换 kernel 的等价性验证 —— KL 散度做法](../kernel-equivalence-validation/)。

#### 3.4.9 FP8 的 tile 空间已到顶（2026-08-15 扫描，64 芯片）

> ⚠️ 本次扫描是在**漏算配置**上做的（对照组 base = 1,014.6）。
> 三个维度的**上限约束**（mlp 已取满、embed 撞 Mosaic 限制）与 bug 无关，仍然成立；
> 但**绝对数值全部作废**，补齐 all-gather 后需重扫。
