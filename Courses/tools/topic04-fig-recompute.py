# -*- coding: utf-8 -*-
r"""专题四 · §2.2「全量重算：用三分之一的算力，换掉二十四分之二十三的显存」
...
#   于是这一组数整体下修：一层 MoE 块 71.14 → 55.39 GiB@128K，
...
def main():
    f = Fig(W, "全量重算这笔交易的两边：算力从三倍变四倍，只多付三分之一；"
               "而激活显存从四点一五 TiB 掉到一百七十七点九 GiB，只剩原来的百分之四点二。"
               "付出那边几乎没变，省下那边几乎归零 —— "
               "所以在大模型训练里它默认就是开着的")

    y0 = f.header(
        "全量重算　——　<tspan font-weight=\"700\">"
        "用三分之一的算力，换掉二十四分之二十三的显存</tspan>",
        "⭐ 两边都归一到「原来 ＝ 100%」"
    ...
    LEVELS = (
        ...
        (BL, "#e8f0fe", "选择性", "按比值挑着扔",
         "⭐ 128K：约 1.9% 算力换约 77% 显存（4K 是 9.3%）——　性价比最高"),
    ...
    #   ⭐ 而 V3 上这两个量是 1.75 : 55.39 ≈ **1 : 32**。
    #   正确的目标函数是 c·(L/k) + a·k，最优 k ＝ √(cL/a)：
    _C_CKPT, _A_LAYER, _L_V3 = 1.75, 55.39, 61        # GiB@128K / GiB@128K / 层
    _K_OPT = (_C_CKPT * _L_V3 / _A_LAYER) ** 0.5
    assert abs(_K_OPT - 1.39) < 0.02, "V3 的最优密度算出来是 %.2f" % _K_OPT
    #   ⇒ **k ≈ 1.4，也就是「基本每层都存」** ——&#160;跟「最优点在中间」正好相反。
