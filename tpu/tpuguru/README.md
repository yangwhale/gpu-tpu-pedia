...
减层跑得快（4–8 层比 80 层快约 3 倍，配置错误照样暴露），所以调配置时经常改层数。
但**常驻显存是所有层的和**，实测档位全是生产层数跑的 —— 拿它们回答一个减层配置，
就是「用 A 的结论回答 B」。

所以层数进了 replay 的匹配键：**层数不一致就不给实测结论**，改走 `mode=projected`：

```
📐 推算：80 层 → 40 层（比例 0.50）
① 这个 batch 在生产层数下有实测点，得 93.19 GB
② 常驻按参数量精确重算 23.34 → 11.71 GB（嵌入层不随层数变，已单独扣出）
   激活按层数比例折算

⚠️ 这是推算，不是实测。用它挑起始 batch 可以，定上限必须真跑 AOT。
```

推算模式在视觉上跟实测完全分开：黄色横幅、标题写「📐 推算：装得下」、顶部标 `推算模式`。

#### 三段拆解 —— 不拆，反解出的 batch 上限会大一倍

第一版反解 batch 上限时假设「非常驻 = 随 batch 走」，结果推荐 batch 59，实跑 46 就爆。
拆开看才发现：

| | 80 层 absmax FSDP128 |
|---|---|
| 非常驻 ÷ batch | 72.17 ÷ 13 = **5.55 GB/档** |
| 相邻档位实测斜率 | **2.81 GB/档** |

差的那 ~37.8 GB 是**跟 batch 无关的激活与临时缓冲**（梯度缓冲、通信缓冲、编译器工作区）。
所以模型必须是三段：

```
峰值 = 常驻 + 固定激活 + 每档 × batch
94.74 = 11.71 + 18.92 + 1.41 × batch      （40 层）
        → 反解上限 45，保守推荐 44
```

**验证**：按推荐值逐档真跑 —— 44 ✅ 92.45 / 45 ✅ 93.85 / 46 ❌ 95.26。反解精确命中。

这个三段拆解同时改进了生产层数下的余量卡：「一个 batch 增加多少显存」现在是
实测斜率，不是「非常驻平摊」。
