...
>
> **代码仓库**: [vllm-project/tpu-inference](https://github.com/vllm-project/tpu-inference)（main branch ≥ **2026-05-15**, 含 PR #2366 + [PR #2577](https://github.com/vllm-project/tpu-inference/pull/2577) commit `04077875`）
>
...
> - **5-shot Q/A pattern** 仍是 GSM8K eval 的标准方式，但**不再是 chat 的唯一稳定路径**
>
> **决定性证据 — GPU vs TPU 对照**（同模型 / TP=8 / FP8 attention quant / prompt `"What is the capital of France?"` / `enable_thinking=True`）：
>
> | 平台 | 输出 |
> |---|---|
> | **GPU** (TP=8, FP8) | `The user is asking for the capital of France. ... </think> The capital of France is **Paris**.` ✅ |
> | **TPU v7x-8** (TP=8 EP=8, FP8, **缺 PR #2577**) | `Wait, I need to think about about about about about about...` ❌ 退化为单 token 高频死循环 |
> | **TPU v7x-8** (TP=8 EP=8, FP8, **含 PR #2577**) | `... </think> The capital of France is **Paris**.` ✅ |
>
> 同模型同 TP=8 同 FP8 attention quant，GPU 正常输出 Paris，TPU 在缺 PR #2577 时退化为高频 token 单峰死循环 → **bug 在 TPU 推理栈的 GDN bf16 数值通路**，不在模型本身。这也是 5-shot Q/A pattern 历史上能"绕过"的原因（首 token 走 Standard Attention 而不是 GDN scan）。
>
> **升级方法**: 用 main branch ≥ 2026-05-15 (含 commit `04077875`)。验证方法：
> ```bash
> kubectl exec $POD -- grep -c 'jnp.float32' \
>   /workspace/tpu_inference/tpu_inference/layers/jax/attention_interface/recurrent_scan_v2.py
> # 输出 ≥5 = 已含 PR #2577；输出 <5 = 需升级
> ```
...
| **GSM8K full 1319 (5-shot, thinking OFF)** | **93.93%–97.26%** ✅ | 随 nightly 版本变化, 详见 [GSM8K](#gsm8k-accuracy5-shot-thinking-off-via-in-context-15-min) |
...
### A. PR #2366 + PR #2577 patches（**必须**）

两个独立的 fix，缺一个症状不同；都必须含。验证一次跑双 grep：

```bash
kubectl exec $POD -- bash -c "
  grep -c '_hybrid_uniform_page_size_bytes' /workspace/tpu_inference/tpu_inference/runner/kv_cache_manager.py
  grep -c 'jnp.float32' /workspace/tpu_inference/tpu_inference/layers/jax/attention_interface/recurrent_scan_v2.py
"
# 期望: ≥1  <newline>  ≥5    (≥1 = PR #2366 已 patch；≥5 = PR #2577 已 patch)
```
...
Qwen3.5 默认 thinking ON（输出 `<think>...</think>` reasoning + 答案）。

> **2026-05-15 PR #2577 修复后**，单 user message chat + `enable_thinking:false` 已稳定，不再依赖 5-shot Q/A pattern 兜底。下表为历史踩坑记录：

| 尝试 | 实测（含 PR #2577） |
|---|---|
| `--chat-template-kwargs='{"enable_thinking":false}'` (启动 flag) | silently 忽略（CLI 限制） |
| Request body `chat_template_kwargs={"enable_thinking":false}` + 单 user message | ✅ **work**, reasoning_len=0（PR #2577 修复前会陷入 `</think>` 死循环） |
| 5-shot Q/A pattern + `enable_thinking:false` | ✅ work（GSM8K eval baseline，实测 93–97%） |
| User prompt 加 `/no_think` 标记 | 失效（chat template 不识别） |

**生产推荐**（按场景）：
1. ⭐ **Chat（单 user message）+ `enable_thinking:false`** — PR #2577 后稳定，覆盖绝大多数交互式场景
2. **5-shot Q/A pattern + `enable_thinking:false`** — GSM8K / few-shot eval baseline
3. **接受 thinking ON + `max_tokens` ≥ 3500** — 需要完整 reasoning trace 的长答案场景
4. `/v1/completions` raw prompt — 仅 debug 用，不推荐生产

---

## 🧭 部署模式选择

| 模式 | TPU | 适合场景 | 启动时间 | Patches | 复杂度 | Dogfood 状态 |
|---|---|---|---|---|---|---|
| **1. 单机** | 1 × v7x-8 (8 chips, TP=8) | low-latency / batch eval / GSM8K / **大多数生产用例** ⭐ | ~7 min | 2 (PR #2366 + PR #2577) | 低 | 🟢 全验证 (perf + GSM8K + chat) |
| **2. PD 分离 (1P1D)** | 2 × v7x-8 (TP=8 P + TP=8 D) | 长 prompt 高吞吐 / **TPOT 优化** / 大并发 | ~10 min | 3 (PR #2366 + PR #2577 + HMA) | 中 | 🟡 部署+smoke ✅, perf 待补 |
| **3. Multi-host TP=16** | 2 × v7x-8 (16 chips, TP=16, LWS) | **超大模型 (>500B) / 验证 multi-host capability** | ~11-30 min | 4 (PR #2366 + PR #2577 + 2 mrope bypass) | 高 | 🟡 部署+smoke ✅, perf 待补 |
...
## ⚡ Quick Start (老手 5 命令复现单机)

> 已部署过 / 熟悉 GKE TPU + vLLM？直接复用下面 5 命令，省去读后面 200 行。新用户请走 [部署模式 1 完整步骤](#部署模式-1单机-vllm-serve)。

```bash
CTX=<your-gke-context>; POD=<your-tpu-pod>; MODEL=/lustre/models/Qwen3.5-397B-A17B-FP8

# 1. 验证模型 + 双 patches (PR #2366 + PR #2577)
kubectl --context=$CTX exec $POD -- bash -c "ls $MODEL/*.safetensors | wc -l && grep -c '_hybrid_uniform_page_size_bytes' /workspace/tpu_inference/tpu_inference/runner/kv_cache_manager.py && grep -c 'jnp.float32' /workspace/tpu_inference/tpu_inference/layers/jax/attention_interface/recurrent_scan_v2.py"
# 期望: 94  <newline>  ≥1  <newline>  ≥5   (94 shards + PR #2366 + PR #2577 都已 patch)

# 2. 写 file-based launcher 到 host (kubectl exec 的 multi-line nohup 会被 SIGKILL)
...
# 4. 等 ~7 min cold start (轮询 health)
...
> 💡 **PD / Multi-host quick start** 见 [部署模式 2](#部署模式-2pd-分离1p1d) / [部署模式 3](#部署模式-3multi-host-tp16-2--v7x-8--实测-2026-04-26--lm-推理-55-pass) 的 Step 0-7。这两个模式比单机多 patches + Ray + LWS, 不适合一行式速查。
...
### Step 2: 应用 PR #2366 + PR #2577 双 patch（如未 patch）

> ⚠️ **两个 patch 一起做**：缺 #2366 = gibberish/OOM/EngineCore crash；缺 #2577 = chat 死循环（`about about about...` / `\n/**\n/**`）。装 vLLM 镜像 commit ≥ 2026-05-15 应已同时包含两者，先跑 dual grep 检查。
...
# 第一行 ≥1 = PR #2366 已 patch；第二行 ≥5 = PR #2577 已 patch
# 任一为 0 = 走对应小节 patch
```

#### 2a. PR #2366 patch（KV cache padding）
...
grep -c '_hybrid_uniform_page_size_bytes' $TMP   # 应输出 ≥1
...
#### 2b. PR #2577 patch（GDN scan bf16→fp32 + chunk_size 64→32）

> ⚠️ **两个文件必须一起 patch**：`recurrent_scan_v2.py` (fp32 upcast 5 处) + `gdn_attention.py` (`chunk_size` 64→32)。只补一个会引起 shape/numeric 不匹配。

```bash
# Host 端 — 拉两个文件
TMP_RS=$(mktemp /tmp/recurrent_scan_v2.XXXXXX.py)
TMP_GA=$(mktemp /tmp/gdn_attention.XXXXXX.py)
curl -sf https://raw.githubusercontent.com/vllm-project/tpu-inference/main/tpu_inference/kernels/gdn/recurrent_scan_v2.py -o $TMP_RS
curl -sf https://raw.githubusercontent.com/vllm-project/tpu-inference/main/tpu_inference/layers/common/gdn_attention.py -o $TMP_GA
grep -c 'jnp.float32' $TMP_RS         # 应 ≥5
grep -n 'chunk_size' $TMP_GA | head -3  # 应看到 chunk_size=32

# Pod 内备份 + cp + verify
RS=/workspace/tpu_inference/tpu_inference/kernels/gdn/recurrent_scan_v2.py
GA=/workspace/tpu_inference/tpu_inference/layers/common/gdn_attention.py
kubectl --context="$CTX" exec $POD -- cp $RS ${RS}.bak
kubectl --context="$CTX" exec $POD -- cp $GA ${GA}.bak
kubectl --context="$CTX" cp $TMP_RS $POD:$RS
kubectl --context="$CTX" cp $TMP_GA $POD:$GA
kubectl --context="$CTX" exec $POD -- bash -c "
  grep -c 'jnp.float32' $RS                    # verify ≥5
  grep -n 'chunk_size' $GA | head -3           # verify chunk_size=32
  find /workspace/tpu_inference -name '__pycache__' -type d -exec rm -rf {} + 2>/dev/null || true
"
rm -f $TMP_RS $TMP_GA
```
...
**等待看到关键 log（PR #2366 + PR #2577 + 启动完成的标志）**：
```
Hybrid KV cache: padding every layer spec to XXXXXXXX bytes ...   ← PR #2366 padding (具体值随 nightly 版本变化)
regular_attn_shape=(num_blocks, (...))                            ← block_size (patch 前是错的 4352)
num_gpu_blocks_override=NNN
INFO: Application startup complete.
```

> 💡 **padding bytes / num_gpu_blocks 随 nightly 版本变化**：早期 nightly 是 `23289856 bytes / 945 blocks`，新版 nightly（含 compact-mamba 优化）可能是 `12804096 bytes / 1718 blocks`。只要看到 `Hybrid KV cache: padding` log 就说明 PR #2366 生效——具体数值不需要对齐。

> 💡 **PR #2577 没有专属 startup log**——它的验证靠 Step 2 dual grep（`recurrent_scan_v2.py` 的 `jnp.float32` ≥5）和 Step 4a 的 chat smoke test（finish=stop 且 content 非死循环）。如果 Step 2 grep 通过但 Step 4a 看到 `about about about...` / `\n/**\n/**` 重复，说明 patch 文件被 `__pycache__` 旧字节码覆盖——回去 `find /workspace/tpu_inference -name '__pycache__' -exec rm -rf {} +` 再 restart。

### Step 4: 验证（chat smoke test）

> ⚠️ **首次请求 XLA 编译**: 每种新的 prompt shape 首次请求会触发 XLA 编译（60-90 秒），此后同 shape 请求 <1 秒。Smoke test 的第一次 curl 如果超时不要慌——等编译完成后重试即可。建议 curl 加 `--max-time 120`。
...
#### 4a. ⭐ Chat 单 prompt（PR #2577 修复后主推）

```bash
# 单 user message chat — PR #2577 后稳定（缺 PR #2577 会陷入 about/\n/** 死循环）
kubectl exec $POD -- curl -s --max-time 120 http://localhost:8000/v1/chat/completions \
  -H 'Content-Type: application/json' \
  -d "{\"model\":\"$MODEL\",\"messages\":[{\"role\":\"user\",\"content\":\"What is the capital of France? Answer in one sentence.\"}],\"max_tokens\":80,\"temperature\":0,\"chat_template_kwargs\":{\"enable_thinking\":false}}" \
...
# 期望: content: 'The capital of France is Paris.' (或同义短句)  reasoning_len: 0  finish: stop
# ⚠️ 若 finish: length 且 content 是 'about about about...' / '\n/**\n/**...' 死循环 → PR #2577 未 patch, 回去跑 Constraint A 的 dual grep
```

#### 4b. GSM8K eval baseline（5-shot Q/A pattern）

```bash
# 5-shot Q/A pattern — GSM8K / few-shot eval 使用, 实测 93–97%
SHOTS="Question: Capital of Japan?\nAnswer: Tokyo.\n\nQuestion: Capital of Germany?\nAnswer: Berlin.\n\nQuestion: Capital of Italy?\nAnswer: Rome.\n\nQuestion: Capital of Spain?\nAnswer: Madrid.\n\nQuestion: Capital of Brazil?\nAnswer: Brasilia.\n\n"
...
**预期（4a 和 4b 都要过）**：`finish: stop` + content 合理。两个都过 = PR #2366 + PR #2577 双 patch 生效。
...
### Step 2: Stage HMA + PR #2366 + PR #2577 patches 到 Lustre
...
# 拉 PR #2577 两个 patch 文件（GDN bf16 → fp32 upcast + chunk_size 64→32）
# commit 04077875 (2026-05-15 已 merge to main)
curl -fsSL https://raw.githubusercontent.com/vllm-project/tpu-inference/main/tpu_inference/kernels/gdn/recurrent_scan_v2.py \
  -o /tmp/recurrent_scan_v2.py
curl -fsSL https://raw.githubusercontent.com/vllm-project/tpu-inference/main/tpu_inference/layers/common/gdn_attention.py \
  -o /tmp/gdn_attention.py
kubectl cp /tmp/recurrent_scan_v2.py $POD:/tmp/recurrent_scan_v2.py
kubectl cp /tmp/gdn_attention.py $POD:/tmp/gdn_attention.py
...
kubectl exec $POD -- bash -c "
  ...
  cp /tmp/recurrent_scan_v2.py /lustre/patches/qwen35-pd/
  cp /tmp/gdn_attention.py /lustre/patches/qwen35-pd/
  # verify
  ...
  echo 'PR #2577 recurrent_scan fp32:' \$(grep -c 'jnp.float32' /lustre/patches/qwen35-pd/recurrent_scan_v2.py) '(expect ≥5)'
  echo 'PR #2577 gdn chunk_size:' \$(grep -c 'chunk_size.*=.*32' /lustre/patches/qwen35-pd/gdn_attention.py) '(expect ≥1)'
"
```

> 💡 init container 在 Step 3 会从 `/lustre/patches/qwen35-pd/` cp 这 4 个文件到 `/workspace/tpu_inference/` 对应目录：`tpu_connector_hma.py` → `scripts/`, `kv_cache_manager.py` → `tpu_inference/runner/`, `recurrent_scan_v2.py` → `tpu_inference/kernels/gdn/`, `gdn_attention.py` → `tpu_inference/layers/common/`。每次 pod 启动自动覆盖 + 删 `__pycache__`，不需要手动 cp 进 pod。
...
3 个 manifest 自带 init container：每次 pod 启动从 Lustre `/lustre/patches/qwen35-pd/` cp HMA + PR #2366 + PR #2577 共 4 个文件到 `/workspace/tpu_inference/` 对应子目录，删 `__pycache__`，verify grep。

> ⚠️ **如果 yaml init container 还在用旧版（只 cp 2 个文件）**：先编辑 `manifests/qwen35_prefill.yaml`、`qwen35_decode.yaml` 的 initContainers 段，把 cp 列表扩到 4 个文件（tpu_connector_hma.py → scripts/、kv_cache_manager.py → tpu_inference/runner/、recurrent_scan_v2.py → tpu_inference/kernels/gdn/、gdn_attention.py → tpu_inference/layers/common/），再 apply。否则即便 Lustre 有 4 文件，pod 内只会覆盖 2 个，chat 仍会死循环。
...
# 一层验证：HMA + PR #2366 + Hybrid KV manager 全 enable（log 有信号）
PREFILL_POD=$(kubectl --context="$CTX" get pods -l app=vllm-prefill -o jsonpath='{.items[0].metadata.name}')
DECODE_POD=$(kubectl --context="$CTX" get pods -l app=vllm-decode -o jsonpath='{.items[0].metadata.name}')
kubectl --context="$CTX" logs $PREFILL_POD | grep -E "TPUConnectorHMA|Hybrid KV cache: padding|Application startup"

# 二层验证：PR #2577 在 pod 内已生效（fp32 upcast 是 kernel 内部精度，无启动 log，只能 grep 源码）
for POD in $PREFILL_POD $DECODE_POD; do
  echo "--- $POD ---"
  kubectl --context="$CTX" exec $POD -- bash -c "
    echo 'PR #2577 recurrent_scan fp32 upcast:' \$(grep -c 'jnp.float32' /workspace/tpu_inference/tpu_inference/kernels/gdn/recurrent_scan_v2.py) '(expect ≥5)'
    echo 'PR #2577 gdn chunk_size 32:'         \$(grep -c 'chunk_size.*=.*32' /workspace/tpu_inference/tpu_inference/layers/common/gdn_attention.py) '(expect ≥1)'
  "
done
```

**期望看到**（具体数值随 nightly 版本变化，只要 pattern 匹配即可）：
```
...
> ⚠️ **PR #2577 没有启动 log 信号**：fp32 upcast 跑在 GDN kernel 内部，只能通过 (1) 上面的源码 grep 静态确认 patch 在 pod 内 + (2) [Step 5 Smoke Test](#step-5-smoke-test验证-pd-kv-传输工作) 通过 chat 不死循环动态验证。如果 grep count 不达预期，多半是 init container cp 漏了文件或 `__pycache__` 没删干净。
...
- `Hybrid KV cache: padding every layer spec to XXXXXXXX bytes` ← multi-host PR #2366 padding (数值随 nightly 版本变化，比单机小因 TP=16 切分更细)
- `Hybrid KV cache layout: num_kv_cache_groups=4, ... num_blocks=NNNN` ← TP=16 num_blocks 比单机大数倍
...
> 💡 **新版 nightly 准确率更高**：2026-05-10 复测（`vllm-tpu:nightly` v0.20.2rc1.dev178，含 compact-mamba 优化），621/1319 部分样本实测 **97.26%** (604/621)。新版 KV cache 数值优化改善了数值精度。如果你的 nightly 较新，GSM8K 结果可能高于 93.93%。
...
| 症状 | 根因 | 修复 |
|---|---|---|
| **chat 单 user message 输出 `about about about...` / `\n/**\n/**` 死循环 / 语言错乱 / Thinking 空输出** | 缺 PR #2577（GDN recurrent scan kernel bf16 数值不稳定） | 走 [Step 2b](#b-pr-2366--pr-2577-patches必须)，dual grep 第二个数字应 ≥5；GPU vs TPU 对照见[已知关键限制](#-已知关键限制历史-bug已修复) |
| **多并发输出乱码 / OOM `vmem 86MB > 64MB` / `HBM 95G > 94.75G` / EngineCore silent crash** | 缺 PR #2366 (KV cache 状态损坏) | 走 [Step 2](#b-pr-2366--pr-2577-patches必须)，dual grep 第一个数字应 ≥1 |
...
| **首次请求超时 / curl 返回空** | 每种新 prompt shape 首次触发 XLA 编译（60-90s） | 加 `--max-time 120` 重试，后续同 shape 请求 <1s |
...
### 上游 PR Watchlist（何时可以删 patches）

> 当 vllm-tpu nightly image 升级到 ≥ PR merge 日期后，对应 patch 可删。当前 image 用的 `nightly-20260330` 比所有 4 个 PR 都早 3-4 周，所以 patches 仍必须。

| PR | Author | Merged | Status | 哪个 patch / 模式依赖 |
|---|---|---|---|---|
| [#2322](https://github.com/vllm-project/tpu-inference/pull/2322) | wyzhang | 2026-04-20 | ✅ merged | PD: transfer stats tracker (无需 cp，已在 main) |
| [#2336](https://github.com/vllm-project/tpu-inference/pull/2336) | wyzhang | 2026-04-22 | ✅ merged | PD: `tpu_connector_hma.py` (cp 进 pod) |
| [#2366](https://github.com/vllm-project/tpu-inference/pull/2366) | qizzzh | 2026-04-23 | ✅ merged | **全模式: `kv_cache_manager.py`** (cp 进 pod) |
| [#2577](https://github.com/vllm-project/tpu-inference/pull/2577) | qizzzh | 2026-05-15 | ✅ merged (`04077875`) | **全模式 (chat 必须): `recurrent_scan_v2.py` (bf16→fp32, 5 处) + `gdn_attention.py` (chunk_size 64→32)** (cp 进 pod) |
| 🔬 mrope bypass (内部) | — | 待 upstream | ⚪ 内部 patch | Multi-host: `tpu_runner.py` + `persistent_batch_manager.py` (5+4 行 defensive None check) |

**升级 image 后的清理流程**:
1. `kubectl exec $POD -- ls /workspace/tpu_inference/.git/HEAD` 查 main commit hash
2. 对比 PR merge 日期：image 内 commit ≥ PR commit → 该 patch 可删
3. 删 init container 里的 `cp ... patches/` + verify counter 行
4. **mrope bypass 不要删** — 还没 upstream PR，是 multi-host 必须

> 💡 **未来方向**: mrope bypass 应 file PR 到 vllm-project/tpu-inference。修复点：当 `disable_mm_from_limits=True` 时，`PersistentBatchManager.__init__` 应该 lazy 读 `runner.uses_mrope` 而不是 init 时 capture。
