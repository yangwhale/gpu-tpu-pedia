...
⭐⭐⭐ 而这张图在**本讲**里的位置，是一口气把三处接上（三条都是本讲自己的合题）：
  ...
  · 接 §1.6 ——&#160;「按上游亮度成比例」解释了**为什么反向必须用到前向的值**：
    ...
    **而那个亮度是前向算出来、被存下来的。** 激活扔不掉的根子就在这儿。
    ⚠️ 2026-09-23 起这一格**不在主图里了** ——&#160;拆成 `fig4-vote-bright.svg`，
      教材里折起来（现场：「这个内容插入的就有点突兀，把它折起来，不放这张」）。
...
def main():
    ...
    # ══════════ Ⓑ 四千多股力，加成一股 ═══════════════════════════
    # ⛔⛔⛔ 2026-09-23 现场，第二次说同一格：「这个内容插入的就有点突兀，
    #   把它折起来，不放这张。」——&#160;指的是原来夹在中间的那一格
    #   「上游越亮、加粗越划算」。
    #   ⭐ 上一轮我把它**重画 ＋ 改名**（补上「梯度」两个字）就交卷了，
    #     可他这次抱怨的**不是那一格讲不清，是它不该在这儿** ——&#160;
    #     这张图的主线是「一屋子人各提各的要求 →&#160;取平均」，
    #     中间插一段「哪根线改起来最划算」，是**另一条轴**。
    #   ⭐⭐ 判据：**同一个抱怨来第二次时，别再修上一次修的那个维度。**
    #     第一次说「看不懂」，改讲法是对的；第二次还说，就该问「它在不在位置上」。
    #   ⇒ 整格拆成独立的一张（fig4-vote-bright.svg），教材里折起来。
    #     ⚠️ 没删 ——&#160;「上游越亮越划算」仍是整本显存账的根子（§1.6），
    #       只是从主线降级成「想看再展开」。
...
# 折起来的那一张：上游越亮，加粗越划算
# ══════════════════════════════════════════════════════════════════
# ⭐⭐ 为什么要单独成一张，而不是在主图里留着不画：
#   「这一段能不能折叠」看着是排版问题，**它先是个切分问题** ——&#160;
#   一张 SVG 要么整张在、要么整张不在，想折的那块得先是一个独立的产物，
#   否则折叠这个动作根本没有抓手。（同 fig4-slider-jac 那次）
...
def bright():
    f = Fig(W, "想让一个数变大有三条路：改偏置、改权重、让上一层更亮。"
               "这一格只看改权重。两根线连到同一个输出，"
               "上面那根的上游很亮是四点零，下面那根很暗只有零点二。"
               "把两根加粗同样多，都加零点一，"
               "上面那根让输出涨零点四，下面那根只涨零点零二 —— "
               "同样的力气，回报差二十倍，而这二十倍正好是亮度之比。"
               "说的就是梯度：一根权重的梯度等于上游那一刻的亮度乘下游传来的责任，"
               "这一格画的是前面那个乘数。"
               "所以改哪根线最划算，答案不在反向里，在前向里 —— "
               "而那个亮度是前向算出来、必须被存下来的，"
               "整本显存账的根子就在这一句上")

    y0 = f.header(
        "一根权重的<tspan font-weight=\"700\">梯度</tspan>，"
        "正比于<tspan font-weight=\"700\">上游有多亮</tspan>",
        "⭐ 两根线<tspan font-weight=\"700\">加粗同样多</tspan>，回报差二十倍"
        "　——　而「回报有多大」就是「<tspan font-weight=\"700\">这根权重的梯度有多大</tspan>」",
        [(OR, "上游亮"), (GY2, "上游暗"), (GR, "回报之比 ＝ 亮度之比")])

    PH2 = 372
    py2 = f.panel(0, y0, W, PH2,
                  "同样加粗 <tspan font-weight=\"700\">0.1</tspan>，"
                  "接在亮的那根上<tspan font-weight=\"700\">回报大 20 倍</tspan>", GR,
                  sub="⛔ 这一格是从 <tspan font-weight=\"700\">fig-vote</tspan> 里拆出来的"
                      "　——　它另开一根轴（「哪根线改起来最划算」），"
                      "所以不占主线")
    ...
    yb = f.src(py2 + PH2 + 18,
               "📌 「改权重要按上游亮度成比例，回报最大」取自 "
               "<tspan font-weight=\"700\">3Blue1Brown</tspan>"
               "《What is backpropagation really doing?》官方讲义"
               "（\"adjust the weights in proportion to their associated "
               "activations\"）——&#160;图是我们自己重画的。",
               "⚠️ 他在这里顺带提了赫布理论"
               "（neurons that fire together wire together），"
               "但<tspan font-weight=\"700\">原文自己就说这个类比并不严格</tspan>，"
               "所以只记在这儿，不画上图。",
               "⭐ 「所以激活扔不掉」那一步是<tspan font-weight=\"700\">本讲自己的合题</tspan>，"
               "3B1B 讲的是「反向传播在干嘛」，本讲要的是「它为什么这么费显存」。")

    f.save("fig4-vote-bright.svg", yb + 14)
