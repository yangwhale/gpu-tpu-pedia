...
STATIONS = [
    ...
    # ⛔ 2026-09-05：这里原来写死 sub="MXU 的输入端"，而专题二的 P-39 写
    #    「MXU 那条支线从来不经过它」——&nbsp;**两张图对同一件事给了相反的说法**，
    #    而且 P-39 还在这上面架了一条叙事（「GPU 到 Blackwell 才追上」）。
    #    公开资料定不了这件事（v7 这一层官方本就没公开），按本课规矩：不主张。
    dict(c=GREY, name="③ 向量寄存器", sub="VPU 的输入端",
         ...
         miss="<g>v2/v3 的论文给的是每 sublane 32 深；v7 这一层官方没有公开，"
              "本文不拿旧代的数字顶替。<r>另外：矩阵操作数是从 VMEM 直接进 MXU、"
              "还是也要过这一层，公开资料同样没有明写 —— 本文不下结论。</r></g>"),
