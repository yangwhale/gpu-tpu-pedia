...
两个方块的**面积**按每周期乘加次数等比画，内部按**真实单元数**切分。
...
CARD_H = 152
C_T = BOX_B + 218         # 右侧「把账摊开」面板比左边两块高，section 要让到它下面
H = C_T + 14 + CARD_H + 14 + 76 + 18


def build():
    f = Fig(W, H, "B200 与 TPU v7 每周期乘加次数的等面积对照："
...
def _table(f):
    ...
    rows = [
        ("每周期乘加 · 总量", f"{GPU_MAC:,}", f"{TPU_MAC:,}", "1.16 ×", SUB, False),
        ...
        ("这些数怎么来的", "148 <b>第三方拆解</b>，×4 官方；每核 1,024 官方 + 推导",
         "4 MXU × 262,144 FLOP/周期 × 2.2 GHz，<b>三个数全是官方的</b>", "见左", YL, False),
    ]
    cw = [156, 150, 150, 92]
    ty = BOX_T + 4
    ...
    y = BOX_T + 22 + 6 * 52 + 6
    f.rect(x + 10, y, w - 20, 128, FILL[GN], GN, 1.4, 8)
    ...
    para(f, x + 22, yy + 6, w - 44,
         "<g>˟ 2,250 是 <b>HGX B200</b> 的口径。<b>NVL72 里那颗 GB200 更高</b> —— "
         "每 GPU dense BF16 约 <b>2,500</b>。<b>比域内算力时要用后者</b>，别混 SKU。</g>", "xs", 13)
