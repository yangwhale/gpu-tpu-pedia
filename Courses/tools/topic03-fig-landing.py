...
def main():
    ...
    yy = f.src(yy + 16,
               ...
               # ⭐ 2026-09-14 R61 补测量条件。R60 一度把 §三 整节删掉，
               #   这个号当场变成死指针 ——&#160;虽然 R61 又把整节接回来了，
               #   但那一下暴露出：**号是这几个数唯一的出处凭证**。
               #   ⛔ 出处不能只靠一个会随改版消失的指针，条件要写在正文里。
               "同样 seq=4096，本讲 §3.6 实测（Hunyuan3-295B · v7 64 芯片 · "
