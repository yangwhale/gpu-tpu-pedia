...
12. [📖 专题：JAX 并行编程详解](#12-专题jax-并行编程详解) ⭐ **新增**
...
**完整文档**：[JAX 并行编程详解](./JAX_PARALLEL_PROGRAMMING_GUIDE.md)

> **前置知识**：如果你是 JAX 初学者，建议先阅读 [PyTorch 到 JAX 入门教程](../../torch_to_jax_jumpstart/)，从基础开始学习 JAX 和 HuggingFace 模型的使用。

本专题深入解析 JAX 的并行编程机制，特别是 `shard_map`、`vmap`、`pmap` 的工作原理和使用场景。

### 核心内容

1. **shard_map 工作原理**
   - 显式控制多设备数据分片和计算
   - `in_specs` 和 `out_specs` 的作用机制
   - 自动分片、并行执行、自动合并的完整流程

2. **shard_map vs 传统方法**
   - 只用 `device_put` 会触发 all-gather 导致 OOM
   - `shard_map` 实现真正的计算分片
   - 内存使用对比：8GB → 1GB per device
...
5. **vmap vs pmap 本质区别**
   - vmap：所有数据在单个设备，向量化计算
   - pmap：数据分布在多个设备，真正并行
   - 内存布局和执行方式完全不同

6. **Mesh 的重要性**
   - pmap 的隐式设备管理和限制
   - shard_map 的显式 mesh 定义
   - 支持多维并行（数据并行 + 模型并行）
   - JAX 并行编程的演进历史
...
**核心教训：**
1. `device_put` 只是数据存放方式，不影响计算执行
2. `shard_map` 既是数据分片也是计算分片
3. 跨分片依赖需要显式编程（`lax.ppermute`）
4. mesh 是现代 JAX 并行编程的核心
5. 时序模型需要特别注意因果性
...
### 10.1 CogVideoX VAE Decode OOM 修复全记录 ⭐

这是一个经典的 TPU 内存优化案例，详细记录了从 19GB OOM 到成功解决的完整过程。

**问题**：CogVideoX VAE 在 TPU v6e (32GB HBM) 上解码时需要 19GB 内存，但只有 13GB 可用。

**解决方案演进**：
1. ❌ 移除 JIT（仍 OOM）
2. ⚠️ 逐帧解码 v1（只生成 4 帧，应该 16 帧）
3. ⚠️ 添加时序上采样（Frame 0 正确，Frames 1-3 输出 0）
4. ⚠️ 共享缓存（所有帧输出但数值错误）
5. ✅ **修复缓存保存逻辑**（完全成功）

**关键洞察**：
- 缓存应保存**拼接后 x** 的 last 2 frames，而非原始 inputs
- 参考 WAN VAE 实现的 5 行关键代码解决了问题
- 所有 latent frames 必须共享一个缓存管理器

**最终成果**：
- ✅ 内存占用：<13GB（降低 32%）
- ✅ 正确输出：16 frames（4 latent → 16 video）
- ✅ 性能：首次 50.64s，后续 33.88s（1.49x 加速）

**详细文档**：[CogVideoX VAE OOM 修复全记录](./CASE_STUDY_COGVIDEOX_VAE_OOM_FIX.md)

**可复用模式**：
- 逐帧处理 + 共享状态管理
- 双模式 API 设计（新旧兼容）
- Causal Convolution 缓存正确性模式
