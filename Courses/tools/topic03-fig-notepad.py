...
def main():
    ...
    # ══════════ ③ 板子为什么装不下 ══════════════════════════════
    # ══════════ ②b 「叉掉」叉的到底是什么 ══════════════════════
    # ⭐⭐⭐ 2026-09-13 新增。我们已经把「叉掉」钉在了式子里的 (I − βkkᵀ) 上，
    #   ⛔ 但没画出**它叉的是什么** —— 学生会以为是「擦掉一整块」。
    # 📌 装置出自 Songlin Yang 的 DeltaNet 系列（Householder 反射的几何图）
    #   ＋ Part III 的精确说法：**这个矩阵沿 k 方向的特征值是 1−β，
    #   所有垂直于 k 的方向特征值都是 1。**
    # ⭐ 一张图同时给三件事：叉的是**一个方向**不是一整块；
    #   β 是「叉多狠」的连续旋钮不是开关；k 为什么一定要归一化。
    y2b = y1 + PH2 + 18
    PH2B = 356
    py2b = f.panel(0, y2b, W, PH2B,
                   "②b 「叉掉」叉的是<tspan font-weight=\"700\">一个方向</tspan>，"
                   "不是一整块", OR,
                   sub="沿 k 方向压扁；所有垂直于 k 的方向，一动不动")
    import math as _m
    ey = py2b + 40
    for i, (bta, lab, note, col) in enumerate((
            (0.0, "β = 0", "什么都没擦", GY2),
            (0.5, "β = 0.5", "这个方向压掉一半", OR),
            (1.0, "β = 1", "这个方向擦干净了", GR),
            (1.7, "β > 1", "压过头，翻到平面另一侧", PU))):
        cx, cy2 = 200 + i * 330, ey + 96
        # 平面（＝所有跟 k 无关的方向）
        f.poly([(cx - 120, cy2 + 30), (cx - 40, cy2 - 6),
                (cx + 120, cy2 - 6), (cx + 40, cy2 + 30)], "#f1f3f4", LINE2, 1.2)
        # ⛔ 四格都标全称会互相压 —— 只在第一格标一次，后面三格靠位置认。
        if i == 0:
            f.t(cx - 120, cy2 + 52, "灰平面 ＝ 所有跟 k 无关的方向", GY2,
                size=15)
        # k 方向（法向量）
        f.line(cx, cy2 + 12, cx, cy2 - 76, GY2, 2.0)
        f.t(cx + 8, cy2 - 80, "k", GY2, True, 17)
        # 状态 S 在 k 方向上的分量，被压成 (1−β) 倍
        comp = (1.0 - bta) * 62
        f.line(cx, cy2 + 12, cx - 74, cy2 + 12 - comp, col, 3.0)
        f.t(cx - 86, cy2 + 12 - comp - 10, "S", col, True, 18, "end")
        f.t(cx, cy2 + 76, lab, col, True, 21, "middle")
        f.t(cx, cy2 + 102, note, GY, size=16, anchor="middle")

    f.box(56, ey + 214, 1288, 78, "#fff7ed", OR, 10)
    f.t(80, ey + 244, "⭐ 一张图三件事", OR, True, 21)
    f.t(280, ey + 244, "① 叉的是<tspan font-weight=\"700\">一个方向</tspan>，"
        "不是一整块　·　② β 是「叉多狠」的<tspan font-weight=\"700\">连续旋钮"
        "</tspan>，不是开关", GY, size=17, w=1040)
    f.t(280, ey + 272, "③ 也顺便解释了 <tspan font-weight=\"700\">k 为什么一定要"
        "归一化</tspan> ——　不归一化，这个「压扁」就不受控。", GY, size=17,
        w=1040)
