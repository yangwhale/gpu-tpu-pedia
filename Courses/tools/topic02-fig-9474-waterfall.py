...
rows=[("官方口径","206 × 10⁹ B","206 × 10⁹ B",CHIP,"#1a73e8",None,None,None),
...
p.append(f'<text class="svgsm" x="{(xa+xb)/2+102:.1f}" y="{y4+68}" fill="#d93025">'
         f'⚠️ 这条窄到几乎看不见的缝，就是 {RES:.2f} GiB —— 最容易忘的一步，往往也是最不起眼的那一步</text>')
...
p.append(f'<text class="svgsm" x="0" y="{yb+46}">这三问不是凑数 —— 上面三步各对应一个。'
         f'第 4 节讲 NVL72 时会在集群这一层再撞一次同样的病（72 张是物理域，能训练的是 64 张）</text>')
p.append('</svg>')
