...
L2 被切成四个分区（每个 die 两个，Hopper 的两倍）。第三方实测：
<b>当工作集小到只落在本地那个 L2 分区里时，整颗芯片能达到约 21 TB/s</b>；
一旦开始跨到对面 die，掉到 16.8 TB/s，延迟也变高。
<br><b>⚠️ 这两个数都是「整颗芯片达到的总带宽」，不是「一个分区的带宽」</b> ——
「本分区」说的是<b>数据落在哪</b>（测试条件），不是<b>谁在供带宽</b>（分母口径）。
不写清这一句，读者很容易乘以 4。
<br><em>两个佐证：① 原文的说法就是「footprints contained within the local L2
partition achieve 21 TB/s」，achieve 的主语是整机测试；
② 本课自己算过 L1＋共享内存整颗合计约 35 TB/s ——
<b>L2 的总带宽不可能高过 L1</b>，若 21 是每分区、整颗就是 84，那就反了。</em>
...
<p class="sub">前面七张图的结论，最后收在这一张上。</p>''')
    a(fig(8, "<b>图 G-8　等面积对照。</b>两个方块的<b>面积</b>按每周期乘加次数等比画，"
