...
one() {
  ...
  docker run --rm --cpus=$CPUS -v $HOME/aot-ablation/work:/w \
    ...
    python3 -m src.maxtext.trainers.pre_train.train_compile src/maxtext/configs/base.yml \
      model_name=hunyuan3-295b run_name=a base_output_directory=/tmp/o \
      base_num_decoder_layers=$LAYERS compile_topology=tpu7x-128 compile_topology_num_slices=1 \
