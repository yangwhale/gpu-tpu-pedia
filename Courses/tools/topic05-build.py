...
''' + sec("s三", "三", "第二刀：切权重") + '''
  ...
  <p>打个比方：一间制琴坊，卡是师傅，网络是跑腿的小工。小工搬两样东西：一样是<b>工具</b>（刨刀、琴模、雕刀），就是权重；一样是<b>琴</b>，一把把正在做、一道工序做完递给下一道的小提琴，就是激活，每个 token 一把（V3 里就是那串 7,168 个数）。
    FSDP 让小工搬工具：一副琴模送到，这个师傅手上的每把琴都拿它过一遍，琴越多，这趟越值。</p>
__FIG_INTENSITY__
...
__FIG_TP_ORDER__
  ...
  <p>TP 让小工递的是琴（激活）：几个师傅合做同一批琴，各做一半工序，做完得凑到一起合成。琴多一倍，要递的和要干的都多一倍，<b>batch 在账里约掉了</b>；隐藏维就是琴的尺寸，从小提琴换成大提琴，每递一趟要干的活越多。
    工具能提前送（算这一层时就把下一层的拿过来）；琴是刚做出来的，默认只能等这道工序做完再递，所以 TP 的通信很难藏进计算里。
...
__FIG_WIDE_DEEP__
  <p>段与段之间只在边界上用一对一收发传激活，是所有刀里通信最少的，所以它<b>能跨到慢线上</b>。
    可朴素地切，就是「4 张卡的显存、1 张卡的速度」：同一时刻只有一段在干活。于是把一批数据切成几份 micro-batch 轮着灌进去，像制琴坊的流水线：
    粗坯、雕面板、上漆三道工序各守一个工位，琴一批一批往下传。第一批刚进粗坯时，后两个工位只能干等；最后一批到了上漆，前两个工位又闲着。干等的那段，叫<b>气泡</b>：</p>
__FIG_PP_BUBBLE__
...
<figcaption>micro-batch 从 4 个加到 8 个，灰色的气泡跟着缩一半。
  ...
  <p>FSDP：它每搬一个字节换来的计算就等于每卡 token 数，token 少就藏不住。TP 每一层都要对账，而且琴要等这道工序做完才能递，藏不进计算，只能待在最快的那圈线里。</p></details>
</div></section>
