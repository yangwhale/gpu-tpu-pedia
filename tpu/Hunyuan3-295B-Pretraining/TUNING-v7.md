...
| 试过什么 | 结果 | 根因 |
|---|---|---|
| **FP8 路径的 tile**（18 个参数） | **±0** | 参数确实生效（日志验证过），但 step time 一字不差。见 [§5.4.2](#验证实验六补上-dsv3-的-18-个-tile--零收益一条核心假设被证伪) |
...
| 想要什么 | 为什么拿不到 | 出路 |
|---|---|---|
| **192 experts 上开 QAG** | 找不到既整除 192 又不 OOM 的 FSDP 宽度；**且加卡救不了**（分片厚度只由 FSDP 宽度决定） | **下一代模型专家数取 2 的幂** |
...
⇒ **对 192 experts 的 Hy3，QAG 在 v7 上没有可用的落地路径。**
不是「代价大」，是**没有既整除又不 OOM 的 FSDP 宽度**。

> 🎯 **这条把 §5.4.2 开头那张整除表从「设计建议」升级成「设计约束」：**
> **专家数取 2 的幂不是为了好看，是决定这个模型在目标规模上
> 能不能用 QAG 这条优化路径的分水岭。**
> 192 在 ≤32 芯片能开、看不出问题，一到 64 芯片就彻底关门 ——
> **而这个问题在小规模验证阶段完全暴露不出来。**
