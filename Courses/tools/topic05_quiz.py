...
def answers_html():
    ...
    return """<div class="note ok"><span class="t">开场第三问的答案：V3 为什么敢把两个动量压成半精度，主权重却不行</span>
  V3 把<b>两个动量</b>改用 bf16 存，报告说「没有观察到可见的性能退化」；主权重和用于累积的梯度仍留在 fp32
  （技术报告 arXiv 2412.19437 sec. 3.3.3）。这不是默认做法：常规做法里动量跟着主权重用 fp32，Megatron 要显式打开开关才用 bf16。<br>
  <b>主权重不能压</b>：它每一步只加进一个很小的更新，而且要一直累加下去；bf16 只有约三位有效数字，小更新会在四舍五入时整个被抹掉。<br>
  <b>动量能不能压，要看衰减系数 β₂</b>（本课推导）：二阶动量每一步只掺进 (1 − β₂) 那么多新值。
  V3 的 β₂ ＝ %.2f，新值每步占 %.0f%% 的份量，bf16 分辨得出来；如果用 PyTorch 默认的 %.3f，新值只占 %.1f%%，
  比 bf16 能分辨的最小相对差距（约 0.2%%）还小 —— 除非新来的梯度跟旧值差好几倍，否则这一步的改动会被四舍五入抹掉，二阶动量就卡住了。<br>
  <em>V3 报告里的验证是整套 FP8 方案（含 bf16 动量）对 BF16 基线：16B 和 230B 两个规模各训约一万亿 token，损失相对误差一直低于 0.25%%；
  没有单独把「动量用 bf16」拆出来做消融。</em></div>
""" % (NB.V3_BETA2, step_v3 * 100, NB.TORCH_BETA2, step_torch * 100)
