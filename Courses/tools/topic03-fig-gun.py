...
def main():
    ...
    # ⛔⛔ 2026-09-16 现场：「1048576 这个数上面有一条红线，是箭头还是什么东西
    #   糊在一起。」——&#160;是箭头，而且它**从那串数字中间穿过去了**。
    #   ⭐ 算一下就知道为什么：两个数都从 cx ＝ 60 / 390 起画（anchor 默认左对齐），
    #     38px 的「1,048,576」有九个字符，**自己就占到约 x=597**；
    #     而箭头画在 420→480，正落在它肚子里。
    #   ⛔ 写这行的时候大概按「两个数各占一小块」估的位置 ——&#160;
    #     **第一个数 5 个字符、第二个 9 个字符，宽度差了快一倍。**
    #   ⭐ 判据：**定位一个挨着文字的图元，要按那段文字的<u>实际宽度</u>算，
    #     不能按「它大概在那儿」摆** ——&#160;字数一变，就穿帮。
    #   ⚠️ 顺带记一条探针的盲区：版面体检查的是**文字撞文字**，
    #     这种**线穿过文字**它不报。所以图上有线的地方仍然要用眼睛看一遍。
    f.line(200, top + 88, 356, top + 88, RD, 2.2)
