...
飞书卡片的 Markdown 渲染器**不支持表格语法**（`| col1 | col2 |` 格式会原样显示为纯文本，不会渲染成表格）。当需要展示结构化数据（如性能指标、对比数据）时，应采用以下替代方案：
...
256   1,775.6        6.80     0.112
```
````

**方案 3：使用粗体 + 换行（适合少量对比数据）**
```markdown
**2P1D vs 1P1D (128 并发)**
TTFT: 1.73s vs 2.40s (**-28%**)
P99 TTFT: 6.23s vs 12.07s (**-48%**)
Output: 1,159 vs 1,119 tok/s (**+3.6%**)
```
