...
>
> **踩坑全记录**: https://cc.higcp.com/pages/glm51-inference-debug-20260423.html
>
...
> - MTP: `load_weights()` 硬编码跳过 layer 0-78 以外的权重（`end_ignore_layer_num=79`）
> - DSA: indexer 权重通过 `skip_substrs` 过滤，不加载不报错
> - RoPE: 新增 `InterleavedRotaryEmbedding`（theta=1M, even/odd 交错, 无 YaRN）
> - Config: `GlmMoeDsaConfig(DeepseekV3Config)` 在 `tpu_inference/__init__.py` 注册
>
> **已验证（2026-04-23, 1-layer + 5-layer 推理测试）**：
> - 模型加载 + MLA attention + MoE routing + FP4 cache 加载 + XLA 编译 全部通过
> - MLA kernel 对新维度（qk_nope=192, v=256）兼容，无需 padding
> - API 正常响应（/v1/completions），1-layer 和 5-layer 均能生成 token
> - 详细踩坑记录：https://cc.higcp.com/pages/glm51-inference-debug-20260423.html
>
> **待验证**：全量 77 层完整模型推理 + benchmark
...
8. ~~**1-layer 推理**~~：**已通过（2026-04-23）** — 模型加载 + MLA attention + 权重跳过逻辑验证
9. ~~**5-layer 推理**~~：**已通过（2026-04-23）** — Dense + MoE 混合层，FP4 cache 从 SHM 正确加载
10. ~~**MLA kernel 维度兼容**~~：**已确认** — qk_nope=192, v=256 兼容，无需 padding
11. ~~**InterleavedRoPE**~~：**已确认** — theta=1M, even/odd 交错模式正常工作

### 待验证

1. **全量推理**：77 层完整模型启动 + benchmark
2. **推理质量**：全量模型输出是否有语义正确性
...
25. **GlmMoeDsaConfig 注册时机**：`model_type: "glm_moe_dsa"` 不在 transformers 4.57 内置列表里。
    vLLM 的 `ModelConfig()` 在模型类 import 之前就调用 `AutoConfig.from_pretrained()`，
    所以 config 注册必须在 `tpu_inference/__init__.py` 中完成（不能放在 `glm_moe.py` 里）。
    **额外坑**：GKE pod 里 Docker 预装的 tpu_inference 和 `pip install -e .` 安装的版本会冲突，
    Python PathFinder 优先找到旧版本。用 `python3 -c "import xxx; print(xxx.__file__)"` 确认。

26. **Tokenizer class "TokenizersBackend" 不存在**：GLM-5.1 的 `tokenizer_config.json` 用了
    自定义 `tokenizer_class`。修改为 `PreTrainedTokenizerFast` 即可（需要目录里有 `tokenizer.json`）。
    同时 `extra_special_tokens` 是 list 格式，transformers 4.57 期望 dict，需要转换。

27. **end_ignore_layer_num 被 --hf-overrides 影响**：`--hf-overrides '{"num_hidden_layers":5}'` 会修改
    `hf_config.num_hidden_layers`，导致 `load_weights()` 中的跳过范围太小。
    **解法**：硬编码 `end_ignore_layer_num=79`（78 层 + 1 MTP），与 DeepSeek R1 的 `62` 策略一致。

> **踩坑详细记录**（含完整 traceback、根因分析、修复代码）：
> https://cc.higcp.com/pages/glm51-inference-debug-20260423.html
