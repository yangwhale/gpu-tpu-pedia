...
| DeepSeek V3 **scale-in (31L)** | **128 GPU (4×8)** EP32 | FP8_MX | **full_iteration** | 7680 | **~1550**（全规模 93.5%） | 22.2s | 通过 (30 步) |
| DeepSeek V3 scale-in (31L) | **128 GPU (2×16)** EP32 | FP8_MX | **full_iteration** | 7680 | ~1547（拓扑无关） | 22.2s | 通过 (30 步) |
| DeepSeek V3 scale-in (31L) | 128 GPU (2×16) **EP64** | FP8_MX | **full_iteration** | 7680 | ~1525（EP64 反而低） | 22.6s | 通过 (30 步) |
...
### 发现 2：EP=32 是甜点，EP=64（拉满整域）反而更差

2×16 每域 64 GPU，可以把 EP 从 32 拉到 64（用满整个 NVLink domain）。实测（128 GPU / 31 层 / GBS 7680）：

| EP | 每域 all-to-all 跨度 | 每 rank 专家数 | max reserved 显存 | 稳态 TFLOP/s |
|----|---------------------|--------------|------------------|-------------|
| **32** | 8 节点（半个域） | 8 | 更高 | **~1547** |
| 64 | 16 节点（整个域） | 4 | 113 GB（更省） | ~1525 |

> **结论：EP=32 更优，EP=64 反而低 ~1.3%（1547→1525）。** EP=64 虽然显存更省（专家分片更细），但 64 路 all-to-all 跨 16 节点的通信开销 > 32 路跨 8 节点，这个损失盖过了省显存的好处；且每 rank 只剩 4 专家，单卡计算效率也降。**除非显存吃紧，否则 EP 保持 32。**
