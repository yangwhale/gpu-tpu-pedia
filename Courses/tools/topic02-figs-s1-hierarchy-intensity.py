...
ROWS=[  # (层名, 副注, TPU 内容, GPU 内容, TPU空?, GPU空?)
 ...
 ("片上 · 显式管","谁放什么，由人或编译器决定","VMEM 64 MiB／核<TAB>SMEM 1 MiB<TAB>—— 编译器排布，没有自动行为",
 ...
 ("专用协处理器","矩阵单元干不了的活","SparseCore × 4／chip<TAB>可编程：前缀和 · 排序 · 计数 · scatter","（没有对应物）",0,1),
 ("芯片外","HBM","96 GB／chiplet<TAB>整 chip 7,380 GB/s","186 GB／GPU<TAB>8,000 GB/s",0,0),
]
...
p=[f'<svg viewBox="0 0 1000 {H}" width="100%" role="img" aria-label="TPU v7 与 B200 的存储层级对照，共用一根纵轴，空格子表示该层不存在">',
   ...
   f'<text class="svglbl" x="{LX}" y="{TOP-10}" fill="{BL}">TPU v7（每个 TensorCore）</text>',
   f'<text class="svglbl" x="{RX}" y="{TOP-10}" fill="{GR}">B200（每个 SM / 每 GPU）</text>',
   f'<line x1="{LX-14}" y1="{TOP}" x2="{LX-14}" y2="{TOP+RH*len(ROWS)}" stroke="#dadce0"/>']
def cell(x,w,y,txt,col,empty,note=""):
    dash=' stroke-dasharray="5 4"' if empty else ''
    fill='#fff' if empty else ('#f8f9fa')
    p.append(f'<rect x="{x}" y="{y}" width="{w}" height="{RH-14}" rx="7" fill="{fill}" stroke="{col}"{dash}/>')
    if empty:
...
for i,(lay,sub,tp,gp,te,ge) in enumerate(ROWS):
    ...
    cell(LX,LW,y,tp,BL,te,"GPU 那边整整一层，在这里根本不存在")
    cell(RX,RW,y,gp,GR,ge,"矩阵单元干不了的活，只能回到 CUDA core 手写")
...
p.append(f'<text class="svglbl" x="18" y="{yb+34}" fill="#7a5000">'
         f'⭐ 片上 SRAM 的总量几乎一样，归属完全相反：B200 的 L2 是 126 MB，v7 的 VMEM 是 64 MiB × 2 核 = 128 MiB</text>')
...
q.append(f'<rect x="0" y="260" width="1000" height="56" rx="8" fill="{BL}"/>')
q.append('<text class="svgnum" x="500" y="286" text-anchor="middle" fill="#fff" style="font-size:19px">'
         'BF16　312.6　对　312.5　　｜　　FP8　625.2　对　625.0　　—— 差千分之三</text>')
q.append('<text class="svgsm" x="500" y="306" text-anchor="middle" fill="#ffffffcc">'
         '这不是巧合能解释的。两边被同一批负载和同一代 HBM 技术钉在了同一个比值上</text>')
q.append(f'<text class="svglbl" x="0" y="342" fill="#202124">这个数到底是什么意思</text>')
for i,t in enumerate([
  "· 它是硬件的「胃口」：每从 HBM 搬一个字节，硬件配套能做约 312 次 BF16 运算。喂不满，算力就在空转",
  "· 于是每个算子只剩一个问题：它自己的算术强度，比 312 高还是低？高就是算力受限，低就是带宽受限",
  "· 专题一那八步里，① embedding 的算术强度是 0 —— 差了三个数量级。这就是 §2 要处理的东西",
  "· 而 ④ Dense MLP 的大矩阵乘能远高于 312 —— 那是 §3 的战场。同一颗芯片，两种完全不同的困境"]):
    q.append(f'<text class="svgsm" x="0" y="{362+i*17}" fill="{RD if i>=2 else "#202124"}">{t}</text>')
W('fig1-4.svg',q+['</svg>'])
