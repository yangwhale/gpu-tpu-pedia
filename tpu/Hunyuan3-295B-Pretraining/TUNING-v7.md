...
**正确的判法：逐对量 `-start` → `-done` 之间的窗口里核在干什么。**

TPU 上的集合通信是异步的，op 名字形如 `all-gather.282.cloned.1.call-start` /
`...call-done`。**注意后缀在最后一段**，按 `.` 切第一段会把它切没（本项目踩过）。
`-start` 到 `-done` 之间就是数据在飞的窗口，这段时间核在算别的东西 = 藏住了。

对 960 对通信逐个统计：

| | | |
|---|---|---|
| 传输窗口总长 | **13.911 s** | 所有异步通信在飞的时间之和 |
| **窗口内在算别的** | **11.607 s** | **83.4% —— 真的藏住了** |
| 窗口内还是通信 | 2.303 s | 16.6%，通信排队等通信 |
| 空闲 | 0.001 s | ~0 |
| `-done` 自身耗时 | **1.295 s** | **没藏完的裸露等待** |

窗口里跑的是 `while`（8.463 s，MoE 层体）、`gmm` / `tgmm`（0.983 / 0.868 s，分组矩阵乘）、
`fusion`（0.671 s）—— 实打实的计算。

**结论：通信已经被藏掉八成以上，真正裸露的只有 1.295 s ≈ 墙钟 4%。**

> **教训（这一节最值钱的部分）**：同一份 trace，我先后得出过四个互相矛盾的结论 ——
>
> | 版本 | 结论 | 错因 |
> |---|---|---|
> | ① | 重叠 0.000 s ⇒ 完全裸露 | 在单条顺序 lane 上算时间交集，**恒等于 0** |
> | ② | 80% 是同步阻塞 | 按 `.` 切 op 名，把 `-start`/`-done` 后缀切没了 |
> | ③ | 100% 异步，1.766 s 全是残余 | 方向对了，但没量窗口，把"异步"当成"没藏住" |
> | ④ | **83.4% 藏住，裸露 1.295 s** | 逐对量窗口内实际占用 |
>
> 三条通用教训：**(a)** 在单条顺序 lane 上算并发是同义反复，先确认这条 lane 到底能不能重叠；
> **(b)** op 名字的后缀在最后一段，别用 `split('.')[0]`；
> **(c)** "是异步的"不等于"藏住了"，也不等于"没藏住"——必须量窗口。
...
| **H2（时间花在非 MXU 环节）** | ⚠️ **部分成立，但不是原来以为的样子**。通信 op 确实占墙钟 57.2%，但 **83.4% 已被计算掩盖**，真正裸露的只有 1.295 s ≈ 墙钟 4% |
| 实验方向 | ⚠️ **已修正**：既然通信只裸露 4%，B 组（继续藏通信）的天花板很低。窗口里那 11.6 s 计算本身的效率才是大头 —— **A 组（提算术强度 / FP8）重新回到台面** |
| 对 FP8 的预期 | 需重估。原先认为"FP8 动不了 57.2% 的通信"，但既然通信八成已被藏进计算窗口，**加速计算等于同时缩短窗口**，FP8 的收益上限可能比 [§3.1](#31-关于-fp8先别指望) 记的 +21.4% 更高 |
