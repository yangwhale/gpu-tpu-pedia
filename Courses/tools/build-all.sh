...
if [ "${1:-}" != "--lint" ]; then
  ...
  #    真实情况（见 topic03-build-L200.py 文件头）：
  #      · **HTML 就是源**，正文直接写在 topic03-build-L200.py ／ -L300.py 里
  ...
  # ⛔ 图必须先生成 —— topic03-build-L200.py ／ -L300.py 会 assert 找不到 fig3-chronicle.svg。
...
#   ⭐ 判据：**一句话改不顺，先问它非有不可吗。** ——&nbsp;当时我只想着换一句，
#     而「整类都不要」本来就是一个选项，我没把它摆上台面。
#   这条守着它别悄悄回来（下一个加新专题的人很可能照着老页面抄一行）。
step "封面不要英文副标题（.en 已全课清除）"
if grep -l 'class="en"' "$W"/*.html 2>/dev/null; then
  echo "    ⛔⛔ 上面这些页面里又出现了 .en 英文副标题 —— 2026-09-23 已决定全课不要"
  exit 1
fi
echo "   ✅ 没有页面带英文副标题"
...
# ── 动画首尾一致性（1.4 s，只读 mp4，不重渲）──────────────────────────
# ⭐ 2026-09-18 加的。页面里这些 mp4 都挂 loop，首末帧对不上就每轮跳一下。
#   三支已上线的片子当时**全都在跳**，而没人量过 —— 因为「构建全绿」
