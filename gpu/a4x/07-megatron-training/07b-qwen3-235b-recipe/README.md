...
| 轮次 | recipe | graph | paged_stash | 域 | TFLOPs | step time | 提升 |
|---|---|---|---|---|---|---|---|
| 之前所有测试 | V1 (`-cv v1`) | TE scoped (attn) | False | 跨域 | **685** | 7.1s | baseline |
| **R9** | **V2 (`-cv v2`)** | **full_iteration** | **True** | 跨域 | **1124** | **4.31s** | **+64%** |

### gpu-launchpad-playground 单域复现 (2026-07-08)

在 GKE 集群 `chrisya-a4x-gke-v2`（gpu-launchpad-playground, us-east1-d），16 节点单域 NVL72（64 GPU）复现测试。

| 轮次 | recipe | PP/EP | Graph | MNNVL | TFLOPs | step time | 备注 |
|---|---|---|---|---|---|---|---|
| 单域 V1 | V1 | PP=8 EP=8 | TE scoped | 0 | **930** (peak 933) | 6.47s | 比跨域 685 高 36% |
| 单域 V2 | V2 | PP=8 EP=8 | full_iteration | 0 | **931** (peak 932) | 6.47s | V2 graph 在 PP=8 无提升 |
| 单域 V2 | V2 | PP=2 EP=32 | full_iteration | 0 | 测试中 | — | 对标 baker 跨域 1124 |
| 单域 V2 | V2 | PP=2 EP=32 | full_iteration | 2 | 待测 | — | MNNVL 全速 |

**V1 vs V2 在 PP=8 EP=8 下无差异** (930 vs 931)：PP=8 的 pipeline bubble 是主要瓶颈，graph 优化的 kernel launch overhead 占比极小。1124 的真正来源是 PP=2 EP=32 的并行度切换——PP=2 bubble 小（50% → 6%）、EP=32 MoE 通信效率更高。full_iteration graph 在 PP=2 下才能充分发挥（少 PP stage 意味着 graph 覆盖更大的计算比例）。

**V1 单域 930 vs 跨域 685 差距分析**：
