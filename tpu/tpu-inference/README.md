...
本指南并非纸上谈兵——其能力矩阵、部署配方和前期准备步骤，已在一次面向**首次采用 TPU v7x 的大型企业客户**的真实**现场（onsite）上手支持**中得到验证。

在现场支持过程中，客户被顺利引导走完了**首次 TPU v7x 的完整使用流程**——从基础设施开通、权重准备，到成功拉起大模型推理。这次首跑成功，为客户后续在 TPU 上的迭代与生产落地奠定了坚实基础。

本指南的结构正反映了那次上手之所以高效的关键：

- **能力与特性矩阵**（见下文）——让首次使用者一眼看清哪些模型、架构与部署模式已在 TPU v7x 上验证可用，从第一天起就对齐预期。
- **前期准备**（权重存储布局、`/dev/shm` 策略、环境变量、cache 生成）——把首次 TPU 用户最常卡住的步骤前置，把多天的试错变成一条有引导的路径。
- **每个模型的端到端可执行步骤**——每个模型目录都提供从零到推理成功的完整走查，正是现场工程师快速带客户跑出第一次成功所需。

对于计划类似上手的团队，这套矩阵 + 准备清单就是推荐的起点。
