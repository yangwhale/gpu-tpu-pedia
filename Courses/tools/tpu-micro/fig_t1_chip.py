...
def _die(f, dx, idx):
    ...
    # ⭐ 2026-09-08 补「不是指令内存」这半句。现场原话：
    #   「TPU 里的这个 SMEM 就是 IMEM 吗？」——&nbsp;这是个每个人都会撞上的歧义：
    #   S 容易被当成 instruction 的缩写，而 1 MiB 又正好像个指令内存的量级。
    #   ⛔ 两者是两块：SMEM 是**标量单元的数据**暂存（标量、循环边界、DMA 描述符，
    #     以及 Splash 那张 block mask 表 ——&nbsp;见 2-6／2-7 那张图现算它占多少），
    #     IMEM 是**指令内存**，另一块，公开资料没给容量，本课不写。
    #   ⭐ 判据：**一个缩写会被读成另一个词的时候，图上就得钉死它是哪个。**
    f.t(tx + 254, ty + 180, "标量／描述符（≠ 指令内存 IMEM）" if full else "同左", "xxs")
    ...
    if full:
        ...
        f.t(hx + 10, hy + 54,
            # ⭐ 2026-09-05 解闸。原来公开版把「按 die 拆的口径」整句抹成
            #    「官方未公开」，而同一份产物的课程正文（§3.1）现在给了 v7 的
            #    D2D 数并标注出处 ——&nbsp;**同一门课对「什么算可讲」不能有两套标准**。
            #    ⛔ 解的只是这一类（按 die/core 拆的架构参数）。
            #       内部代号 / 内部通道清单 / 内部 ISA / VMEM 带宽 / 取行重复度公式
            #       仍然闸着，那几条是另外的规矩。
            "整封装 8 个 HBM3E 堆栈、7.37 TB/s；按 die 拆约 3,433 GiB/s"
            "（此拆分口径未定位到官方公开出处）", "xxs")
    else:
...
def _sidebar(f):
    ...
    para(f, RX + 14, y3 + 44, RW - 28,
         ...
         # ⭐ 2026-09-05 同上解闸。⚠️ 但**去掉了「（内部设备表）」这个出处署名** ——
         #    数可以给，「这个数是从哪份内部表上抄的」不必也不该写进公开产物。
         #    改成本课统一的说法：给数 ＋ 标明未定位到官方公开出处。
         + "主机接口 PCIe 侧约 119.2 GiB/s（未定位到官方公开出处）。"
