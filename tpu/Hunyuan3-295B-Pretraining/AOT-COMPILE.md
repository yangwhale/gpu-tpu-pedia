...
## 生产规模验证：64 芯片 / 80 层 / bodaborg

> 2026-08-15 实测。前面 16 芯片那次是缩比版；这次是**真的生产配置**：
> 80 层、64 芯片（`tpu7x-128` = 4x4x4）、`pdbs=12`，在共享集群 bodaborg 上跑。
> 带缓存与不带缓存各跑一遍 10 步，除了 `compiled_trainstep_file` 之外**逐字相同**。

### 产物

| | |
|---|---|
| AOT 机器 | cc-tw，docker 限 32 核 |
| 编译墙钟 | **131.2 s**（其中约 24 s 是序列化 —— 不存产物时同配置是 107 s） |
| 产物大小 | **389 MB**（20 层 / 16 芯片那次是 360 MB） |
| `temp_size` | 80.48 GB，离 94.74 GB 上限还有 14 GB |
...
- **启动省 51.4 s（占启动的 23%）**，加载 389 MB 只花 2.15 s
- **稳态性能零差异**，**loss 逐位相同** —— 再次证明产物与现场编译等价
- 换算：262.7 × 2 = **525.4 TFLOP/s/chip，MFU 22.8%**
  （比 445 的旧基线高是因为 `pdbs=12`；没带 tokamax tile，所以也到不了 630）

### 最重要的一条：省下的不是 10 分钟，是 46 秒

**「v7 编译要 10-17 分钟」这个说法是错的**，这次的日志把时间花在哪写得很清楚：

| 阶段 | 耗时 | AOT 能不能省 |
|---|---:|---|
| TPU init / 建 ICI 切片 | **69.6 s** | ❌ 省不掉 |
| `jit_train_step` 编译 | **45.8 s** | ✅ 省掉 |
| ├ `HLO_PASSES` | 15.2 s | |
| ├ `BACKEND_PASSES` | 23.0 s | |
| └ `CODE_GENERATION` | 6.4 s | |
| 其余小 jit + 数据管线 + step 0 本身 | ~110 s | ❌ 省不掉 |

[TUNING-v7](TUNING-v7.md) 早就推翻过这个说法（那次量到 43.5 s vs 44.3 s，
并指出**真正随规模涨的是建切片不是编译**）。这次在完全不同的路径上
量到 45.8 s 编译 + 69.6 s 建切片，**独立复现了同一个结论**。

> **所以别拿「省十几分钟编译」当 AOT 的卖点。** 它真正的价值是
> **抢卡之前就知道会不会 OOM**，省的是「排队 → 拿到 64 张卡 → 跑挂 → 重排队」
> 这个小时级的循环，不是那 46 秒。

### AOT 预测的显存与真机逐位吻合

同一份配置，AOT 在 CPU 上算出来的 vs 真机 64 芯片跑出来的：

| | AOT 预测 | 真机实测 |
|---|---:|---:|
| `temp_size` | 74.95 GiB | **75.0 GB** |
| `argument_size` | 21.75 GiB | **21.7 GB** |
| host temp | 29.62 GiB | **29.6 GB** |

**一个字节都不差。** 这是 AOT 体检可信的直接证据 ——
它给的不是估算，就是编译器在真机上会得到的同一份账。

> [!warning] 带缓存就拿不到这份账
> 加载预编译产物的那一路**不会打印 `Memory analysis`**（因为压根没编译）。
> 想要显存分解，只能靠 AOT 那一步。**这反而是又一条该跑 AOT 的理由。**

### 这一段踩的两个雷

**7. `exclusive-topology` 的反亲和会把下一个 JobSet 挡在门外。**
第一轮跑完后立刻提第二轮，16 个 pod 只出来 1 个、一直 `Pending`，
`describe` 说 `16 Insufficient google.com/tpu` + `16 node(s) didn't match pod anti-affinity rules`。
原因是上一轮那些**已经 `Completed` 的 pod 还挂在节点上**，
`alpha.jobset.sigs.k8s.io/exclusive-topology` 的反亲和不让两个 JobSet 共用一个节点池。
**跑完一轮要显式 `kubectl delete jobset`，不能只等它自己 `Completed`。**
另外这种情况下 JobSet 只会创建 leader 一个 pod（`active: 1`），
**看 pod 数会误判成"只申请了 1 台"**，要看 `describe` 里的调度事件。

**8. 写了不存在的 `priorityClassName` 不报错，静默不 admit。**
集群里 `very-high` / `high` 已经没了，现存最高可用是 `medium`(500)。
写了不存在的档，JobSet 看着在正常排队，但 Kueue 根本不给建 Workload。

## 拿 AOT 当搜索器：18 个探针换一条真机结论

既然 AOT 的显存预测跟真机逐位一致，它就可以拿来**在 CPU 上免费扫配置空间**。
2026-08-15 跑了 18 个探针（每个约 2 分钟，全在一台机器上 3 并发），
只问一个问题：**装不装得下**。原始数据在
[`maxtext-hunyuan3/data/aot-64chip-config-probe-20260815.csv`](maxtext-hunyuan3/data/aot-64chip-config-probe-20260815.csv)。
...
| 14 | 1792 | — | ❌ 差 1.79 GB（`Used 96.53G of 94.74`） |
| 15 / 16 | — | — | ❌ 差得更远（16 要 102.59 GB） |

> **`pdbs=13` 比 `pdbs=12` 占用更少，这条反直觉，但复现了两次、数字一字不差**
> （78,503,516,064 字节）。机制没查明 —— 多半是编译器在这个形状上选了
> 另一套调度/切分。**记在这里是因为它可执行，不是因为它被解释清楚了。**

p14 只差 1.79 GB，试了三种省法都补不上：
`out_proj=offload` 省 1.06 GB（还差 0.73）、再叠 `mlpwi=offload`、
把 splash block 从 2048 砍到 1024 —— **全部仍然 OOM**。

### 真机验证：+0.95%

拿 `pdbs=13` 上 64 芯片真跑一遍（其余配置与生产逐字相同）：

| | `pdbs=12` | `pdbs=13` |
|---|---:|---:|
| 稳态 step | 25.99 s | 27.88 s |
| TFLOP/s/device | 262.69 | **265.18** |
| per-chip | 525.4 | **530.4** |
| MFU | 22.77% | **22.99%** |
| AOT 预测 `temp` | 74.95 GiB | 73.11 GiB |
| 真机打印 `temp` | 75.0 | **73.1** ✅ |

**+0.95%。** 老实说这个收益很小 —— 但**找到它只花了 2 分钟 CPU 时间**，
而且顺带得到一条更硬的结论：**`pdbs` 到 13 就到顶了，14 以上不用再试。**
以前要得到这条得占着 64 张卡试三轮。
...
- **`remat_policy=full` 与 `decoder_layer_input=device` 的账完全相同**
  （都是 115.08 GB）—— 说明这个模型下 `full` 的实际效果就是把
  decoder layer input 放回 device，其余旋钮没起作用。
- **`qkv_proj=device` 和 `mlpwi=device` 对显存的影响是 0**（仍是 80.48 GB，
  与基线一字不差）。这两个旋钮在本模型的 custom remat 路径上**根本没被读到**。
  改了没反应 ≠ 改对了 —— **这正是 AOT 能替你发现的那类事。**

> **别把这一节当调优结论。** AOT 只回答「装不装得下」，
> 不回答「跑多快」。上面 18 个探针里唯一的性能数字来自那次真机运行。
> 正确用法是：**AOT 先把不可行的砍掉，把真机额度留给活下来的少数几个。**

## 顺藤摸出来的一条：tile 该用配置参数，不是那个 monkeypatch

扫配置时想问「`pdbs=13` 跟生产那套跑到 630 的 tokamax tile 叠起来还装不装得下」，
于是把 [QUICKSTART-v7 §3.4](QUICKSTART-v7.md) 的 `tkcfg.py` 补丁挂进 AOT ——
**结果发现它根本没被调用过。**
...
`tkcfg.py` 打的是 `tokamax` 的 `PallasMosaicTpuRaggedDot._get_heuristics_config`，
而 `moe.py:1500` 只有 **`use_tokamax_gmm=True`** 才走那条路 ——
**这个开关在 v7 上会死锁，生产一直是关着的**（见 TUNING-v7 §6.7）。

生产实际走的是 `jax.lax.ragged_dot`，它的 tile 由
...
正是 `run.sh` 的 **v5p 分支在传、v7 分支没传**的那一组。
...
| 参考：文档旧最优（monkeypatch 路径） | 21.67 s | — | 630 | 27.31% | 91.94 G |

- **改用配置参数比旧最优高 5.1%**（662.2 vs 630），
  多出来的部分应该来自 **`dlhs` / `drhs` 两条反向路径也被 tile 了** ——
  monkeypatch 那条路只影响前向的启发式
- **`pdbs` 12 → 13 再加 0.66%**，合计 **666.6 / MFU 28.89%，比旧最优高 5.8%**
- **`pdbs=12` 带不带 tile 的 loss 逐位相同**（12.751 → 12.720 → 12.701），
  tile 只改性能不改数值
...
> **性能数字全部来自真机，AOT 一个都给不了。**

> [!warning] AOT 看不见运行期打的 monkeypatch
> 这也是 AOT 的一条硬限制：它体检的是**配置文件描述的那个程序**。
> 任何在 pod 里现场注入的补丁，AOT 都看不到 ——
> 上面的 `pdbs=13 装得下` 严格来说只对「配置参数能表达的那套」成立。
> 好在 tile 恰好也是配置参数，所以这次结论仍然有效。
...
| 想省下大 slice 的编译时间 | **收益比想象的小**。64 芯片上编译只占启动的 46 s / 227 s |
