...
下面有一部分数字来自内部资料（片上带宽、内部代号、内部实测），
<b>不要外发、不要进公开课件</b>。公开版由同一份稿子构建，
构建时闸门会把这些条目删掉或换成「官方未公开」，并跑一遍禁字自检。</div>'''}
</section>''')
...
不需要任何内部信息就能推出上限是 50%。
''' + gate.IP("<br><br><b>内部实测佐证：</b>把 Qwen3-30B 的注意力从 32 头 × 128 "
              "改成 16 头 × 256（参数量和 FLOP 完全不变），MFU 在 8K / 16K / 32K 序列长度上"
              "分别提升 <b>21% / 32% / 46%</b>。<br><br>",
              "<br><br>这一条本文只讲几何上限；公开资料里没有可引用的实测对照，"
              "所以不给具体的提升幅度。<br><br>",
              why="内部实测数据") + '''
...
''' + gate.IP("Hunyuan3 那个尺寸下（M＝7×4096，K＝120,832，N＝4096），这条 matmul 是 "
              ...
              "one-hot 矩阵乘的<b>绝对耗时比一次 gather 高出一个量级</b>，"
              "只是相对于整步的分母小到可以忽略。所以它<b>不是</b>一个「省时间」的选择 ——"
              "常见的那句「反正 MXU 闲着，用算力换带宽很划算」在算术上站不住。",
              why="内部实测耗时") + '''
<br><br>真实动机我<b>没查实</b>，最强的候选是反向传播 —— scatter-add 在分片下不好做，
而 matmul 的反向天然还是 matmul。但这只是候选，<b>不写成结论</b>。</div>

<p>至于 SparseCore 为什么没被派去干这件事：判据是 <b>duplication factor</b>
＝ 这一批取了多少个索引 ÷ 可选的行数，它衡量的是<b>「重复取同一行」的程度</b>。
推荐系统那边是几亿行词表里取几百个，重复率极高，那是 SparseCore 的主场；
而语言模型的一个 batch 里几乎每个位置取的都是不同的词，
''' + gate.IP("我们这次的语言模型是 131,072 ÷ 129,280 ≈ <b>1.01</b> —— "
              "几乎每个索引都取不同的行，落在判据<b>最不划算的那一端</b>，"
              "编译器于是根本没把它派过去。",
              "重复率接近 1，落在判据<b>最不划算的那一端</b>，编译器于是根本没把它派过去。") + '''</p>
...
<li><b>编译很慢，而且慢得有道理。</b>''' + gate.IP(
        "v7 上一次 XLA 编译常见 <b>10–17 分钟</b>。",
        "一个大模型的 XLA 编译动辄几分钟到十几分钟。",
        why="内部集群实测耗时") + '''
...
<p>三级的划分标准写在 §0。这里只补一句：<b>「第三方」不等于「不可信」，
...
<tr><td>Qwen3-30B 的 MFU 21% / 32% / 46%</td><td>未公开实测</td></tr>
<tr><td>Hunyuan3 的 28.38 TFLOP / 0.62% / 24.6 ms / duplication 1.01</td><td>未公开实测</td></tr>
<tr><td>XLA 编译 10–17 分钟</td><td>内部集群实测</td></tr>
