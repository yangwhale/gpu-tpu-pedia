...
### 6.4 闭环案例：一张图提前判了一个实验的死刑

这是本轮**最值得学的一段**，因为它是完整的「预测 → 实验 → 证实」，不是事后解释。

**第一步｜从图上提出假设（还没跑实验）**

看完 [§6.3](#63-r0--基线的图长什么样) 那张 30 ms 窗口，我写下的判断是：

> v5p 的瓶颈**不在通信裸露**，v7 那条「藏通信」的主线在这里优先级要降。

依据只有一条：窗口里几乎全是连续的 `tgmm`，**没有成片的 `-done` 等待块**。
而 v7 同样缩放下，`gmm` 和 `all-gather...call-done` 是交替出现的。
...
### 6.6 抓 trace 的实测耗时（按规模，别照抄小规模经验）
