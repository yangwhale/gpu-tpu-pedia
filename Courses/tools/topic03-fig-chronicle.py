...
for i, (tm, mdl, cyc, ctx, kvspec, note) in enumerate(ROWS):
    ...
    if g is None:
        ...
        lab = kv_fmt(g)
        # ⭐⭐ 2026-09-07 现场：「MHA 的 KV cache 太长了，影响后面的按比例对比，
        #   要不做成两折叠吧，上下两条那种。」
        # ⭐ 折行不只是"把长的塞下"，它顺带**把刻度放大了一倍** ——
        #   满刻度从「一行 400px」改成「两行 800px」，于是 GPT-3 正好占满两行，
...
box(16, LZ, W - 32, 196, "#e8f0fe", BL, 6)
t(30, LZ + 20, '⭐ 这张格子图一眼能看出七件事', "svglbl", "#174ea6", size=12)
...
# ⭐⭐⭐ 2026-09-07 现场自己推出来的一条，而且是整张表里最反直觉的：
#    「DeepSeek v2 虽然模型很小，但它的 KV cache 算出来跟 v3 差不多大。
#      是不是因为它们 MLA 的 lora rank 都一样，然后层数还是一个 60 一个 61？」
#    ——&nbsp;完全正确。这条值得单独占一格，因为它推翻了一个几乎人人都有的默认假设。
_v2 = kv_gib(("mla", 60, 576))
_v3 = kv_gib(("mla", 61, 576))
t(30, LZ + 166, '⑦ ⭐⭐ <tspan font-weight="700">最反直觉的一条：MLA 之后，'
                'KV cache 跟「模型多大」<tspan style="text-decoration:underline">脱钩</tspan>了。</tspan>'
                'DeepSeek-V2 是 236B、V3 是 671B，<tspan font-weight="700">参数差 2.8 倍，'
                'KV 却只差 %.1f%%</tspan>（%s vs %s）。'
                % ((_v3 / _v2 - 1) * 100, kv_fmt(_v2), kv_fmt(_v3)), fill="#174ea6")
t(46, LZ + 184, '因为 MLA 的 KV 只跟<tspan font-weight="700">「层数 × (kv_lora_rank ＋ rope 维)」</tspan>走 ——&#160;'
                '<tspan font-weight="700">跟专家多少、hidden 多宽、总参多大，一点关系都没有</tspan>。'
                'MHA 时代 KV 是跟着模型一起长的（看 GPT-3 那行），'
                '<tspan font-weight="700">这条链在 MLA 这里被剪断了。</tspan>', fill="#174ea6")
