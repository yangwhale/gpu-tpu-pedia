...
_FORBIDDEN = [
    ...
    # ⛔⛔ 2026-09-06 补。触发事件：现场读到教材里一个折叠标题写着
    #    「……顺便审出本课自己一个洞（<真人姓名> 现场追问）」——
    #    **一个具体人名，出现在一份要公开、要录像、要出版的教材正文里。**
    #    往回一扫，源码注释里还有一百四十多处「<姓名>：『……』」的私下对话引语，
    #    而 HTML 注释是随文件一起进公开仓库的。
    # ⭐ 判据：**这门课是公开材料。谁问的不重要，问出了什么才重要。**
    #    所以内容一个字不删，只把归属换成中性的场景词（现场追问／试讲时／原话是）。
    # ⚠️ 例外只有一个：作者署名合法，用零宽断言放行。
    #    别把这条改成裸的名字 —— 那样每一页的署名都会被自己的闸门拦下。
    # ⛔ 别把下面这个名字换成别的写法 —— 2026-09-06 全仓扫人名时，
    #    脚本把**这条规则自己**也扫了一遍，规则当场失效。
    #    这就是本文件上面 SELF 白名单存在的理由，批量替换时也要照办。
    (r"Chris(?! Yang)",            "正文里出现真人姓名（作者署名除外）"),
    ...
    # ⛔⛔ 2026-09-05 补的第一条**「不许公开的说法」**，前面全是「不许公开的词」。
    #
    # 触发事件：SparseCore 那条判据的具体算式，讲义里明确写过不进对外材料，
    # 而它照样渲染进了公开产物并推了一版。事后翻代码才发现最讽刺的一点 ——
    # 课件里**早就有一条注释写着这个规矩**，还顺带写了「T-5 那张图在公开模式下
    # 也是这么处理的」。可 T-5 根本没这么处理：那句是**假设**，从没验证过。
    #
    # ⭐ 两条教训，两条都值钱：
    #   ① **写在注释里的规矩不是执行。** 规矩要么变成代码里的判据，要么它就只是
    #      一句愿望。这条闸门就是把那句注释变成判据。
    #   ② **「另一个文件已经处理好了」是一个断言，不是一个事实。** 当时只要
    #      grep 一次 T-5 就能戳破。跨文件的信任必须当场验，不能顺手写进注释。
    #
    # 判据只盯**算式的形状**，不盯那个术语本身 —— 术语在公开文献里有，
    # 白话释义也允许写；被挡住的只有「÷ 可选的行数」这个具体算法。
    (r"÷\s*可选的行数|除以可选的行数",  "未获准公开的判据算式"),
]
