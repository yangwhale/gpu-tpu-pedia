...
> 含全部环境坑与缓存持久化做法。本文件下面那个 "Quick Start" 章节写于实测之前、
> 照 config 反推、**未经验证**，保留仅作对照，**不要照它操作**。

## 📌 文档状态

| | |
|---|---|
| 版本 | **v1 — 已端到端跑通并与上游基线对标**（2026-08-13） |
| 已验证 | `tp8-dp1` 的 `isl1024_osl8192_c64` 这一个 cell，640/640 成功 |
| 未验证 | 该 config 其余 5 个 cell；`tp1-dp8` / `tp2-dp4` 两种布局 |

### ✅ 实测结果（本目录自跑，非引用）

`TP=8 DP=1 EP / ISL 1024 / OSL 8192 / 并发 64 / 640 请求`

| 指标 | 上游 baseline | 本目录实测 | 偏差 |
|---|---|---|---|
...
环境：`torch 2.13.0 / jax 0.10.2 / libtpu 0.0.44.1 / torch-tpu 0.1.1.dev20260804130134 /
vllm 0.26.1rc0+tpu`，v7x 4 芯片（8 device），编译缓存命中。

> 单次测量，未验证重复性，也不知上游 baseline 的版本组合 —— **是线索不是结论**。
...
| 单个 cell | 26 min 42 s | 26 min 42 s |
| **合计** | **超出 55 min 窗口** ❌ | **约 45 min** ✅ |

**结论：397B 必须先做缓存持久化**（[QUICKSTART 3.5 节](../QUICKSTART.md#35-跑大模型必读把权重和编译缓存放持久盘)）。
跑完 `tp8-dp1` 全部 6 个 cell 需要 6 个窗口，或申请独占资源。
