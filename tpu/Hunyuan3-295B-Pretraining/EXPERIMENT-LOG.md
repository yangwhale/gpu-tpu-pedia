...
| 稳态 step | 63.17 s | **21.67 s**（BF16 最优） | — |
| **TFLOP/s / 计算单元** | **161.0** | **629.9** BF16 / **674.0** FP8 | 854.0 |
| **MFU** | **35.07%** | **27.31%** | 34.2% |
| 整机 tok/s | 265,588 | 290,331 | 399,488 |
| 调优状态 | ✅ 已收敛 | ✅ **BF16 调参见底**，已达目标区间上沿 | 已调优 |
...
- **v5p 的 35.07% 略高于 GB300 的 34.2%**（原记 31.6%，分母更正见 §7.2），而单卡算力只有它的 1/5.4。
- **v7 首跑 17.54%，一路调到 27.31%**（445 → 630），已超过 DeepSeek V3 在同一硬件上的
...
145.96 / 459 = **31.8% MFU**，跟 GB300 上 34.2% 处在同一量级。
...
| 杂项组 | 5 个（dvfs / bf16 emission / opt barrier 等） | ✅ **dvfs 已测：`=7` 值 +8.6%**（2026-08-11，见 §DVFS）；其余 4 个 ⬜ |
...
| BF16 TFLOPS / chip | 459 | 459 | **2,307** | 2,500 |
| FP8 TFLOPS / chip | 459（无加速） | 459 | **4,614** | 5,000 |
| 总 BF16 算力 | **117.5 PFLOPS** | 58.8 PFLOPS | 147.6 PFLOPS | 160.0 PFLOPS |
...
|---|---|---|---|---|---|---|
| A1 冠军 | BF16 | 1 | 64 | **854.0** | **34.2%** | 6,242 |
| C1 | FP8_MX | 1 | 64 | 1,285.9 | 25.7% | 9,396 |
| **C2 最快** | **FP8_MX** | **2** | **64** | **1,360.4** | **27.2%** | **9,945** |

MFU 分母：GB300 **dense BF16 2,500 / FP8 5,000 TFLOP/s per GPU**（官方 GB300 NVL72 规格表 360 / 720 PFLOPS 均为 sparse，dense 减半 ÷ 72 GPU）。
> ⚠️ **2026-08-31 更正**：此前写的 2,700 / 5,400 是把 GB200 的数字乘 1.2 推出来的，**官方没有这两个数** —— GB300 的 BF16/FP8 峰值与 GB200 NVL72 完全相同，Blackwell Ultra 提升的是 dense FP4（1.5×）与 attention（2×）。
...
**1. v5p 的 36.7% 略高于 GB300 的 34.2%。** 单卡算力差 5.4 倍，
...
## DVFS 频率档位（2026-08-11，64 芯片 / 16 节点 / 共享 v7 集群）

**结论：`--xla_tpu_dvfs_p_state=7` 值 +8.6%，零代价。580 → 630 TFLOP/s/chip。**

同一个常驻 `sleep infinity` JobSet（`hy3-dvfs`，4x4x4）、同一份代码包、
同一批 16 个节点，三轮之间只改这一个 flag，其余配方全部照 [QUICKSTART-v7 §0](QUICKSTART-v7.md#0-最优配方速查)
的 64 芯片版（`ici_fsdp_parallelism=-1`、pdbs 12、tokamax tile 512/2048/1536）。

| 轮次 | flag | step（step 3–7 均值） | TFLOP/s/device | **TFLOP/s/chip** | **MFU** | 峰值 HBM |
|---|---|---|---|---|---|---|
| R1 | 不带 | 23.537 s | 289.93 | 579.9 | 25.14% | 91.94 G |
| **R2** | **`--xla_tpu_dvfs_p_state=7`** | **21.670 s** | **314.97** | **629.9** | **27.31%** | 91.94 G |
| R3 | `--xla_tpu_dvfs_p_state=3` | 23.539 s | 289.90 | 579.9 | 25.14% | 91.94 G |
| R4 | `--xla_tpu_dvfs_p_state=5` | 22.438 s | 304.19 | 608.4 | 26.37% | 91.94 G |

每轮 5 个稳态 step 的方差都在 ±0.02 s 以内，8.6% 远超 ±3% 噪声带。

**三条可复用的结论：**

1. **默认档就是 3。** R3 与 R1 逐位一致 —— `run.sh` v5p 分支 line 33 那句
   `--xla_tpu_dvfs_p_state=3` 是在显式写默认值，等于没写。真正值钱的只有 **7**。
2. **v7 分支从来没带过这个 flag。** 它只出现在 v5p 分支，以及 `tpu/v7_perf/` 下的
   Qwen3 推理实验（那边一直在用 `=7`）。训练侧漏了整整一条战线。
3. **推翻了「580 是 64 芯片天花板」。** 那个结论只在给定频率档下成立；
   64 芯片现在 **630，反超 256 芯片的极限值 599**。

**档位单调递增，没有拐点**（R3 → R4 → R2）：

```
p_state=3（默认） 580  ──┐
p_state=5         608    ├─ 每档约 +2.4~2.5%，线性
p_state=7         630  ──┘
p_state=9         非法 —— `INVALID_ARGUMENT: P-state is out of range [0, 7]`
```

**7 就是顶格**，实测撞过边界，不用再往上试。

**FP8 + QAG 叠加 `dvfs=7` 已实测（当晚 21:58 补跑成功）：674.0 TFLOP/s/chip。**
外推值曾写 679，实测 674.0，**误差 −0.7%**。详见下一节。

**未测：** 256 芯片上是否等幅；
长时间高频运行是否触发降频（本实验每轮只有 8 步 / 约 3 分钟，未观察到衰减 —— 5 个稳态 step 的
step 时间方差 ±0.02 s，看不出热节流）。

---

## 128 芯片 `FSDP=256 / DP=1` —— 计划与配额硬约束（2026-08-11）

**目标**：`DP=1 × FSDP=256` + pdbs 16 + `dvfs_p_state=7`，看 BF16 MFU 能否上 30%。

**外推值先摆出来，免得期望错位：**

```
256 芯片 DP2×FSDP256 pdbs16  = 599      （已实测，2026-08-04）
128 芯片 DP1×FSDP256 pdbs16  ≈ 599      （同构：DP 组内部完全一样，per-chip 必然相同）
              再叠 dvfs=7    ≈ 650      （599 × 1.086）
                        MFU  ≈ 28.2%    （650 / 2307）
```

⇒ **按现有数据，128 芯片这条路大概率到不了 30%（需要 692 TFLOP/s/chip）。**
30% 要么靠 FP8 路径（625 × 1.086 ≈ 679，分母是 4614 所以 MFU 口径不同），
要么靠还没找到的新收益项。这一轮的价值在于**验证 dvfs 增益是否随规模等幅**，不在于冲 30%。

### ⛔ 配额算不过来：在这个共享 v7 集群上，我们队列的绝对上限是 119 芯片

| ClusterQueue | nominalQuota（芯片） | 当前用量 | 说明 |
|---|---|---|---|
| 另一支团队的队列 | 256 | **256（满）** | 用满自己的 nominal，**不算借用 ⇒ 不可被 reclaim** |
| **我们的队列** | **119** | 0 | 提任务用的就是它 |
| `default` | 0 | 48 | 全是借用 ⇒ 可被 prio 更高的作业回收 |
| **cohort 合计** | **375** | 304 | 物理节点 92 台 = 368 芯片 |

算一遍最好情况：回收 `default` 借走的 48 之后，我们能拿到 `375 − 256 = 119`。
**119 < 128 ⇒ 只要那支队列满载，128 芯片就永远 admit 不了**，Kueue 报
`insufficient unused quota for google.com/tpu in flavor tpu7x-flavor, 121 more needed`。

而 TPU 拓扑不能取中间值（`4x4x4`=64 → `4x4x8`=128 → `4x8x8`=256，没有 96 这一档），
所以**在对方释放之前，这个集群上能跑的最大单 slice 就是 64 芯片**。

**已提交 `hy3-128c` 排队等对方释放**（`priorityClassName: medium` / prio 500，
`4x4x8`，32 节点，sleep-infinity 占位）。挂了 `watch-task`（10 min 轮询）盯准入。

### ⚠️ 踩坑：不带 `priorityClassName` 的 JobSet 是 prio 0，谁都能抢

`hy3-dvfs`（今天跑 DVFS 那四轮用的）提交时**没写 `priorityClassName`**，Kueue 判为 **prio 0**，
最后被一个 prio 500 的作业抢占驱逐：

```
Preempted to accommodate a workload (...) due to prioritization in the ClusterQueue
```

FP8 + QAG 那轮正是死在这儿 —— 编译到 `BACKEND_PASSES` 时节点被收走，`exit 137`。
**在共享集群上提任何想活过 10 分钟的作业，都要显式写 `priorityClassName`。**
本集群可选：`very-low`(100) / `low`(250) / `medium`(500)。

---

## `dvfs_p_state=7` 到底提了谁的频率？—— 不是整颗芯片（2026-08-11 分析）

**问题**：p_state 拉满是主频提高，那芯片里所有单元（MXU、HBM、ICI）是不是都按同一比例变快？

**答案：不是。只有计算域跟着提频，HBM 和 ICI 在各自独立的时钟域，不随 p_state 走。**

### 证据：同一个 flag，五个负载拿到的收益不一样，且与 compute-bound 程度强相关

| 负载 | 类型 | 规模 | MFU | **DVFS=7 收益** | 来源 |
|---|---|---|---|---|---|
| Qwen3-4B | dense SFT | 8 dev | 27.0% | **+8.3%** | wiki `qwen3-4b-sft-guide-20260406` |
| **Hy3-295B** | **MoE 预训练** | **64 chip** | **25.1%** | **+8.6%** | **本文档，2026-08-11** |
| Qwen3-8B | dense SFT | 8 dev | 36.6% | **+9.6%** | 同上 |
| Qwen3-14B | dense SFT | 8 dev | 40.2% | **+10.0%** | 同上 |
| Wan 2.1-14B | diffusion | 64 chip | — | **+11.2%** | wiki `wan21-reproduce-guide-20260402` |

**MFU 与 DVFS 收益的 Pearson r = 0.967（n=4）。**

**这个相关性本身就是结论**：如果整颗芯片等比例提频，那么加速比是纯粹的时间缩放，
**所有负载都该拿到同一个数**。实测从 8.3% 到 11.2% 单调铺开，而且**越 compute-bound 收益越大** ——
只能解释为「一部分时间随频率缩短，另一部分不随」，也就是 Amdahl。

### 排除掉另一种解释

有人会说：收益差异是因为**默认档不同** —— 轻负载被 DVFS 降得更狠，所以提上来赚得多。

**Qwen3 那三行直接否掉了它**：同一硬件、同一框架、同一 batch/seq，只有模型变大，
收益却是 8.3% → 9.6% → 10.0% **单调上升**。若是「默认降频幅度」在起作用，
负载越重应该默认就已经跑在高频、提升空间越小，方向正好相反。

另外本文档 R1 ≡ R3 说明 **Hy3 这个负载默认稳定在 p3**，5 个稳态 step 方差 ±0.02 s，
看不出动态浮动 —— 所谓「动态调频」在满载训练下并没有真的在上下跳。

### 反推核心频率的提升幅度

把 Wan（diffusion，计算最密集）近似当作 100% compute-bound，它的收益就是频率提升的上界：

```
f ≈ 21.18 / 18.82 − 1 = 12.54%
```

代回 Hy3（`step = T_scaled/(1+f) + T_fixed`，实测省下 1.869 s）：

```
T_scaled = 16.77 s  →  71.3%   随核心频率缩短（MXU / VPU 上的计算）
T_fixed  =  6.77 s  →  28.7%   不随频率缩短（HBM 搬运、暴露的 ICI、固定开销）
```

**⇒ Hy3 在 v7 上有约 28.7% 的 step 时间是频率买不动的。** 这个数与
[§4.1 roofline 分解](TUNING-v7.md) 里「remat 融合等高 FLOP 但访存重的 op 占大头」是一致的 ——
那些 op 卡在 HBM 带宽上，MXU 转得再快也没用。

> [!note] 这条推断的证据等级
> 结论建立在**五组黑盒实测的横向对比**上，不是读到的硬件手册或 XLA 实现。
> 想钉死还需要两件事之一：① `xla_tpu_dvfs_p_state` 的实现（Codesearch 当时限流、Moma 认证挂了，没查成）；
> ② 同一负载在 p3 / p7 下各抓一份 profile，直接比 HBM 带宽利用率的绝对值 ——
> **若 HBM 也提频，`GB/s` 的峰值应同比上升；若不变，就坐实了只有计算域在动。**
> 下次拿到卡时值得顺手做，成本只有两轮 profile。

**实践含义**：越 compute-bound 的作业越该开这个 flag；
反过来，如果你的作业只拿到 6–7%，那说明它有相当比例的时间卡在访存或通信上，
真正该改的是那部分，不是频率。

---

## FP8 + QAG 叠加 `dvfs_p_state=7` —— 64 芯片新高 674（2026-08-11 晚）

**同一个常驻 pod 内的 A/B，两轮只差一个 flag：**

| 配方 | step | TFLOP/s/device | **TFLOP/s/chip** | MFU<sub>FP8 峰值 4614</sub> | tok/s/chip | 峰值 HBM |
|---|---|---|---|---|---|---|
| FP8 + QAG，不带 dvfs | 12.758 s | 312.07 | 624.1 | 13.53% | 4,495 | 92.42 G |
| **FP8 + QAG + `dvfs=7`** | **11.814 s** | **337.01** | **674.0** | **14.61%** | **4,854** | 92.42 G |

配方：`DP2 × FSDP64` + pdbs 7 + `use_qwix_quantization=True` + `quantization=fp8_full`
+ `shard_exp_on_fsdp=True` + `weight/act_quantization_calibration_method=fixed,-224,224`
+ tokamax tile (512, 2048, 1536)。

**三条读数：**

1. **674.0 是 64 芯片目前的绝对最高吞吐**，4,854 tok/s/chip = GB300 单卡（6,242）的 **77.8%**。
   调优前起点是 3,208，现在 1.51×。
2. **旧基线被干净复现**：不带 dvfs 这一轮跑出 624.1，与 2026-08-08 记录的 W3（625.4）
   差 **−0.20%** —— 隔了三天、换了 pod、换了节点，说明整套配方与环境是可复现的，
   本次 A/B 的对照组可信。
3. **外推准到 0.7%**：分析时按 `625 × 1.086 ≈ 679` 外推，实测 674.0。

### 这一轮顺便验证了「只有计算域提频」的推断

DVFS 在两种精度上的收益**不一样**，而且方向与[上一节](#dvfs_p_state7-到底提了谁的频率-不是整颗芯片2026-08-11-分析)的模型预测一致：

| 精度 | DVFS=7 收益 | 用 f=12.54% 反推「随频率缩放」的时间占比 |
|---|---|---|
| BF16 | **+8.62%** | 71.3% |
| FP8 + QAG | **+7.99%** | 66.4% |

**FP8 把计算时间压下去了，访存和通信却一点没变** —— 于是不受频率影响的那部分占比从
28.7% 涨到 33.6%，Amdahl 分母变大，同一个 flag 能买到的加速就变小。
这是一个**事前没用来拟合、事后独立落在预测方向上**的数据点，
比单纯的相关系数更能支持那个推断。
