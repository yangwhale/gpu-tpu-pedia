...
出处：
  ...
  · 我们自己的 TPU v7x PD 分离实测（wiki qwen3-coder-480b-pd-disagg-tpuv7x-20260425）：
    KV 4 跳约 100 ms，8K prompt 的 KV 约 1 GB（FP8），占 1–2 s prefill 的 5–10%。
...
def fig_pd():
    f = Fig(W, "为什么要把 prefill 和 decode 分开。上面是放在同一批卡上：大家都在一步一步 decode，"
               ...
               "代价是多了一趟 KV 传输。我们在 TPU v7x 上实测这一趟约 100 毫秒，占一次 prefill 的百分之五到十")
    ...
    f.t(X0 + 13 * CW + 10, py + 222, "KV cache 传过去（我们在 v7x 上实测约 100 ms）", BL, True, 13)
    x = X0
    for i in range(20):
...
def fig_afd():
    f = Fig(W, "attention 和 FFN 也拆开。左边 M 台机器只算 attention，右边 N 台机器只放专家。"
               ...
               "为了把这来回两趟藏起来，把一批请求切成两个小批，一个在算 attention 时另一个在算专家，像打乒乓")
    ...
    # 乒乓时间线
    TX, CW = 830, 54
    f.t(TX, py + 50, "乒乓：两个小批错开跑", INK, True, 15)
    ...
    yb = f.band(py + PH + 20, "ok", "拆得越细，每一边越能挑适合自己的机器和切法", [
        ...
        "代价是每一层都有一趟来回，得靠乒乓小批藏住；两边之间的网络要够快。",
    ])
