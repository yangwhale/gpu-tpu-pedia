...
> ⚠️ 那份官方配置是 **tpu7x** 的，不是 v5p。按 [§2.3](#23-所以xla--kernel-层的结论天生绑硬件代次)
> 的规矩，**具体数值不能照抄**，但**结构（哪一维该给满）是矩阵乘的数学性质，跨代成立** ——
> 跟 [§5.2](#52-第二批两个重点未测项) 里"约束跨代、性能不跨代"是同一条边界。
...
`project` **必传**。耗时**按规模差一个量级**，见 [§4.6](#46-抓-trace-的实测耗时按规模别照抄小规模经验)。
...
### 4.6 抓 trace 的实测耗时（按规模，别照抄小规模经验）
...
MCP 的 60 s 硬超时在任何规模都会命中；**256 芯片下 ssh 直调也要给到 15 分钟以上**。
...
| **R7** | **`use_tokamax_gmm=True`** | v7 上表现为"前几步极慢，看门狗超时被误记成死锁"，**稳态数据始终没拿到**。它同时是 `use_gmm_v2` 的强制前置，而 `gmm_v2` 又是 `num_moe_emb_chunks`（沿 embedding 维分块）的前置。**打不通它，整条藏通信的功能族都用不了** | ⬜ |
...
| R13–R15 | 官方逐通路 tile 模式 / +tokamax / +FP8 | 🔄 |
