...
| **R11** | 用 R5 省出的 HBM 加大 `per_device_batch_size` | ⚠️ **12 超 18.07 G，10 超 7.04 G**（`102.77G / 95.73G`）。每降一档约省 5.5 G → **9 应该刚好能过**，跑测中 |
| **R12** | `--xla_tpu_pcie_bandwidth_multiplier` 0.03 → 1.0 | 🔄 跑测中。当前值 0.03 极不寻常，值得单独看 |
