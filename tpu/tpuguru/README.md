...
## 4.7b 报告分两份：AOT 报告 / 真机报告

它们回答的是**不同的问题**，混在一起会让人拿 A 的结论去信 B：

| | AOT 报告 | 真机报告 |
|---|---|---|
| 回答 | 装不装得下 / 走哪条代码路径 / 踩没踩坑 | **跑多快** |
| 代价 | 3 分钟，0 张卡 | 20–40 分钟，占共享集群 |
| 顶部第一条 | 结论（装得下 / 装不下） | **XProf trace 链接** |
| 深挖按钮 | 🔬 HLO 深挖 | 🔬 真机分析 |

两边都有「让 agent 写解读」的按钮，喂进去的都是**事实不是结论**。

## 4.8 ★ 上真机：把「跑多快」那一半补上

AOT 只回答「装不装得下」。**「多快」只有真机能答**，而真机数据很贵
（占共享集群、几十分钟、要抢卡），所以这条链的第一原则是
**跑一次就把所有东西留下来**。

### 卡数只能选 AOT 验证过的那个

上机弹层里 4/16/64/128/256 都列出来，但**只有 AOT 验过的那一档是亮的**，
其余灰掉并说明为什么：

> AOT 验的是 tpu7x-128。**换卡数就换了分片宽度**，那份显存结论对这个规模不成立 ——
> 先把拓扑改成 tpu7x-256 再跑一次 AOT（3 分钟，不占卡）。

后端也拦一道（不只靠前端）：`/api/metal` 收到与 `target.topology` 不符的 topology 直接 400。

这条看着像限制，其实是**省事**：想换规模，先花 3 分钟跑个 AOT，
比上去 30 分钟才发现装不下划算得多。
...
1. **当前配置的 AOT 编译通过**（`ok=true` 且结论没被判无效）
2. **集群状态灯不是红的**

灰着的时候鼠标移上去会说清是哪一条不满足。
装不下就上机，是在浪费别人的卡。

### 集群状态灯

顶栏那盏灯的判据是**看队列不看节点** —— 这个集群里「0 节点」通常意味着
Kueue 还没 admit 你，而不是抢不到容量；反过来「Kueue 记账满了」也不等于机器忙。

| 灯 | 含义 |
|---|---|
| 🟢 绿 | cohort 空闲 ≥ 你要的，几十秒起来 |
| 🟡 黄 | 在保底内需抢占，或超出保底但借得到 —— 要等几分钟到十几分钟 |
| 🔴 红 | 超出的部分 > cohort 空闲，借不到。改小规模 / 等释放 / 挑 peak 窗口 |

三种都带一句判据文字，不让人对着一盏灯猜。

### 跑完之后存什么

| 存什么 | 放哪 | 为什么 |
|---|---|---|
| 完整训练日志 | GCS run 目录 + 本地 | 出了事只能靠它 |
| profile 原始产物（xplane） | 同上，并拉一份进 XProf 的 logdir | trace 是最贵、最难复得的 |
| per-chip TFLOP/s / MFU / step time / loss | Firestore `tpuguru_metal` | 能排序、能画趋势 |
| **XProf trace 链接** | 报告页**最上面一条** | 这是真机跑完最重要的产出 |

**配置指纹是把两半缝起来的针。** `tpuguru_metal` 的 doc 带同一个 `fingerprint`，
于是同一套配置下「AOT 说装得下 93.19 GB」和「真机跑出 670.8 TFLOP/s」
会自动并排出现，而不是两笔各存各的。

### 日志解析的三条口径纪律

1. **跳过前几步。** 编译与 warmup 混在里面，取稳态。
2. **step 时间取中位数**，不取平均 —— 单步抖动会把平均带歪。
3. **`TFLOP/s/device` 要 ×2 才是 per-chip。** v7 是 2 device/chip，
   框架日志一律按 device 报。忘了这个 ×2 就差一倍，而且看着很合理。
   MFU 的分母也要写出来（BF16 峰值 2307）。

另外 **loss 全是 0 要当场报警**：梯度没流动的话，吞吐数字再漂亮也不能用。

### 部署

`https://cc.higcp.com/tpuguru/` → hk-jmp 的 Caddy → 本机 `:8820`，**IAP 在前**。
照 `/xprof` 那段写：`redir` 补斜杠 + `strip_prefix`。
两个坑：服务要绑 `0.0.0.0`（只听 127.0.0.1 会 502），
前端资源与 API 全用相对路径（`strip_prefix` 后端不感知前缀，绝对路径会指到根上）。
...
### ★ J-0. 一次被真 AOT 证伪的推算（2026-08-16）

**先记这条，因为它推翻了 J 里原本的做法。**

改成 40 层之后，工具用「常驻 + 固定激活 + 斜率 × batch」按层数折算，
给出「推荐 batch 44、上限 45」。听起来很像真的。

然后真跑了一遍 AOT（本机 docker，CPU，并行 6 个容器）：
...
| 12 | 80.82 GB | ✅ | — |
| 13 | 91.94 GB | ✅ | — |
| **14** | **85.19 GB** | ✅ | ← **比 13 少 6.75 GB** |
| **15** | **89.64 GB** | ✅ | **← 真实上限** |
| 16 | 95.32 GB | ❌ 编译期 OOM，超 593 MB | |
| 17 | 97.88 GB | ❌ | |
| 18 | 101.23 GB | ❌ | |
| 19 | 105.58 GB | ❌ | |
| 40 | 192.09 GB | ❌ | |
| **45** | **207.67 GB** | ❌ | **93.85 GB「装得下」** |
| 46 | 211.71 GB | ❌ | |

**推算说上限 45，真实上限 15 —— 差 3 倍；pdbs 45 那一档差 2.2 倍。**

两个原因，都很硬：

1. **激活不随层数线性缩。** 层数减半，激活远没有减半（scan + offload 下尤其不缩）。
2. **batch 方向根本没有「斜率」这回事。** 实测相邻档差：
   `8→12` 是 +5.25/档，`12→13` 是 +11.12，`13→14` 是 **−6.75**，`14→15` 是 +4.45。
   **batch 变大反而少用 6.75 GB**，比 80 层那组（差 0.5~1.3 GB）剧烈得多，一次跳 7%。

于是把 J 里的**推算外推与推荐 batch 整段撤掉**：只保留能精确算的常驻，
没有实测点就不给数字，改成给一份二分计划。代码里留了注释写明这段历史。

> **一个错的推荐值，比没有推荐值更糟。**
> 这一条是这个工具自己的告警（「必须逐档真跑」）反过来把自己按住的一次。

### 并行扫描（`worker/aot-sweep.sh`）

**AOT 是纯 CPU 任务，不该排队跑。** 80 核机器上 5–6 个容器并跑，
两分钟出五个点，比串行快一个数量级。

```bash
LAYERS=40 CPUS=11 worker/aot-sweep.sh 13 14 15 17 18
```

另有一条经验：**OOM 的档失败得快**（显存分配阶段就挂），
装得下的要跑完整 codegen 才结束 —— 所以并行时先返回的往往是坏消息，别急着下结论。

还有一条选点纪律：pdbs 45 需要 207 GB 时，退到 40 只降 8%，纯属浪费一次编译。
**差得远的时候要跳，不要挪。**
...
### ★ K. HLO 深挖 —— AOT 产物里能榨出来的东西

**先扒真产物，再设计维度。** 一次 real 模式的 AOT 会 dump 出 90+ 个文件、
40 MB，里面的东西比「装不装得下」多得多。下面五个维度是照着真实 dump 定的，
不是凭想象列的。

| 维度 | 数据来源 | 挖到什么（Qwen3-235B pdbs 28 实例） |
|---|---|---|
| **显存都花在哪** | `memory-usage-report.txt` | **编译器自己算的**分配明细。第一名 `bf16[28,4096,151936]` 单块 32.46 GiB —— 就是 logits，两块加起来占 peak 七成以上 |
| 编译器做了什么 | `after_optimizations.txt` | fusion 1063 个：kLoop 1154 / kCustom 62 / kOutput 53 |
| 集合通信 | 同上 | all-gather 44、reduce-scatter 16、all-reduce 8 —— **次数这么少说明各层被合并、提出了循环** |
| 精度实际生效没 | 大张量 dtype | 查到 `f8e4m3fn`/`f8e5m2` → FP8 真走到了。**配置写了不等于生效** |
| 算子频次 | 同上 | 14,191 条，parameter 31% / broadcast 12% / … |

**统计做完之后交给带 skill 的 agent 写解读，只喂事实不喂结论** ——
结论让它自己下，这样它给的东西才可能超出预先写死的那几条。
实例里它就多说了一句我没写的：那两块 logits **同时活在峰值上**。

#### 后加的三个维度（第一版没有，翻产物翻出来的）

**★ XLA flag 到底生效没有** —— 这是「配置写了不等于生效」在 flag 层的版本。
`tpu_comp_env.txt`（53 KB）是编译器**实际生效的 1133 个 flag**，
而且给每个偏离默认值的都打了注释标记。拿它跟实际传进去的那份对，分三类：

| | 含义 |
|---|---|
| ✅ 传了且偏离默认 | 真生效了。显示「传的值 vs 默认值」 |
| ⚪ 传了但等于默认 | 不是错，但**别以为自己开了什么** |
| ❓ 编译器环境里找不到 | ⚠️ 名字拼错或这版不认 —— **会静默忽略** |

**SparseCore 卸载** —— 11.8 MB 元数据、139 条记录 = 卸载真的发生了；空的话那一族 offload flag 就白开了。

**LLO** —— 这一栏是**空的，而且该空着**。`--xla_dump_to` 只出到 HLO 与 codegen，
LLO（VLIW 打包、寄存器分配、bundle 占用）要编译器内部开关，云上镜像关着。
所以这一栏写的是「为什么没有」+「该去哪看」：
**LLO 层的问题该去真机 trace 看 XProf 的 op profile** ——
每个算子实际占多少周期、MXU 利用率多少，那才是 LLO 调度好坏的直接证据。
硬凑一个 LLO 面板是假功能。

#### 写这个分析器时踩的三个坑（都是「看着像真的」那类）

**① 单张量大小不能乘份数。** `7×bf16[917504,4096]` 里那 7 份**共用同一块 offset**
（复用），乘了会算出「单张量 49 GiB 装在 32.46 GiB 的块里」这种自相矛盾的数。

**② before/after 算子数不构成「缩减率」。** 我一开始算出 **−111%**。
XLA 是**先展开（scan 展开、大算子拆细）再融合**，优化后条数比优化前多是正常的。
这个指标概念本身就错，该看的是 fusion 的数量与种类。

**③ 拿错了「用户传了什么 flag」，报告因此说了假话。**
最先我用会话里的 `xla_flags`（只有用户粘命令时才有），
用下拉建配置时它是空的 —— 于是报告写「XLA flag 传了 0 个，
编译行为全是默认值，**没有任何调优在起作用**」，
而那次编译实际传了 16 个 flag、全部生效。
**这正是本工具存在要防的那类错，只不过这次犯在工具自己身上。**
修法：把真正传给编译器的那串 flag 记进 run 结果（`xla_flags_used`），
核对时以它为准。旧记录已回填，基于错前提写出的旧解读全部作废重算。

#### 给 agent 写解读的两条纪律

**每次分析必须是独立会话。** 最初所有 explain 调用共用一个 bot session id，
于是它写出「上次我说的」「本轮第一条」「我收回之前那句」——
对刚打开页面的人毫无意义，而且上下文无限增长最终会漂。
要跨次对比就把要对比的**数据**显式喂进去，不要靠隐式聊天历史。

**只喂事实不喂结论。** 实测下来它确实能给出超出预设的东西：
自己验算出 `917504 = 28×4096×8` 反推 top-8 路由、
从 Custom fusion 仅 62 个推断「手写 kernel 基本没接上」、
拿 68 次集合通信跟 94 层比判断被提升出了循环 —— **并主动指出这个判断有歧义**
（层体被 scan 成单份编译也是同样的计数）。
