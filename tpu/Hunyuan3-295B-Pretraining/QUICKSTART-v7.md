...
| FP8 调参空间 | ⛔ 曾以为「换 tile 入口 674 → 1,014.8」，**该结论已撤回**（漏算 189/192 专家）。有效水位仍是 **677**（tokamax+QAG）。见 [TUNING §3.4.10](TUNING-v7.md) |
...
| **64 chip** 🏆 | **FP8 native + FSDP128 + dvfs 7**（不开 QAG） | **20.34 s** | **727.0**（峰值 `fixed`）／ **670.8**（生产 `absmax`） | — | — |
...
| **最高水位** | **727.0** 峰值（FP8 native FSDP128 + `fixed`，⚠️ 仅 benchmark）／ **670.8** 生产（同上 + `absmax`）／ **666.6**（BF16） | 161.0（已收敛） |
