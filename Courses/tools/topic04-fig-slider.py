...
# ⭐ Ⓐ 的小例子：旋钮转 0.01，读数动 0.03 → 兑换率 3
# ⛔⛔ 2026-09-23 换比喻。原来是「调音台的推子 → 难听程度」，现场：
#   「这里边的难听程度是啥？你能搞一个大家都能理解的例子吗？」
#   ⭐ 病因不只是「不好懂」：**「难听」不是个能量的数，是个主观感觉**，
#     而 loss 恰恰是一个数 ——&#160;比喻在最关键的那一点上失效了。
#   ⛔ 还有更深的一层：它听起来像是**输出本身**，可 loss 是**输出跟目标的差距**。
#     那个比喻把这件事悄悄弄反了。
#   ⭐⭐ 换成**淋浴**：旋钮是热水龙头，读数是「水温离你想要的差几度」。
#     三样都对上了 ——&#160;差几度是个真数、它天然就是「跟目标的差距」、
#     而且「转一格，差值动三格」跟原话是同一句。
#   ⭐ 判据：**一个比喻要能把「被比喻物最要紧的那个性质」带过去。**
#     带不过去的，换掉；只是「不够生动」的，不用换。
...
def main():
    ...
    py2 = f.panel(0, py + PH + 20, W, PH2,
                  ...
                  sub="⭐ 一个 6,710 亿个旋钮的淋浴，一次只拧一个，其余全按住")
...
# ══════════════════════════════════════════════════════════════════
# 第二张图：只有那一格。⭐ 课件那边用 <details> 收着（__FIG_SLIDER_JAC__）。
# ══════════════════════════════════════════════════════════════════
def jac():
    g = Fig(W, "前面那张图里的旋钮是一维玩具：一个旋钮、一个读数，"
               "所以那一级的斜率是一个数。"
               "可真实的一级是七千多维进、七千多维出，"
               "它的斜率不是一个数，是出去的每一维对进来的每一维各有一个兑换率，"
               "概念上是一整张表。"
               "但机器里并没有这样一张方阵，真实的权重没有一块是方的。"
               "好消息是我们从来不把那张表算出来 —— "
               "反向只做一件事：拿责任去乘它，直接得到上一级该收到的责任，"
               "那正是反向那两笔乘法里的第二笔")

    y0b = g.header(
        "补一格：真实的一级<tspan font-weight=\"700\">不是一个标量</tspan>",
        "⭐ 前面那张图是<tspan font-weight=\"700\">一维玩具</tspan>；"
        "这一格说清它跟真实的一级差在哪 ——　<tspan font-weight=\"700\">"
        "结论只有一句：那张表我们从来不算出来</tspan>",
        [(RD, "斜率是一整张表")])

    # ══════════ 单独一张图：真实的一级，那个「斜率」是一张表 ══════════
    # ⛔⛔ 2026-09-23 现场：「这部分没有额外的信息，跟上边说的东西
    #   并没有什么加持和增益，把这一块折叠起来。」
    #   ⭐ 它原来是 fig-slider 的第四格。可**面板折不起来** ——&#160;
    #     一张 SVG 要么整张在、要么整张不在。所以拆成**独立的一张图**，
    #     由课件那边用 <details> 收着。
    #   ⭐⭐ 判据：**「这一段能不能折叠」是个排版问题，但它先是个切分问题。**
    #     想折的那块必须先是一个**独立的产物**，否则折叠这个动作根本没有抓手。
    #   ⚠️ 没有删：它回答的是现场自己提过的一问（「真实的一级是个矩阵吧」），
    #     只是从主线降级成了「想看再展开」。
    # ⭐⭐⭐ 2026-09-22 现场那一问的后半句，也是更要紧的那半句：
    #   「实际上它是一个矩阵才能产生这个放大和缩小的作用。」——&#160;完全正确。
    #   ⛔ 前面三格是**一维玩具**（一个旋钮、一个读数），玩具里斜率确实是个标量；
    #     可真实的一级是「七千多维进、七千多维出」，它的斜率是**一整张表**。
    #   ⭐ 而这一格最该留下的不是那张表有多大，是下面这句：
    #     **我们从来不把它算出来。** 反向只做「拿责任去乘它」这一件事。
    DJ = 7168
    PH4 = 320
    py4 = g.panel(0, y0b, W, PH4,
    ...
    g.box(60, py4 + 70, 190, 72, "#e8f0fe", BL, 6)
    g.t(155, py4 + 100, "进来", BL, True, 14, "middle")
    g.t(155, py4 + 126, "%s 个数" % format(DJ, ","), INK, True, 15, "middle")
    g.line(256, py4 + 106, 288, py4 + 106, PU, 2.2)
    ...
    g.box(294, py4 + 62, 200, 88, "#fef7e0", OR, 6, sw=1.6)
    g.t(394, py4 + 90, "装置", OR, True, 15, "middle")
    g.t(394, py4 + 114, "几块权重 ＋ 激活", INK, True, 13, "middle")
    g.t(394, py4 + 136, "（7,168→2,048 这种，<tspan font-weight=\"700\">都不是方阵</tspan>）",
        GY, size=12, anchor="middle")
    g.line(500, py4 + 106, 532, py4 + 106, PU, 2.2)
    g.box(538, py4 + 70, 190, 72, "#e8f0fe", BL, 6)
    g.t(633, py4 + 100, "出去", BL, True, 14, "middle")
    g.t(633, py4 + 126, "%s 个数" % format(DJ, ","), INK, True, 15, "middle")
    ...
    g.box(770, py4 + 58, 580, 210, "#e6f4ea", GR, 8)
    g.t(1060, py4 + 94, "⭐ 但好消息是：", GR, True, 17, "middle")
    g.t(1060, py4 + 130,
    ...
    yb = g.src(y0b + PH4 + 30,
               ...
               "⚠️ 图里那个 %s 是 V3 的残差流宽度；"
               "⛔ 注意<tspan font-weight=\"700\">机器里并没有一块方阵</tspan>，"
               "真实的权重没有一块两边一样宽。" % format(DJ, ","))

    g.save("fig4-slider-jac.svg", yb + 14)
