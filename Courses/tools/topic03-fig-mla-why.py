...
def main():
    ...
    f.t(620, cy + 90, "⭐ 复印件再多，也不会比原稿多出内容", GR, True, 17)
    f.t(620, cy + 130, "所以「存 128 份」本来就是白占地方 ——", GY, size=17)
    f.t(620, cy + 162, "改成「只存那一张原稿」，一个字的信息都没丢。", GY, size=17)
    ...
    f.t(502, dy + 78, "⚠️ 这一步跟上一步，性质完全不同", PU, True, 17)
    f.t(502, dy + 114, "上一步是「扔掉复印件」——&#160;不丢信息，算出来的。", GY,
        size=17)
    f.t(502, dy + 146, "这一步是「把原稿缩小 12 倍」——&#160;一定会糊，", GY, size=17)
