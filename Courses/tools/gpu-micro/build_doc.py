...
<p class="sub">前面七张图的结论，最后收在这一张上。</p>''')
    a(fig(8, "<b>图 G-8　等面积对照。</b>两个方块的<b>面积</b>按每周期乘加单元总数等比画，"
             ...
             "格子差 64 倍，这个反差就是全文的落点。右边 MXU 里的网点只表示「密」，"
             "真按 256×256 画每个 cell 只有 0.27 像素。"
             "右下角绿框是这套口径在 v5e / v6e 上的验尸 —— 那两代四个变量全公开，"
             "是唯一能证伪它的地方。"))
    a('''
<p>这张图是全文的落点。左边一颗 B200 有 592 个 Tensor Core，每个每周期 1,024 次乘加，
合计 606,208；右边一颗 TPU v7 chip 合计 524,288。
<b>总量几乎一样（1.16×），而单元粒度差 64 倍。</b></p>

<h3>这里有一处必须交代的返工</h3>
<p>本文第一版在这张图上写的是「4 个 MXU，每个 131,072 乘加/周期」，理由是
「每个 cell 每周期做两次乘加」。<b>那个说法是错的</b>，而且它错得很隐蔽 ——
它凑出的总量（524,288）跟现在这版一模一样，所以从 B200 那边怎么对都看不出问题。</p>
<p>戳破它的办法是<b>拿同族的已知样本当验尸台</b>。v5e 和 v6e 这两代的
MXU 个数、阵列边长、峰值、时钟<b>全都是公开的</b>，用同一套口径算一遍：</p>
<pre><code>口径：峰值 ÷ 2（一次乘加 = 2 FLOP）÷ 总 cell 数 = 时钟

v5e   4 MXU × 128×128 = 65,536 cell   @ 1.50 GHz  →  196.6 TF   官方 197   差 0.20%
v6e   4 MXU × 256×256 = 262,144 cell  @ 1.75 GHz  →  917.5 TF   官方 918   差 0.05%

两代都还原成功 —— 而且都是「每 cell 一次乘加」，没有双发这回事。
旁证：Google 把 v6e 相对 v5e 的 4.7× 归因于「4 倍 cell + 更高时钟」，
     4 × (1.75 / 1.50) = 4.67，与 918 / 197 = 4.66 吻合。</code></pre>
<p>如果真有「每 cell 双发」，这两代会各算出一倍的偏差。它们没有。
所以那个假设只是为了把 v7 的账凑平而临时发明的 —— <b>一个只能解释目标、
解释不了同族已知样本的模型，多半是错的。</b></p>

<h3>那么 v7 到底是什么样</h3>
<p>把验过的口径用到 v7 的 2,307 TFLOPS 上，<b>能定死的只有一件事：
每周期乘加总数 ＝ 524,288</b>（它是唯一落在 2 的整数次幂、且对应时钟落在合理区间的解；
再高一档要 4.4 GHz，再低一档要 1.1 GHz，都不像话）。</p>
<p>但这 524,288 <b>怎么拆，公开资料自己是打架的</b>：官方 v5e 页写「每个 TensorCore 四个 MXU」，
配 197 TFLOPS 精确吻合；官方 v6e 页现在写「两个 MXU」，可是两个 256×256 的 MXU
配不出它自己那一页上的 918 TFLOPS，要四个才配得出。按 v5e/v6e 这条同构的线读下去，
v7 应该是每核 4 个、全 chip 8 个。<b>本图就是这么画的，并如实标成推导。</b></p>
<div class="note info"><span class="t">拆法不影响这张图的结论</span>
无论是 8 个 65,536 还是 4 个 131,072，总量都是 524,288，
和 B200 的 606,208 都差 1.16 倍；粒度比则在 <b>64 倍到 128 倍</b>之间。
两种读法给出的是同一个量级判断 —— <b>差两个数量级</b>。
这张图想让你记住的是这件事，不是那个具体倍数。</div>
...
B200 这个是推的，另有康奈尔教程旁证）；TPU v7 每 chip 524,288 乘加 / 周期，
以及它<b>拆成 8 个 MXU</b> 这个读法</td>
<td>§2 用 A100 / H100 两代官方数字验证口径；§8 用 v5e / v6e 两代验证。
<b>第一版写的「每 cell 双发」已被 v5e / v6e 证伪并撤回</b>，返工过程写在 §8</td></tr>
</tbody></table></div>
</section>''')
...
<tr><td>TPU v7 的时钟</td>
<td><b>官方未公开</b>，本文不给数字。相关的是：v7 每周期乘加总数<b>能</b>定死（524,288），
但这个总量<b>拆成几个 MXU</b> 定不死 —— 官方 v6e 页写「2 个 MXU」，
可是两个 256×256 配不出它自己那页上的 918 TFLOPS，要四个才配得出。
官方文档在这一点上自相矛盾，本文按 v5e / v6e 的同构读法画，并标为推导</td><td>G-8</td></tr>
<tr><td>B200 的确切 boost 时钟</td>
...
§8 的「每 cell 双发」（被 v5e / v6e 证伪）、以及 §2 那句「NVIDIA 从不公布」
（官方白皮书里白纸黑字写着）。<b>三处都在正文里留了返工记录，没有悄悄改掉</b> ——
