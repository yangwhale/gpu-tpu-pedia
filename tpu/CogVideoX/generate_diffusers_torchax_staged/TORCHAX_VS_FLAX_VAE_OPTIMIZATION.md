...
3. 🐛 **问题诊断与解决** - 从 500s 到 1.30s 的优化之路
4. 📊 **性能分析** - 详细的性能对比和瓶颈分析
5. 📚 **最佳实践** - TPU 优化的通用经验
...
## 第六章：从 500s 到 1.30s 的优化之路

### 6.1 问题诊断

```mermaid
pie title 优化前 Flax 性能问题分布
    "无 JIT 编译 (解释执行)" : 60
    "无 TPU 分片 (单 TPU OOM)" : 30
    "数据转换开销" : 10
```
...
3. **性能优化**：通过分片 + JIT 实现 400x 加速（从 ~500s 到 1.30s）
