...
# ⛔ 2026-09-05：这里原来就叫「片上累加器」，而 3.2 的 T-2 图里
#    「累加器」指的是挂在每个 MXU 上的硬件寄存器（1 MiB）。同名不同物，
#    于是「累加器 128 MiB 比整块 VMEM 还大」读起来像硬件寄存器爆了。
#    ⭐ 形状：**一个词在两节里指两个东西，比数字写错更难查** ——
#      两处单独看都对，只有并排读才炸。
t(sx + 276, sy + 68, '这一段的局部和（占 VMEM）', fill=GY)
...
t(918, YB + 74, '那样每个 n 块的局部和要占', fill="#a50e0e")
t(918, YB + 94, '131072 × 256 × 4 B', "svglbl", "#a50e0e", size=13)
t(918, YB + 114, '＝ 128 MiB', "svglbl", RD, size=18)
t(918, YB + 138, '——&#160;<tspan font-weight="700">比整块 VMEM（64 MiB）还大</tspan>。',
  fill="#a50e0e")
...
COLS = (16, 190, 380, 600)
for cx, h in zip(COLS, ('一段多少行', '局部和占 VMEM', 'B 进阵列的总次数', '乒乓藏得住吗')):
