...
#### 3.4.8 ✅ 已定案：native 路径没有算少，FP8 配方可以用

> **2026-08-15 22:40 结论。32 步 loss 对拍完成，疑点排除。**
> 下面保留完整的调查过程，因为**错误的推理链本身值得记**。

##### 定案证据一：logits 分布的 KL 散度 = 0（最强的一条）

> 方法本身已抽成可复用的文档：[**换 kernel 的等价性验证 —— KL 散度做法**](../kernel-equivalence-validation/)。
> 下面只记本次的具体结果。

loss 是归约过的标量，能掩盖差异。**正确的度量是在 logits 上比 KL 散度** ——
它衡量的是「模型对下一个 token 的概率分布」是否相同，而不是某个中间激活差多少。

做法（三个坑都要绕）：

1. **在图里算 `log_softmax`**，让 XLA 自己做跨分片的 max/sum。
   直接把 logits 分片存盘再离线归一化是错的 —— 每张卡只有词表的一段。
2. 存下来的分片按序一一对应（两次 run 的 mesh 与分片规格相同），
   离线 `KL = Σ_v p_v (log p_v − log q_v)` 逐片求和 = 整词表上的精确 KL。
3. 钩子挂在 `max_utils.cross_entropy_with_logits`。**注意同一个文件会被以
...
> 在其内部做的归约拿到的是**本卡那一片**的值，不是全局值；哪张卡先打不确定，
> 于是两次 run 天然对不上。**得到的「换 tile 就 0/30 位相同」是采样噪声，不是数值差异。**
> ② 想额外打 `max/min/头 16 元素原始位` —— 与 `decoder_layer_input=offload` 冲突，
...
`ici_data=2 / ici_fsdp=64`，64 芯片）：

| step | native loss | tokamax loss | Δ |
|---|---|---|---|
| 0 | 13.423 | 13.423 | **0.000** |
| 3 | 13.001 | 13.161 | +0.160 |
| 9 | 12.572 | 12.375 | −0.197 |
| 28 | 11.002 | 10.998 | −0.004 |
| 29 | 10.981 | 10.977 | −0.004 |
| 30 | 10.963 | 10.959 | −0.004 |
| 31 | 10.945 | 10.941 | **−0.004** |

**第 0 步逐位相同**（13.423 = 13.423）—— 前向数值完全一致，
`3` 那个维度不是「少算的专家数」。中段最大偏离 0.197（step 9），
之后**重新收敛**，第 31 步只差 0.004（0.04%）。

**真算少 64 倍的专家贡献不可能是这个形状** —— 那会单调发散，不会先分开再合回来。
中段的鼓包是 FP8 量化在两种 kernel 下的舍入路径不同，属噪声。

同时 native 快 **50%**：7.84 s/step @ 507.6 TFLOP/s/device
vs tokamax 11.78 s/step @ 338.2。

> 完整日志：`~/tpu-logs/hy3-tk30-32step.log`（32 步全量）。
> **所以「疑点 2 —— 我把维度认错了」是对的**：`[192,1536,4096]` 与 `[3,4096,1536]`
> 连维序都不一样，本来就不是同一个张量的两种分片，我从一开始就在比错东西。

---

##### 以下为调查过程存档（结论已被上面推翻）

> 2026-08-15 20:35 当时的判断。**保留原文，不要照着改生产配置那句已失效。**

##### 疑点

profile 里同一个位置的 MoE kernel，两条路喂进去的形状不同：

| | 循环内 kernel 的第一个操作数 | 平均单次 |
|---|---|---|
| tokamax | `bf16[192,1536,4096]` | 5,692 us |
| native | `bf16[3,4096,1536]` | **94.5 us** |

`192 / 3 = 64 = ici_fsdp_parallelism`，耗时比 60× 与之吻合。

**而 Hy3 在 TPU 上的 MoE 设计是「token 不搬家、每卡 all-gather 全部专家、
在本卡上算自己的 token × 192 个专家」** —— 两边 HLO 里 `all-to-all` 都是 **0 个**，
印证了这个设计。那么每卡只拿 3 个专家去算，**除非之后对专家维做规约，否则就是算少了**。

**但我在 native 的 HLO 里找不到那次规约**：两边 `all-reduce` 都是 36 个、形状分布也几乎一样。

##### 与之矛盾的证据

**loss 对得上**：FP8 新旧两轮逐步差 ≤0.001（10 步）。如果真少算 64 倍的专家贡献，
不该这么接近。

##### 三个待排除的可能

1. **10 步太短看不出来** —— 前几步 loss 由 embedding/attention 主导，MoE 被削弱一时不显形。
2. **我把维度认错了** —— 注意 `[192,1536,4096]` 与 `[3,4096,1536]` **连维序都不一样**
   （1536/4096 对调）。这不像同一张量的分片，**更可能是两个不同的张量**，
   即我从一开始就在比错东西。**这一条嫌疑最大。**
3. **规约藏在 fusion 里**，正则没抓到。

##### 怎么定论

**跑 30 步以上的 loss 对拍。** 如果 native 真算少了，一定发散。
64 卡两轮，约 40 分钟。**已做，见本节开头 —— 没发散，疑点排除。**

> **本轮调查我连错三次归因**，都是「看到一个合理机制就当答案」：
> ① 说 tokamax 没融合量化链 → HLO 证明两边都融了；
> ② 说是 autotune cache miss 的搜索开销 → fallback 默认就是 `heuristics`，autotune 没跑；
> ③ 说 native 把专家分片是 bug → QAG 的 all-gather 是**故意设计**，
> 且 QAG 只在 tokamax 分支存在（`ops.py` 里那段在 `if use_tokamax_backend:` 里面，
> `backend.gmm` 的签名根本没有 `weight_gather_axes`）。
>
> **教训：性能数字可以信（多次复现、XProf 自证），但机理归因必须能被独立证据钉死，
> 否则就标成未决。**

#### 3.4.9 FP8 的 tile 空间已到顶（2026-08-15 扫描，64 芯片）

现配方那 18 个 tile 参数是在 BF16 上扫出来的，FP8 沿用。
FP8 每个格子能装的元素翻倍，理论上最优点可能更大 —— **实测证伪，往上没有空间了**。

| 组 | (batch_seq, embed, mlp) | TFLOP/s/chip | 结果 |
|---|---|---|---|
| base（对照） | 512, 2048, 1536 | **1014.6** | ✅ 复现基线，环境无漂移 |
| bs1024 | **1024**, 2048, 1536 | 1013.9 | ✅ 跑通但**无收益** |
| em4096 | 512, **4096**, 1536 | — | ❌ `INVALID_ARGUMENT: Vectorizing shape must be 1D.` |
| mlp3072 | 512, 2048, **3072** | — | ❌ `AssertionError: v=1536 bv=3072 s=1536` |

**两条失败各自的意思**：

- `mlp3072` 是**配置本身非法**：`base_moe_mlp_dim = 1536`，分块要 3072 —— 比维度还大。
  断言里 `v=1536`(实际维度) / `bv=3072`(给的分块) 直说了。
  **反过来说明现配方的 mlp tile 1536 已经等于整个 mlp 维，没有上升空间。**
- `em4096` 是 **Mosaic 的向量化限制**：embed 维 4096，tile 取满就触发
  `Vectorizing shape must be 1D`。所以 embed 最多只能到 2048（半个维度）。

| 维度 | 实际大小 | 现用 tile | 还能往上吗 |
|---|---|---|---|
| mlp | 1536 | **1536** | ❌ 已取满整个维度 |
| embed | 4096 | 2048 | ❌ 取满撞 Mosaic 限制 |
| batch_seq | — | 512 | 可以，但 1024 无收益 |

**结论：`512 / 2048 / 1536` 就是这条路的终点。** 与 BF16 那边「调参见底」一致，
下一个量级的提升不在 tile 上。

> ⚠️ 扫描设计的教训：**分块不能大于维度本身**。
> 设计参数扫描前先把每个维度的实际大小查出来，否则一半的组是无效配置，白烧机时。
