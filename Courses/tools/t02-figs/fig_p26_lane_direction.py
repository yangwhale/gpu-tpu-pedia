...
都是公开资料的说法；而**「列与列之间没有直连通路」是我为「它为什么贵」给的解释** ——
公开资料没有描述 XLU 的内部电路。底带那条红带不能省，
它是这张图里唯一一处「我在推，不是查到的」。
"""
...
def _bound(f):
    ...
    f.t(38, BND_Y + 28, "⚠️ 这张图里有一处是我在推，不是查到的", "sec")
    y = para(f, 38, BND_Y + 54, 1324,
             "<b>「列与列之间没有直连通路」是我为「它为什么贵」给的解释。</b>"
             "公开资料只说 XLU 是个独立单元、而且慢，"
             "<b>没有描述它的内部电路</b>。", "xs", 19)
    para(f, 38, y + 2, 1324,
         "查到的那几条是：<b>沿 8 那个轴的 shuffle 约一个周期</b>、"
         "<b>跨 lane 要经过 XLU</b>、<b>XLU「慢且贵」</b>、"
         "<b>一个标量核配 2 个 XLU</b>（未标代次）。<b>机制这一层没有出处。</b>",
         "xs", 19)
