...
一整天下来，真正能用的只有两个开关，但它们**完全线性叠加**：

| 配置 | step (s) | TFLOP/s/chip | MFU | Δ |
|---|---|---|---|---|
...
> tile 512 那条做过**同批次复核**：61.510 s vs 早期单独跑的 61.496 s，
> 差 **0.02%**。这个 +2.67% 站得住。

**这是本文档所有实验里唯一的净收益。** 其余十几轮全是负的或崩的——
它们的价值在于[§2](#2-主线结论v7-的经验一条都没搬成) 那条主线结论，不在性能。

### 1b.1 HBM 用量：一直印在日志里，我却一直没读

**每一轮的日志里都有这两行**，我却直到最后才去看，之前只在 OOM 时被动从报错里捡数：
...
| 从实测峰值反推 `93.17−92.68`（8→9） | **0.49 G** |

差别在于 **OOM 报的是"编译期估算的 HLO 临时量总需求"，不是运行时真实峰值。**

**② 所以 `pdbs=10` 未必真的装不下。** 按真实峰值外推 `92.68 + 2×0.49 = 93.66 G`，
离 95.73 还有 2 G 余量。**它是被编译器的保守估算挡回来的。**

**③ [§2.1](#21-五项判决) 里"优化器 BF16 省 HBM 换 batch"那句要改。**
实测只省 **1.2 G**（91.47 vs 92.68），连 3 档 batch 都换不到，
而且稳态反而更高（80.32 vs 79.35）。**这条不能当省显存手段。**

> **教训**：从头到尾都该记的一列数据，我只在报错时才被动去看。
> **一个开关对 HBM 的影响，和它对速度的影响一样是实验结果**——
> 记全了才能算出"batch 到底能开几档"，而不是一档一档试到 OOM。
> 更糟的是：不记这一列，就发现不了「OOM 报的数和真实峰值差 10 倍」这种事。
...
| 优化器状态 BF16 | 未测 | −0.19% | ⚪ 速度零收益（0.25% 抖动内）。~~省 HBM 换 batch~~ → **实测只省 1.2 G**（[§1b.1](#1b1-hbm-用量一直印在日志里我却一直没读)），换不到 3 档 batch，此路不通 |
...
拿到 R16 的 −16.44% 之后，我做了一轮纯理论分析，得出三条推测。
随后去查上游资料，**三条全错**。这段留着，因为错法很典型。

| # | 我的推测 | 依据 | 真相 |
|---|---|---|---|
| 1 | `tile_m` 应随分组数缩放（192 组该比 128 组用更小的 m） | 组边界浪费 ∝ `g × tile_m / 2m` | ❌ **完全无关**。上游启发式只看 `m` 和 VMEM 余量，搜索空间里根本没有 `num_groups` 这一维 |
| 2 | 慢是因为"收缩维不分块"（`tile_k=4096`） | 跟 [§2.5](#25-预测又错了连结构也不跨代) 官方模式 −17.33% 数字接近，猜同源 | ❌ **前向最优仍然是 `tile_k=4096` 不分块**。真正该改的是 `tile_n` 和 `tile_m` |
| 3 | 该往 512 靠（v5p 实测偏好 512） | [§2.2](#22-最典型的一条moe-tile-完全反序) 全 512 是唯一正收益 | ❌ v5p 上最优 `tile_m = 128` |

**推测 1 错得最有代表性**：我算的那个组边界浪费公式本身没错，
128 组是 3.13%、192 组是 4.69%——**但增量只有 1.56 个百分点，跟实测的 16.44% 差一个数量级**。
我当时已经算出这个数量级不匹配了，却还是把它写成了"大概率原因"。

> **数量级对不上的时候，那就不是原因。** 别因为"能想到的只有这个"就把它当解释。

**推测 3 的错法则是过度归纳**：`wi_tile_*` 那套参数（v5p 上 512 最优）
走的是 **megablox** 路径，而 tokamax **根本不读这些参数**
（调用处不传 `tiling`）。两套 tile 是完全独立的东西，
我把一套的经验搬到另一套上——这跟本文档反复在犯的"跨代搬结论"是同一类错误，
只是这次跨的不是硬件代次，而是 **kernel 实现**。

#### 真正的答案：v5p 的调优值一直存在，只是没进公开包

上游针对 v5p 跑过完整的 ragged_dot 调优（104 条实测记录），
但**这份数据没有随公开发行的包一起发布**——我们装的包里
`data/autotuning/` 只有 `tpu7x` 和 `tpu_v5_lite`(v5e)。

拿到真值后对照，我借用的 v7x 配置错得相当离谱：

| 通路 | 借用的 v7x 值 | **v5p 实测值** |
|---|---|---|
| fwd wi | (256, 4096, **1536**) | (**128**, 4096, **768**) |
| fwd wo | (**512**, 1536, 1536) | (**128**, 1536, **2048**) |
| dlhs wi | (**1024**, 1536, 1024) | (**128**, **2048**, 1536) |
| dlhs wo | (**1024**, 1024, 1536) | (**128**, **768**, 4096) |
| drhs wi | （无对应条目） | (256, 1024, 1536) |
| drhs wo | （无对应条目） | (256, 1536, 1024) |

**反向通路差得最狠：`tile_m` 1024 vs 128，差 8 倍。**
而且我原来的补丁只镜像了 fwd/dlhs，**drhs 那两条压根没有 128 组的条目可借**，
一直在用默认的 128³。

> **教训（跟 [§2.3](#23-所以xla--kernel-层的结论天生绑硬件代次) 同源但更具体）**：
> 借用别的硬件代次的 kernel 调优值，**不是"次优"，是"可能完全错"**。
> 借之前先确认：你这代硬件的真值是不是只是"没发到你手上"，而不是"不存在"。

#### R18 实测：用"对的"硬件代次的值，反而更慢

**77.813 s，−23.13%。比借用 v7x 值的 R16（73.590 s）还差 5.7%。**

| | step (s) | TFLOP/s/dev | MFU | Δ vs 基线 |
|---|---|---|---|---|
| 基线 megablox | **63.199** | 160.9 | 35.05% | — |
| R16 借用 v7x tile | 73.590 | 138.2 | 30.11% | −16.44% |
| **R18 v5p 实测 tile** | **77.813** | 130.7 | 28.47% | **−23.13%** |

（R18 稳态 9 步落在 77.812–77.826，抖动 < 0.02%，数据质量没问题。）

**为什么"对的"值反而更差**：那份 v5p 数据是给 **128 个专家**调的。
查找表的键里本来就带分组数——**说明上游自己也认为分组数会影响最优 tile**。
两组值都不是为 192 组调的，**谁更接近纯属运气**。

顺带修正上一节的一处理解偏差：

> 我说过"上游启发式只看 m 和 VMEM，与分组数无关"——这句话只对**兜底启发式**成立，
> 那是 LUT miss 时的粗糙 fallback。**真正调出来的表是按含分组数的键存的。**
> 所以"分组数影响最优 tile"本身没错，我推测 1 错在**方向**：
> 我以为组变多 tile 该变小，实测是**更小的 tile 更慢**（128 < 256）。

#### 这条线到此为止：不再调 tile

三轮下来（默认 128³ / v7x 借用 / v5p 借用），tokamax 最好成绩 **73.590 s**，
基线 **63.199 s**。**这条 kernel 路径在 v5p + 192 专家上就是负收益，继续调只是少亏。**

停手的三条依据：

1. 上游自己追踪的 tokamax/megablox 差距是 **0.79×**，我们已经在 **0.859×**——**比上游基准还好**。剩余空间小。
2. 真调优（`tokamax.autotune`）要几小时，而天花板被第 1 条压住。
3. **打通 tokamax 的价值从来不在它本身快**，而在于它是 `use_gmm_v2` → `num_moe_emb_chunks` 的强制前置。

所以下一步直接跳到 R19 去测那条功能族——**但那条路有个自带的坑，见下**。
...
`num_moe_emb_chunks` **强制要求 `use_ring_of_experts=True`**，
而 ring of experts 在 v5p 上是 [§2.1](#21-五项判决) 里**本轮最大负收益（−21.48%）**。

也就是说：emb 分块必须先把这 21.48% 挣回来，才谈得上净收益。
v7 上 ring 是靠专家并行场景赚回来的，而本轮按需求**不开 EP**。

**仍然值得跑一次**：tokamax 现在正好通着，这是唯一的测试窗口；
即使结果是负的，也能把"这条藏通信的功能族在 v5p 上到底行不行"这个空白填掉。

#### R19 实测 −45.27%，但这个数字有个方法论缺陷

**91.810 s，−45.27%，MFU 从 35.05% 掉到 24.13%。** 本轮最差。

| | step (s) | MFU | Δ |
|---|---|---|---|
| 基线 | 63.199 | 35.05% | — |
| 单开 ring of experts | ~76.8 | — | −21.48% |
| **R19（ring + gmm_v2 + emb 分块 4）** | **91.810** | **24.13%** | **−45.27%** |

> ⚠️ **这一轮违反了 [§3.1](#31-五条方法论沿用-v7-的教训不重新踩) 第 1 条「每项只动一个维度」——
> 我一口气开了三个开关，所以 −45.27% 无法归因到具体哪一个。**
>
> 为什么会这样：我脑子里那条线是「打通 tokamax 是为了解锁 emb 分块」，
> 于是直奔终点，**把 `use_gmm_v2` 当成了"必经的管道"而不是"要独立测量的变量"**。
> 目的性挤掉了方法论。
>
> 代价不只是少一个数据点：**如果 gmm_v2 单开是正收益，那这 −45% 就全是
> ring 和 emb 分块的锅，结论完全不同。** 三变量捆一起测，
> 等于把一个可能的正收益埋在两个负收益底下。补测见下。

#### 一个更早就该发现的事实：gmm_v2 根本不查 LUT

补测 gmm_v2 时才注意到，MaxText 里这两条是**不同的代码路径**：

```python
if self.config.quantization or self.config.use_gmm_v2:
    output = mblx.gmm(..., tiling=tiling, use_tokamax_backend=True, use_gmm_v2=True, ...)
else:  # 纯 tokamax
    output = tokamax.ragged_dot(...)   # 没有 tiling 参数
```

**开了 `use_gmm_v2` 就走上面那条，`tiling` 是显式传进去的，压根不查那张查找表。**

也就是说 [R16](#r16-实测不挂了但还是比现有方案慢) / [R18](#r18-实测用对的硬件代次的值反而更慢)
两轮跟 LUT 的搏斗——镜像条目、比较 v7x 和 v5p 哪组值更好——
**只要一开始就开 `use_gmm_v2`，全都不必做。**

> **教训**：遇到"库内部查表查不到"这类问题时，
> **先确认有没有一条"把参数显式传进去"的旁路**，再决定要不要去改库里的表。
> 我直接冲着改表去了，因为那是第一个看懂的机制。

前置关系也澄清一下（之前一直没查清）：

| 开关 | 前置 |
|---|---|
| `use_gmm_v2` | 只需 `use_tokamax_gmm=True`。**不需要 ring** |
| `num_moe_emb_chunks > 0` | 需要 `use_gmm_v2` **且** `use_ring_of_experts` |
| `use_gmm_v2` + 量化 | **互斥**，所以 gmm_v2 和 FP8 只能分开测 |

补测队列：`gmm_v2@tile1024` → `gmm_v2@tile512` → `FP8@tile512`。

#### 另外两条值得记的上游事实

1. **`tokamax` 和 `megablox` 的性能差距是已知的、上游在追的**：
   最近一次记录是前向 0.93×、反向 0.72×、**合计 0.79×**。
   我们实测 `63.199 / 73.590 = 0.859×`——**比上游基准还好一点**。
   所以 R16 那 −16.44% **不是我们配错了**，是这条 kernel 路径当前的真实水平。
2. **LUT miss 之后本该有启发式兜底，那个功能至今是 TODO**，
   所以只能返回固定的 `Config(128,128,128)`。
   上游较新的版本已经把硬编码 LUT 换成了「per-chip JSON + VMEM 感知启发式」，
   **升级 kernel 库是这个问题的根本解**，改 LUT 只是本地绕行。
...
| # | 项 | 状态 |
|---|---|---|
| **R8** | **`sa_use_fused_bwd_kernel=True`**（v5p 当前 `False`，v7 是 `True`） | ✅ **61.094 s / +3.33%** —— 全文档第一个正收益 |
| **A1** | `use_tokamax_splash=True`（v7 一直开，v5p 从没开过） | ❌ **68.956 s / −9.11%**。我押它是因为"attention 侧刚证明有空间"——**这个推论不成立**，R8 那 +3.33% 是那一个开关的功劳，不是那一侧有富矿。跟[§7.9 的错法](TUNING-v7.md#79-一条方向性教训占比大--有空间)同型，只是方向反过来 |
| **A2** | **`sa_use_fused_bwd_kernel` + tile 512** | ✅ **59.400 s / +6.01% / MFU 37.30%** ← **当前最优**（[§1b](#1b-目前的最优配置601)）。3.33+2.67 完美线性叠加 |
| **C2** | tile 512 同批次复核 | ✅ **61.510 s / +2.67%**，vs 早期 61.496 s 差 0.02%，站得住 |
| **B1** | FP8 `fp8_full`+qwix，tile **512 / 256 / 1536** 三档 | ❌ 三档**报同一个错**：`Mosaic failed to compile: infer-vector-layout: unsupported shape cast` (`gmm.26`)。**所以不是 tile 除不尽的问题**，是 qwix 量化路径在 v5p 的 Mosaic 后端上跟 tile 无关的 bug。换 tile 这条路已堵死 |
...
| **R18** | tokamax + **v5p 实测 tile**（取代借用的 v7x 值） | ✅ **77.813 s / −23.13%** —— 比借用 v7x 的还差，那组值是给 128 专家调的（[§2.6](#26-打开盒子不跨代的机理落到了一张查找表上)） |
| R17 | 对 192 分组真跑一次 `tokamax.autotune` | ❌ **取消** —— 三轮 tile 实测（默认/v7x/v5p）最好仍 −16.44%；上游 parity 0.79× 而我们已在 0.859×，天花板压死 |
| **R19** | tokamax + `use_gmm_v2` + `num_moe_emb_chunks=4` | ✅ **91.810 s / −45.27%**（MFU 24.13%）本轮最差。⚠️ **三变量同开，违反单变量原则，无法归因** |
| **E1** | `use_gmm_v2` 单开（tile 1024） | 🔄 补 R19 的单变量对照。gmm_v2 走 `mblx.gmm` 显式传 tiling，**不查 LUT** |
| **E2** | `use_gmm_v2` + tile 512 | ⬜ 配上 v5p 最优 tile |
| **P1** | `fp8_full` + qwix + tile 512 | ⬜ **计算侧最大杠杆**（算力翻倍）。与 gmm_v2 互斥须分开测 |
| P2 | 优化器 BF16 + `per_device_batch_size=12` | ⬜ 用省出的 HBM 换吞吐 |
| P3 | `sa_use_fused_bwd_kernel=True` | ⬜ 减少 kernel 数，纯计算侧 |
| **R11** | 加大 `per_device_batch_size` | ⚠️ **9 能跑但不划算**：66.556 s，MFU 37.45%（全场最高）、TFLOP/s 171.9（全场最高），但 step 拉长 12.05%。换算真实吞吐 `9/8 ÷ 1.1205 = 1.0040`，**净收益 0.4%，在抖动内**。10 / 12 被编译期估算挡回（[§1b.1](#1b1-hbm-用量一直印在日志里我却一直没读)） |
| **R12** | `--xla_tpu_pcie_bandwidth_multiplier` 0.03 → 1.0 | ⚪ **59.359 s / +6.08%**，vs 最优 59.400 差 **0.07%**，在抖动内 → **无效**。那个可疑的 0.03 动它没用 |
| **E1–E3** | `mlpwi=offload mlpwo=offload`（+ batch 10 / 12） | 🔄 跑测中。**思路变了**：不再一档档试 batch，而是先量「多 offload 两个 MoE 张量能省多少 HBM」，再据此算能开几档 |
