...
| 🔬 **FP8（未调优）** | `DP=2 × FSDP=256` | 16 | 29.46 s | 618 | 13.39%<sub>对 FP8 峰值 4614</sub> | 1,139,022 | 4,449 | 92.80 G |

> **FP8 那一行是「跑通了但一格没调」的状态，别拿它跟上面几行的 BF16 数字直接比。**
> 它对 FP8 峰值的 MFU 只有 13.4%，DSV3 官方同口径是 16.1%（743.5 TFLOP/s/chip），**我们落后 20.3%**。
> 原因是 **FP8 走的是另一条 GMM kernel 路径，那条路上的 tile 一次都没扫过** ——
> 而 tile 在 BF16 路径上值 +17.4%。潜力估算约 726。
> 详见 [TUNING-v7 §5](TUNING-v7.md#5-fp8-与-qag能拿的已经拿到剩下的要改模型)。
...
| **FP8 待调优** | 256 芯片实测 **618**（对 FP8 峰值 4614 是 MFU **13.4%**；+3.2% vs BF16），DSV3 同口径是 743.5 / 16.1%。**那条 kernel 路径的 tile 一次都没扫过**，潜力约 726。见 [TUNING-v7 §5](TUNING-v7.md#5-fp8-与-qag能拿的已经拿到剩下的要改模型) |
