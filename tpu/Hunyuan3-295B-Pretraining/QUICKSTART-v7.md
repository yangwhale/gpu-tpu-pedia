...
### 64 芯片 FP8 → 1,014.8（✅ 已验证，可用）

> [!note] 2026-08-15 22:40：正确性疑点已排除
> 曾怀疑 native 路径「每卡只拿 3 个专家 = 算少了」。
> **32 步 loss 逐步对拍推翻了这个怀疑**：第 0 步与 tokamax 逐位相同（13.423），
> 第 31 步差 0.004（10.941 vs 10.945），中段最大偏离 0.197 后重新收敛 ——
> 真少算 64 倍会单调发散，不会先分开再合回来。那个 `3` 和 `192`
> 连维序都不一样（`[3,4096,1536]` vs `[192,1536,4096]`），本来就不是同一个张量。
> 同配置下 native 还快 **50%**（7.84 s/step vs 11.78）。
> 完整证据见 [TUNING-v7 §3.4.8](TUNING-v7.md)。
