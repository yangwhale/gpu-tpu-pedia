...
HERE = P.HERE
WEB = P.WEB
OUT = os.path.join(WEB, "topic-03-L300.html")

# ⭐ head / 图装配 / 锚点 / 吸顶目录都搬去 topic03_page.py 了（2026-09-14 R62）——
#   ⛔ 那是**两页共用**的一份，别在这儿再抄一份回来。理由见那个文件的头。
head = P.make_head(
    "专题三 · 注意力演进（L300 · 完整版）",
    "注意力演进 · 从 RNN 的一个补丁，到今天各家的混合配比",
    "名词多到像各搞各的，但只有三个旋钮可以拧 —— MLA、GQA、SWA、DSA、GDN、KDA 全部收进同一把尺子，每一条都回到一手 config 核过。",
    "topic-03-L300.html")
...
<!-- ⭐ 2026-09-15 现场提问补。L200 那边写了完整版，这里给**压缩版**：
     L300 的读者多半知道 RNN 是什么，但**解剖学来历**和**递归/循环的中文误译**
     这两条，知道的人不多，而且后一条会真的让人读错文献。 -->
<div class="note info"><p>📌 <b>把名字拆开：RNN ＝ <u>R</u>ecurrent <u>N</u>eural
  <u>N</u>etwork ＝ <b>循环</b>神经网络。</b></p>
<p><em>R 指的是<b>状态回到自己输入端</b>的那条边（recurrent connection）。
  词源是拉丁 <b>recurrere</b> ＝ re（回）＋ currere（跑）。</em></p>
<p>⭐ <b>这个词是从解剖学借的，不是数学造的</b> ——&nbsp;
  <em>解剖学里它一直用来描述环状结构：Cajal 1901 在小脑皮层观察到
  "recurrent semicircles"，Lorente de Nó 1933 用高尔基染色发现
  "recurrent, reciprocal connections"。<b>在那之前神经系统被当成纯前馈的</b>，
  1940 年代才有人正经提出脑内存在反馈。</em>
  <b>而 <a href="#s七">§七</a> 末尾 Albert Gu 那句「像一个大脑」，正好把这个词还回去了。</b></p>
<p>⛔ <b>中文误译提醒</b>：<em>「递归神经网络」指的是
  <b>Recursive</b> Neural Network（沿语法树递归），<b>是另一类模型</b>。
  读中文文献时这两个词经常被混用，看到「递归」先确认原文是哪一个。</em></p></div>
...
<p>📌 MLA 那条另有一处麻烦：RoPE 必须单独走一路，
  因为上投影吸收不了带位置旋转的那几维（见本节前面那段）。</p></div>
...
<div class="note danger"><p>⛔ <b>但代价也写在同一格里。</b>
  ...
  分块并行（chunkwise）就是为了把并行度再找回来（见 §7.4）。</p>
...
<div class="note warn"><p>⛔ <b>这里原先还列了第三条「KV cache 最多降 75%」——&nbsp;那一条不是 NoPE 的功劳。</b>
  ...
  <em>⭐ 这个错误值得留在页面上：<b>本节那张配比图</b>的 ⚠️ 注早就写对了，
...
<!-- ⛔ 这个文件由 Courses/tools/topic03-build-L300.py 生成。
...
<div class="wrap" style="padding:32px 0 64px">
  <p style="color:var(--gray)">
    ...
    <a href="topic-03-L300-lecture.html">📝 讲义</a></p>
  <p style="color:var(--gray);font-size:13px">
    本页由 <code>Courses/tools/topic03-build-L300.py</code> 生成 ——&nbsp;
...
FIGS = {
    ...
    "__FIG_HYBRID__": ("fig-hybrid", "fig3-hybrid.svg", 'topic03-fig-hybrid.py',
        ...
        '反而表现不好 —— 加线性层不只是省钱。'
        '那张消融表长什么样、它又<b>问不出</b>什么，见 §8.2。</em>'),
    ...
    "__FIG_COPY_MATRIX__": ("fig-copy-matrix", "fig3-copy-matrix.svg",
        ...
        '所以必须接 §5.4b 的「吸收」。这张图是它的前提，不是替代。</em>'),
...
P.finish(_html, OUT, SECTIONS, "topic-03-L300.html")
