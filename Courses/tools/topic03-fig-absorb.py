...
⭐ 它其实就是本讲那个装置**第一次真的出场**：把括号挪个位置。
   ⛔ 2026-09-14 改：原来这里写「第三次出场」，错的 ——&nbsp;
   **挪括号一共只有两次**（这张图、线性注意力那一章）。
   第二章「点积赢加性」换的是打分函数，跟结合律无关，不算这条线。
...
def main():
    ...
    # ⛔⛔ 2026-09-14 R15：标题原来写「第三次出场」，而**它底下只列了两条**，
    #    最后一行还写着「两次都是同一个数学恒等式」——&#160;**图跟自己打架**。
    #    更麻烦的是图外面：正文那边写「第二次见到了」，第七章写「第三次」。
    # ⭐ 核完的事实：**挪括号（结合律）一共只有两次** ——&#160;MLA 吸收、线性注意力。
    #    第二章那次（点积赢加性）**不是挪括号**：它换的是打分函数，跟结合律无关。
    #    那一次属于另一条线（硬件决定公式），已在正文里归位。
    # ⭐⭐ 判据：**一条靠计数成立的暗线，作者自己先得数得准。**
    #    数不准的时候，通常不是漏了一次，是**把不同类的东西算进了同一条线**。
    yy = f.band(yy + 14, "info", "同一个把戏，本讲一共只挪两次括号 ——　这是第一次", [
        "<tspan font-weight=\"700\">这一次 ——&#160;MLA 的吸收</tspan>（这张图）："
        "把括号从 qᵀ(W_UK c) 挪成 (W_UKᵀ q)ᵀ c ——&#160;压缩包就不用拆了。",
        "<tspan font-weight=\"700\">还有一次，在线性注意力那一章</tspan>："
        "把括号从 (QKᵀ)V 挪成 Q(KᵀV) ——&#160;那个句长×句长的大方块就不用建了。"
        "<tspan font-weight=\"700\">那一次更赚。</tspan>",
