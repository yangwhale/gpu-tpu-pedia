...
def main():
    ...
    # ⛔⛔⛔ 2026-09-16 现场问出来的**真错**：「那个 128 倍压缩，真的是从那
    #   4 倍压缩再压 32 倍来的吗？是分两级压缩的吗？」——&#160;**不是。**
    #   ⭐ 扒了 MaxText 源码核实：HCA 的压缩器签名是
    #     `__call__(hidden_states: [batch, seq_len, emb_dim], ...)` ——&#160;
    #     它拿的是**这一层的原始输入**，reshape 成 `(batch, n, 128, -1)`
    #     然后**一次** softmax 门控求和。**一步压到 128，没有中间的 4。**
    #   ⛔ 而且 CSA 和 HCA **根本不是同一层**：它们是两种层型，在网络里交替排
    #     （V4-Pro 的 compress_ratios 是 [128,128,4,128,4,…]）。
    #     **每一层都直接从 token 压，不存在谁接谁的力。**
    #   ⚠️ 原来那个肘形箭头 ＋「再压 32 倍」，把「128 ÷ 4 ＝ 32」这个**比值**
    #     说成了一个**机制**。128 确实是 4 的 32 倍，但那只是两个独立参数的商。
    #   ⭐⭐ 判据（CLAUDE.md 第一原则的又一次应验）：
    #     **两个数成整倍数关系，不等于它们之间有因果或先后。**
    #     箭头是最容易把「比值」偷换成「流程」的图元 ——&#160;
    #     画箭头之前先问一句：**这条箭头上真的有东西在流吗？**
    # ⭐⭐ 第二版：**连箭头一起去掉。**
    #   第一版只改了标注文字，箭头还在 ——&#160;渲染出来那两行字正好压在
    #   箭头的竖段上，而且**箭头本身就是病根**：它是把「比值」偷换成「流程」
    #   的那个图元。判据自己说得很清楚：**画箭头之前先问一句，
    #   这条箭头上真的有东西在流吗？** 这里的答案是没有 ——&#160;那就别画。
    #   ⭐ 澄清整段搬进落点带（那里是要人读的地方），图上只留一行短的。
    # ⛔ 第三版：这行短标注也去掉了 ——&#160;它撞在 HCA 的出处行上
    #   （py2+346，只差 2px）。⭐ 这一格已经没有地方放它了，而澄清整段本来
    #   就在落点带里写全了。**要人读的写成字，别硬往图上挤。**
    f._pan = None
    ...
    yy = f.band(py2 + PH2 + 22, "ok", "这张图真正要说的一句话", [
        ...
        "⛔⛔ <tspan font-weight=\"700\">但它们不是一条流水线</tspan> ——&#160;"
        "CSA 和 HCA 是<tspan font-weight=\"700\">两种不同的层，在网络里交替排</tspan>，"
        "<tspan font-weight=\"700\">每一层都直接从原始 token 压</tspan>。"
        "128 是 4 的 32 倍，可那只是两个参数的商，<tspan font-weight=\"700\">没有谁接谁的力</tspan>。",
