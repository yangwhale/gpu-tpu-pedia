...
| 指标 | 5×H800（官方宣称） | 5×B200（我们实测） | TPU v6e |
|---|---|---|---|
| ★ 生成 1 s 视频耗时 | **0.556 s** | **0.604 s** | 待测 |
| ★ 实时倍数 | 1.80× | **1.66×** | 待测 |
| 每块周期（12 帧 = 0.48 s 视频） | 0.267 s | 0.290 s | 待测 |

**全部是纯生成时间，启动/加载/编译不计** —— 常驻服务只在部署时付一次。

**同样 5 张卡，B200 比 H800 慢 8.7%** —— 纸面算力 2.3 倍、带宽 2.4 倍，却跑输了。
原因是瓶颈落在不吃 tensor core 的 elementwise 碎片上（详见基线页第四节）。

成功档位：**能用** ≥1.0× ｜ **追平 B200** ≥1.66× ｜ **追平 H800** ≥1.80× ｜ **超过** ≥2.33×。
