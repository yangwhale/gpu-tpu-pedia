...
> 🆕 **2026-08-03 单节点实测拿到第一条正收益**：给 tokamax 显式指定
> `tile(512, 1024, 1536)`，比 megablox **+4.0 %**（189.5 vs 182.0 TFLOP/s/device）。
> 复现步骤 + 多卡复测清单见 **[§7.8.1](#781-v7-单节点实测总表2026-08-034-芯片--8-devices)**。
...
#### 7.7.2 根因（v5p 视角，**已被 §7.8.1 补充修正**）

> ⚠️ **本小节只描述了两套查表机制中的第一套。**
...
|---|---|---|---|
| megablox 基线 | 1.321 s | 182.0 | — |
| **tokamax `tile(512, 2048, 1536)`** | **1.220 s** | **197.2** | **+8.4 %** 🏆 |
| tokamax `tile(256, 2048, 1536)` | 1.219 s | 197.2 | +8.4 %（并列） |
| tokamax `tile(512, 1024, 1536)`（抄表值） | 1.269 s | 189.4 | +4.0 % |
| tokamax `tile(128, 4096, 1536)` | 1.451 s | 165.7 | −9.0 % |
| tokamax 默认（回退 `128³`） | 17.955 s | 13.4 | −93 % |

**未命中调优表的代价 = 12.4×**（17.955 → 1.451 s，其余参数不变）。

#### 完整 tile 扫描（15 组，`tile_n` 只取 1536 的因数）

**第一步：固定 `tile_n=1536`，扫 `tile_m × tile_k`**
...
**第二步：固定最优 `tile_m=512, tile_k=2048`，扫 `tile_n`**
...
**三条规律**：
1. **`tile_k=2048` 是甜点** —— 不是越大越好（4096 直接 OOM），也不是抄表的 1024。
2. **`tile_n` 取满 n（1536）最好**，越小越差，单调。
3. **`tile_m` 256/512 无差别，1024 变差** —— 与「`tile_m` 随 `m` 线性走」的表内规律**不完全一致**，
   实测该取 512 而非表里 `m=131072` 对应的值。**抄表是好起点，不是终点。**
...
def _patched(self, ba):
    ...
    return dataclasses.replace(c, tile_m=_TM,
                               tile_k=min(_TK, k), tile_n=min(_TN, n))
...
#### ⛔ 死路 4：FP8 在最优 tile 上仍是负收益

之前 §7.8.2 死路 3 里 V8（FP8 + `gmm_v2` + tile512）是 −3.53 %，
我一度归因为「tile 不对」。**在全局最优 tile 上重测，依然是负的：**

| 配置 | step | TFLOP/s/dev | HBM | vs 最优 BF16 |
|---|---|---|---|---|
| tokamax `tile(512,2048,1536)` BF16 | 1.220 s | 197.2 | 69.62 G | — |
| 同上 + **FP8**（`fp8_full` + qwix） | 1.234 s | 194.9 | 74.55 G | **−1.2 %** |

**结论：FP8 慢不是 tile 的锅。** v7 的 FP8 峰值是 BF16 的 2×，但量化/反量化开销
在这个规模上吃掉了全部收益，还多花 4.9 G HBM。
**要用 FP8 必须先查清开销来源**（大概率是 per-tensor scale 的额外 pass），
不能指望「开了就快」。

#### ⛔ 死路 5：官方专家维分片消不掉 `gmm_v2` 的 copy 惩罚

死路 2 查到 `gmm_v2` 的 layout 惩罚后，我推论：
官方 `shard_exp_on_fsdp=True` 让 emb 维不分片，**可能正好让 layout 对上**。

**实测证伪：**

| 配置 | step | TFLOP/s/dev | vs 最优 |
|---|---|---|---|
| tokamax `tile(512,2048,1536)` | 1.220 s | 197.2 | — |
| **`gmm_v2` + 官方分片 + tile512** | 1.291 s | 186.2 | **−5.6 %** |
| tokamax + 官方分片（对照） | 1.220 s | 197.1 | 0.0 % |

**`gmm_v2` 那条路彻底判死**：换分片方式救不回来，`operand_layout_constraints`
是写死在 custom-call 上的，跟权重怎么分片无关。

> 又一次「机理听起来合理 → 实测证伪」。这次我先做了对照组（tokamax + 同样分片 = 0.0 %），
> 所以能干净地把 −5.6 % 归到 `gmm_v2` 头上，而不是分片头上。

#### ⛔ 死路 6：`shard_exp_on_fsdp` 在单节点上恒为 0.0 %（三次独立测量）

| 配对 | 不开 | 开 | Δ |
|---|---|---|---|
| megablox | 1.321 s / 182.0 | 1.322 s / 181.8 | 0.0 % |
| megablox（同批复现） | 1.323 s / 181.8 | — | — |
| tokamax + 最优 tile | 1.220 s / 197.2 | 1.220 s / 197.1 | 0.0 % |

**不是开关没生效**（config 日志确认 `True`/`False`，代码走
`moe.py:443` 的 `wi_kernel_axes = ("embed_moe", None, "mlp_moe")` 分支）。

**是两种切法每卡字节数完全相同**：
`192 experts × (4096/8) emb` **=** `(192/8) experts × 4096 emb`。
单节点 all-gather 走 ICI，量也一样 → 物理上就没有差别。
**要看出差别必须跨机 DCN。** → M2。
...
> | | 每卡持有 | 通信 |
> |---|---|---|
> | 我们默认 | 全部 192 专家，**每个被劈成 1/8**（emb 维） | all-gather 拼 emb |
> | 官方 `shard_exp_on_fsdp` | **24 个完整专家** | all-gather 聚专家 |
> | 真 EP（`ici_expert_parallelism`） | 部分专家，**不 all-gather** | **AllToAll 搬 token** |
>
> 官方那个**不是 EP**，是 FSDP 换了个切分轴。真 EP 我们在 16 芯片测过：**−71.36 %**（见 §7.4）。
