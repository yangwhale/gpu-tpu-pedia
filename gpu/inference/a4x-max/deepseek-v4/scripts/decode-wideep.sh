#!/bin/bash
# Wide-EP decode（对齐 SemiAnalysis InferenceX 官方 8K/1K GB300 recipe）
# 用法: bash decode-wideep.sh <node_rank> <dist_addr> <ep_size> <nnodes>
#   dep16: bash decode-wideep.sh $R $D0:5000 16 4
#   dep32: bash decode-wideep.sh $R $D0:5000 32 8
# 与 decode-dep8.sh 的差异（依据官方 recipe，见 runbook §11）：
#   - EP 加宽到 16/32（官方 frontier；dep8 已被 PR #1586 判为 dominated 删除）
#   - 不开 MTP（官方 8K/1K wide-EP recipe 无 speculative 配置）
#   - mem-fraction 0.94 / max-running 18432 / swa 0.20
#   - NUM_MAX_TOKENS_PER_RANK 随 EP 宽度调（dep32 用 1280）
NODE_RANK=$1; DIST_ADDR=$2; EP=${3:-16}; NN=${4:-4}
[ "$EP" -ge 32 ] && TPR=1280 || TPR=2560
...
export SGLANG_OPT_DEEPGEMM_MEGA_MOE_NUM_MAX_TOKENS_PER_RANK=$TPR SGLANG_OPT_DEEPGEMM_MEGA_MOE_USE_FP4_ACTS=1 SGLANG_OPT_DEEPGEMM_MEGA_MOE_USE_MXF4_KIND=1
export SGLANG_OPT_USE_CUSTOM_ALL_REDUCE_V2=0
export SGLANG_OPT_USE_ONLINE_COMPRESS=1 SGLANG_CLIP_MAX_NEW_TOKENS_ESTIMATION=8
...
python3 -m dynamo.sglang --model-path /mnt/ssd/DeepSeek-V4-Pro --served-model-name deepseek-ai/DeepSeek-V4-Pro \
  ...
  --tensor-parallel-size $EP --data-parallel-size $EP --expert-parallel-size $EP --pp-size 1 \
  --nnodes $NN --node-rank $NODE_RANK --dist-init-addr $DIST_ADDR \
  ...
  --mem-fraction-static 0.94 --swa-full-tokens-ratio 0.20 --context-length 9216 \
  --max-running-requests 18432 --cuda-graph-max-bs 1280 --stream-interval 60 --disable-radix-cache --enable-metrics
