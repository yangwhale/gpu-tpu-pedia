...
def main():
    ...
    # ══════════ ① 三招：保温台上的预制套餐 ══════════════════════
    PH = 422   # ⚠️ +50：三招行距 44→52（塞形状），成绩条再让 22
    ...
    # ⭐ R39：这个框原来是三行**断言共同点的文字**。现在它是下面那个形状的
    #   **图例** ——&#160;带标注的那一份在这儿，下面三行是它的无标注复制品。
    #   ⭐⭐ 「图例 ＋ 若干实例」这个结构本身就在说「它们是同一个东西」。
    LEGX, LEGY = 920, 0          # LEGY 在 shape() 定义之后才知道，占位

    # ⭐⭐ 2026-09-14 R39：这一格 2026-09-13 已经因为「就是一块写字板」重画过一次，
    #   可**重画只救了第三招** ——&#160;保温台画的是「预制套餐」，
    #   另外两招还是纯文字，而标题声称的是「三招**都是**同一个形状」。
    # ⛔ 更要命的是：那个「共同形状」是一个**写着字的框**。
    #   用文字断言共同点，恰恰是这张图想替代的东西。
    # ⭐⭐ 判据（R38b 在 fig3-nope 上立的，这里是第二次用）：
    #    **标题里出现连接词（都是 / 一起 / 同时），图上就得有对应的图形连接物。**
    # ⭐ 装置：**把同一个形状连画三遍。** 重复本身就是「都是」的证据 ——&#160;
    #   三行的形状一模一样，不一样的只有右边的文字。
    #   形状取正文登记表那条更准的口径：**一个动态 → 一批静态，来了挑一个。**
    def shape(x, cy):
        """运行时才知道的一个 → 开工前备好的几个。三行共用，一个像素都不许差。"""
        f.box(x, cy - 18, 48, 36, "#fff", GY2, 6, 1.4, dash="5 4")
        f.t(x + 24, cy + 7, "?", GY2, True, 20, "middle")
        f.line(x + 54, cy, x + 74, cy, GY2, 1.4)
        for k in range(3):
            on = k == 1
            f.box(x + 82 + k * 36, cy - 15, 30, 30,
                  "#e6f4ea" if on else "#fff", GR if on else LINE2,
                  5, 2.2 if on else 1)

    # ── 图例：带标注的那一份 ────────────────────────────────────
    f.box(896, ay + 12, 448, 116, "#fff", GR, 10)
    f.t(920, ay + 44, "⭐ 共同形状（下面三招都是它）", GR, True, 19)
    shape(LEGX, ay + 78)
    f.t(920, ay + 115, "运行时才知道的<tspan font-weight=\"700\">一个</tspan>"
        "　→　开工前就备好的<tspan font-weight=\"700\">几个</tspan>，来了挑一个",
        GY2, size=15)

    f.t(56, ay + 146, "下面三招，<tspan font-weight=\"700\">形状跟右上角那个"
        "一模一样</tspan> ——　不一样的只有右边那行字。", GY2, size=15)
    ...
    for i, (h_, d_) in enumerate(FIX):
        cy = ay + 184 + i * 52
        shape(56, cy)
        f.t(272, cy + 7, h_, GR, True, 19)
        f.t(500, cy + 7, d_, GY, size=17, w=844)
    # ⚠️ 300 时第三个形状（下沿 ay+306）压在这条上 —— 截图才看见
    f.box(56, ay + 322, 1288, 34, "#e6f4ea", GR, 8)
    f.t(76, ay + 346, "⭐ 成绩：Llama 3 8B 在 TPU7x 上 ——　"
    ...
    for k in range(5):                                     # 货架
        ...
        f.t(623 + k * 78, by + 102, "%d 号" % (k + 1), BL, size=16,
                anchor="middle")
    f.t(774, by + 150, "这道菜的料从哪个货架 ——　自己算", BL, True, 17, "middle")
    ...
    f.t(76, ey + 220, "⛔ 但要诚实：公开的那套 TPU 生产注意力 kernel 走的是"
        ...
        "（TensorCore ＋ Pallas/Mosaic），", RD, size=17, w=1248)
    # ⛔ 2026-09-15：原来这句结尾是「台下如果有 TPU 的人，含糊一句就会被抓住」——
    #    那是**写给讲师的舞台指示**，却印在读者自己看的那一页上。
    # ⭐ 判据：读者页上出现「台下 / 现场 / 念一遍」，读者会觉得自己在偷看别人的备课笔记。
    f.t(76, ey + 250, "<tspan font-weight=\"700\">不是拣货小队</tspan>。"
        "没有公开材料说有人用它跑注意力的 top-k ——　"
        "<tspan font-weight=\"700\">这一条到今天仍然只是「看起来对」。</tspan>",
        RD, size=17, w=1248)
    ...
    yy = f.band(yy, "warn", "⛔ 第三格这条要留在「看起来很对、但还没被公开验证」上", [
        ...
        "⭐ 所以这一条的准确说法是：<tspan font-weight=\"700\">硬件条件看起来对得上，"
        "但这件事还没有被公开验证过。</tspan>",
    ], fold=True)
