...
| **R18** | tokamax + **v5p 实测 tile**（取代借用的 v7x 值） | 🔄 **进行中** —— R16 借错了硬件代次的值（[§2.6](#26-打开盒子不跨代的机理落到了一张查找表上)） |
| R17 | 对 192 分组真跑一次 `tokamax.autotune` | ⬜ **降级** —— 上游已有 v5p 真值可直接用；且 tokamax/megablox 差距 0.79× 是上游已知水平，自己调优天花板有限 |
| R19 | tokamax 打通后测 `use_gmm_v2` + `num_moe_emb_chunks` | 🔜 **这才是打通 tokamax 的真正目的** —— 沿 embedding 维分块藏通信，整条功能族的前置就是 tokamax |
