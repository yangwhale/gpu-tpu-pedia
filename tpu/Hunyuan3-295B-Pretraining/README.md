...
| 稳态 step | 63.2 s | **21.7 s** | 30.4 s | — |
| **TFLOP/s / 计算单元** | 161.0 | **629.9** | **598.8** | 854.0 |
| **MFU** | **35.07%** | **27.31%** | 25.96% | 31.60% |
| **整机 token 吞吐** | 265,588 tok/s | 290,331 tok/s | **1,103,757 tok/s** | 399,488 tok/s |
| **每单元 token 吞吐** | 1,037 | **4,536** | 4,312 | 6,242 |
| 最优配方 | 官方 DSV3 v5p 配方 | `DP1×FSDP128`<br>tile + pdbs 12 + **dvfs 7** | `DP2×FSDP256`<br>tile + pdbs 16 | — |
| 状态 | ✅ 调优收敛，异地复现 | ✅ **BF16 最高，已达目标区间上沿** | ✅ 达目标线 600–630 | 已调优 |
| FP8（同硬件，**分母 4614**） | — | 🏆 **674 / MFU 14.6%**<br>**4,854 tok/s/chip**（QAG + dvfs 7） | 无 QAG 618 | DSV3 官方 743.5 / 16.1% |
...
1. **v7 每芯片吞吐约为 v5p 的 4.68 倍**（4,854 vs 1,037，⚠️ **v5p 那个数是 8K 序列下测的，此倍数为高估**），MFU 却更低（27.31% vs 35.07%）——
   因为 v7 的 BF16 峰值是 v5p 的 5.03 倍，而 HBM 带宽只涨 2.64 倍、ICI 只涨 2 倍。
   **MFU 下降但绝对吞吐大涨，是硬件比例失衡的正常表现，不是调优没做好。**
2. **v7 已追到 GB300 单卡吞吐的 77.8%**（4,854 vs 6,242），调优前只有 51.4%。
...
| **v7 FP8 + QAG** | ✅ **已收敛**。64 芯片开 QAG 后 **625**（vs 无 QAG 594，+5.3%）；256 experts 探索到 **645**。<br>⚠️ 原先记的「FP8 那条 kernel 的 tile 完全没调过、潜力 726」**已被实测证伪** —— FP8 内部仍走 tokamax，tile monkeypatch 一直生效。<br>2026-08-05 又扫了 tile / XLA flag / SparseCore 卸载 / 推 batch 共 8 格，**无一正收益**：调参空间已见底，再往上要加卡、改模型形状或写代码。见 [TUNING-v7 §4.6](TUNING-v7.md#46-什么能调什么不能调--一张总表) |
