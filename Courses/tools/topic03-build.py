...
SECTIONS = [
    ...
    ("s三", "三", "先解决一个误会：那个矩阵从来没被存下来过 —— FlashAttention"),
    ("s四", "四", "分类学：只有三个旋钮"),
    ("s五", "五", "旋钮①：每个 token 存多少"),
    ("s六", "六", "旋钮②：每个 query 看多少"),
    ("s七", "七", "旋钮③：换一套数学 —— 线性注意力"),
    ("s八", "八", "旋钮④（其实是元旋钮）：混合"),
    ("s九", "九", "代价：没有免费的午餐"),
    ("s十", "十", "落到硬件（本专题的落点）"),
...
<section id="s三"><div class="wrap"><div class="stn"><span class="badge">第 三 节</span><h2>先解决一个误会：那个矩阵从来没被存下来过 —— FlashAttention</h2></div>
...
<!-- ⛔ 2026-09-08：主线图 __FIG_TX_BASE__ 从这里挪到了 §一。
     ⭐ 理由：它画的就是「一层 Transformer 长什么样」——&nbsp;
       那是 MHA 那一节的内容，放到 FlashAttention 才第一次亮相太晚了。
       读者到这儿之前已经被要求接受「那个矩阵」好几次了，却一直没见过它长在哪。
     📌 一个占位符只能用一次，所以这里不再重复插图；
       本节用的是同一组图的 fa 变体（__FIG_TX_FA__，只点亮被 FlashAttention 动到的那处）。 -->
<h3>3.1 它跟三个旋钮不是一类东西</h3>
...
<section id="s四"><div class="wrap"><div class="stn"><span class="badge">第 四 节</span><h2>分类学：只有三个旋钮</h2></div>
...
<section id="s五"><div class="wrap"><div class="stn"><span class="badge">第 五 节</span><h2>旋钮①：每个 token 存多少</h2></div>
__FIG_TX_K1__
<h3>4.1 MHA → MQA → GQA：一个连续旋钮</h3>
...
<section id="s六"><div class="wrap"><div class="stn"><span class="badge">第 六 节</span><h2>旋钮②：每个 query 看多少</h2></div>
__FIG_TX_K2__
<h3>5.1 SWA（滑动窗口）</h3>
...
<h3>5.2 Attention sink —— 一个现象，不是一个方案</h3>
...
<h3>5.4 DSA（DeepSeek-V3.2）—— Lightning Indexer</h3>
...
<h3>5.5 ⭐ CSA + HCA（DeepSeek-V4）—— 压缩率按距离分层</h3>
...
<h3>5.5b ⭐ 第二阶段：索引本身成了开销 —— IndexShare 与 IndexCache</h3>
<p>前面四个方案（NSA / DSA / CSA+HCA）都有一个共同的零件：<b>一个决定"该看哪几块"的索引器</b>。 §5.4 讲 DSA 的时候它叫 Lightning Indexer。到这一步为止，所有心思都花在<b>让每个 query 少看几块</b>上。</p>
...
<section id="s七"><div class="wrap"><div class="stn"><span class="badge">第 七 节</span><h2>旋钮③：换一套数学 —— 线性注意力</h2></div>
__FIG_TX_K3__
<h3>6.1 基本换法</h3>
...
<h3>6.2 谱系：四步，每一步都在修上一步的一个具体毛病</h3>
<table>
...
<h3>6.2b ⭐⭐ Mamba 在哪？——&nbsp;它不是另一支，它是这一支的祖宗</h3>
...
<h3>6.3 ⚠️ 它不是「更快的 attention」，是另一个模型</h3>
...
<h3>7.2 配比：3:1 是怎么来的，以及它不是定律</h3>
<table>
...
<h3>7.3 ⭐ K3 的 NoPE —— 混合带来的一个意外红利</h3>
...
<p>三个后果，一个比一个实在：</p>
<ol><li><b>不用调 RoPE 外推。</b> 模型直接外推到 1M，不需要任何位置编码的重标定 —— 长上下文扩展里最烦人的一块调参，直接消失了</li><li><b>MLA 层在推理时可以退化成纯 MQA。</b> 位置编码没了， §4.3 里那条"不可吸收的 64 维"也就不存在了 —— <b>上投影可以完全吸收</b></li><li><b>KV cache 最多降 75%</b>（Kimi Linear 的数字）， 1M 上下文下 TPOT 从 11.48 ms 降到 1.84 ms，<b>6.3×</b></li></ol>
...
<h3>7.4 ⭐ 各家速查：你日常在用的那些模型，注意力到底是什么</h3>
...
<p>这跟"腾讯评估过线性注意力但最终没上"的说法对得上，而且是一手证据。 然后 Hy4 一步跨到全层稀疏 —— <b>它整个跳过了线性这一支。</b> 对照 §7.2 那张配比表：<b>混合不是唯一解，只是最多人选的那个解。</b></p></div>
...
<tr><td>GLM-5.2（744B）</td><td>2026-06-16</td><td>MLA ＋ DSA ＋ <b>IndexShare</b></td><td>每四个稀疏层共用一个索引器（见 §5.5b），1M 下省 <b>2.9×</b> FLOPs</td></tr>
