...
# ⛔ 2026-09-10 现场：「把这些给我折叠起来。」——&nbsp;figx-10 里那三条色带搬到这儿。
#   ⭐ 第一条（「看文件大小」不算验证）跟上面那折**完全重复**，直接删了，没搬。
#   ⚠️ 「推翻重写」那条单独成折 ——&nbsp;它是全讲最有说服力的一段，
#     搬出图 ≠ 弱化，折叠条上直接写落点，不点开也拿得到。
a(fold('⭐⭐ 三阶段那份例子的真正用途：<b>它是一个「把切口露出来」的装置</b>',
       """    <p><b>教学上</b>：让人去 <code>stage_outputs/</code> 把那两个文件打开看一眼，
      <b>比讲十分钟结构管用</b>。</p>
    <p><b>调试上</b>：全黑 / NaN / 动作快进这些问题，<b>在一体化脚本里无从下手</b>，
      三阶段能<b>逐段定位</b>。</p>
    <p><b>部署上</b>：既然只认文件不认进程，<b>三段就能跑在三台机器上</b> ——&nbsp;
      下一节讲各放哪台。</p>"""))
a(fold('⛔ <b>这张图我推翻重写过一次</b> ——&nbsp;'
       '<b>注解抵消不了图形本身的断言</b>（那个错值得讲给学员听）',
       """    <p>初版画的是「数据体积对数轴」，最粗的一根柱子标着
      <b>457 GB —— 注意力分数矩阵</b>，旁边我还很小心地注了一句「从不落地」。</p>
    <p>⛔ <b>那是错的</b>：那个矩阵<b>从来没有被物化出来过</b>。
      Flash / Splash Attention 是<b>分块算的</b>，算完即弃，
      HBM 里根本不存在这么一块。</p>
    <p>⭐ 而「从不落地」那句注解<b>并不能救它</b>：
      把一个不存在的量画成「管子最粗处」，
      <b>整张图的比例尺就锚在了虚构上</b> ——&nbsp;
      读者记住的是柱子，不是柱子旁边那行小字。
      <b>注解抵消不了图形本身的断言。</b></p>"""))
a(fold('这张图的数是从哪儿来的（含一处 <b>config 写的 ≠ 实际跑的</b>）',
       """    <p>三个文件的字节数、张量形状、dtype 与 metadata ——&nbsp;
      <b>直接解 safetensors 文件头得到</b>；文件就在本仓库
      <code>tpu/Wan2.1/generate_diffusers_torchax_staged/stage_outputs/</code> 下，
      <b>可自行复核</b>。CogVideoX 那条对照取自它同名目录下的
      <code>stage2_latents.safetensors</code>。</p>
    <p>⚠️ 官方 <code>wan_t2v_14B.py</code> 写 <code>text_len ＝ 512</code>，
      而 diffusers 这条路<b>实际存下来的文本 embedding 是 226</b> ——&nbsp;
      不影响 X-9 的结论（交叉注意力只占约 5% 算力），
      但<b>「config 写的」与「实际跑的」在这里确实不是一个数</b>。</p>"""))
a('<section id="s五">')
