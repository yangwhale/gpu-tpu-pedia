...
| 参数 | 值 |
|------|-----|
| 模型 | DeepSeek V3.2 671B |
| 架构 | MoE（256 experts, top-8 routing）+ MLA + DSA |
| 总参数量 | 671B |
...
> | 内容 | 大小 | 路径 |
> |------|------|------|
> | 模型权重 | ~700 GB | `/data/models/DeepSeek-V3.2` |
...
| `NEW_MODEL_DESIGN` | 启用 MLA 模型设计，DeepSeek V3/V3.2/R1 强制要求 | `1` | ⚠️ **必填** |
...
> 以下踩坑记录源自 DeepSeek R1 671B FP4 推理实测。由于 V3.2 与 R1 架构完全相同，
> 所有问题和解决方案均适用于 V3.2。
...
**原因**：vLLM 代码更新，MLA 架构模型（DeepSeek V3/V3.2/R1）强制要求 `NEW_MODEL_DESIGN=1` + DP attention
...
**原因**：`jax.device_put()` 内部的 pjit compiled transfer programs 缓存在进程内存中（C++ LRU cache），不会持久化到磁盘。DeepSeek V3.2/R1 的 1367 个 non-MoE tensor 只有 ~25 种 unique (shape, sharding) 组合，warm 状态下命中 cache 跳过编译。进程重启后 cache 清空，所有 25 种 shape 重新编译
