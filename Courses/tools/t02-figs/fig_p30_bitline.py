...
SQ_Y, SQ_H = PORT_Y + PORT_H + 22, 156       # ③ 为什么端口贵
...
def _square(f):
    ...
    y = para(f, 38, y + 2, 1324,
             ...
             "<b>这也把 P-28 那个「约 9 读 7 写」的口径钉住了</b>："
             "真要铺 16 套线，面积会离谱到不可能 —— "
             "<b>那是「同时访问数的上限」，不是「真的铺了 16 套」。</b>", "xs", 19)
    para(f, 38, y + 2, 1324,
         "真实设计一定会<b>分体</b>：把 64 行拆成几组，各组自己一套端口，"
         "同时访问落在不同组里就不用真的加线。"
         "<g>⚠️ 平方关系与分体都是数字设计通则；TPU 具体用什么单元、"
         "几个端口、怎么分组，公开资料没有。</g>", "xs", 19)
