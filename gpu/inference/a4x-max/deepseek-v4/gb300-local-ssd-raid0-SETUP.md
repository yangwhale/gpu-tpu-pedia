...
| **同上现象，但 `/proc/mdstat` 里 md0 存在却是 `inactive`**（2026-07-20 pool-0002 lt06 实测）| **节点残留污染（B 类：mdadm 旧 superblock）**：4 块 SSD 上有**上一次 RAID 留下的 mdadm 1.2 metadata**，开机 mdadm 自动组装时按旧 UUID 把盘**拆成两个坏数组**（`md0` 2 盘 `(S)` + `md127` 2 盘 `(S)`，全 inactive）→ DS 脚本 `grep -q md0` 判定「已存在」跳过 create → `mount` 报 `can't read superblock on /dev/md0` → 无 `set -e` 继续 → 假 `RAID_READY`（其实挂的是 256K tmpfs）| **B 类能 live 清，不用重建节点**（区别于 A 类）。检测：`grep md0 /proc/mdstat` 见 `inactive`。修（在该节点 RAID DS pod 里跑，privileged + Bidirectional 传播回 host）：`umount /mnt/disks/raid/0; mdadm --stop /dev/md0; mdadm --stop /dev/md127; for d in /dev/disk/by-id/google-local-ssd-block*; do mdadm --zero-superblock $d; done`（**关键：抹掉旧 superblock**）→ 再 `mdadm --create /dev/md0 --level=0 --force --raid-devices=4 google-local-ssd-block{0..3}` + `mkfs.ext4 -F` + `mount`。修完删该节点消费者/puller pod 让它重挂真 RAID。|
