...
**测量**（BF16，词表 120,832，8 个 token）：

| 指标 | 值 |
|---|---|
| KL(ref ‖ new) | **0.000e+00**（8 个 token 逐个为 0） |
| max\|Δ log p\| | **0.000e+00** |
| top-1 预测一致 | ✅ |
| 两份 logits 的 md5 | 相同（产出端校验） |
| 对照尺度 H_ref | 0.4992 nats |
| 两次 run 的吞吐 | 75.4 vs 5.8 TFLOP/s/device（差 13×） |

**结论**：等价。那个 `3` 和 `192` 连维序都不一样
