...
''' + sec("s三", "三", "第二刀：切权重") + '''
  ...
  <p>FSDP 这笔账很干净（⚠️ 推导，按稠密模型算）：一步搬的是前向拼一次权重、反向再拼一次、再分一次梯度，各约 2Ψ 字节，合计约 6Ψ（Ψ 还是参数个数，bf16 每个 2 字节）；
    一步算的是 6ΨT 次（专题四那条「训练每个 token 约 6 倍参数量次运算」），T 是每张卡分到的 token 数。<b>两者一除，正好等于 T</b>，跟模型多大没关系。
    换回砌墙的说法：FSDP 送的是「工具」（权重），一件工具送到，这一批每个 token 都要拿它算一遍，token 越多，同一趟送货够师傅忙得越久。</p>
__FIG_INTENSITY__
  <p>TPU v7 每芯片每秒能算 2,307 T 次（bf16）。芯片之间的 ICI 每颗芯片每秒能发出约 0.6 TB
    （常说的 1.2 TB/s，⚠️ 按推算是收发两个方向加起来的；上面搬的字节是「每卡发出」的量，只能跟发出那一半比，推算见台账）。
  ...
  <h3>3.2　TP：切进矩阵内部</h3>
  <p>张量并行把一层的权重矩阵本身切开，每张卡只存、只算其中一块。英伟达的 Megatron-LM 框架的切法里藏着一个巧思：</p>
__FIG_TP_MLP__
...
<figcaption>通信只在最后那一下；可每一层都有这一下。
  ...
  <p>TP 切不到的那几段，是逐个 token 做的小运算（归一化、dropout、残差相加），每张卡都存着一整份激活。
    Megatron 的序列并行（SP）把这几段沿序列切开（名字里有「序列」，但它是 TP 的搭档，跟第四刀切序列不是一回事，见 8.8），顺手把每次 AllReduce 拆成两半：进 TP 那块之前用 AllGather 把序列拼齐，出来时用 ReduceScatter 切回
    ——&nbsp;就是第一节那个等式，<b>通信量跟原来一样</b>，省下的是激活显存。
    （为了连 TP 块里要用的那份激活也只存一段，反向时要再拼一次，但可以跟算权重梯度叠在一起藏掉。）</p>
  ...
  <ul>
    ...
    <li>① 3,845 假设三根轴都用满、而且每根轴首尾连成环；TP 8 路只有 4 颗芯片（v7 一颗芯片算 2 个 device），用不满，实际门槛高好几倍，已经在线下了。</li>
  ...
  <p>所以 batch 小的时候多用 TP，batch 大的时候多用 FSDP，两把刀各管一边。看图就是：左边蓝线还没爬过红线，靠橙线；右边爬过去了，交给 FSDP。</p>

  <h3>3.5　PP：按层切</h3>
  <p>TP 每一层都要通信，出不了最快的那一圈互联。要横跨很多台机器、走慢线，又不想像 FSDP 那样每层搬权重，就按层切。
    流水线并行把模型按层切成几段，每张卡负责一段，段与段之间只在边界上用一对一收发传激活，
...
<figcaption>micro-batch 从 4 个加到 8 个，灰色的气泡跟着缩一半。
  <span class="sub">（14 秒无声循环，Manim 渲染。）</span></figcaption></figure>
  <p>图里一行就是一段（stage）。开头后面几段要等前面灌满，结尾前面几段要等后面排空；每一行空白的总长都一样，是 p−1 个前向加 p−1 个反向。气泡跟理想计算时间之比是 (p−1) ÷ m，
    p 是段数，m 是 micro-batch 个数。micro-batch 越多越省；可每份太小，卡就吃不饱；而且在最早的 GPipe 调度里，每一份的激活都要攒着等反向，份数越多攒得越多
...
__FIG_MOE_PARAMS__
  <p>换句话说，TP 那把刀要切的是「一个很宽的矩阵」，而 V3 里真正占地方的是「很多个窄矩阵」。
    对后者，最省事的切法不是把每一个都劈开，而是<b>把它们整个分给不同的卡</b>。图底那行就是 V3 训练的真实配置：PP16 ＋ EP64，一路 TP 都没用。</p>
...
__FIG_FOLD__
  ...
  <div class="note ok"><span class="t">一次实测：换一种切法，每张卡的吞吐翻一倍（跟调完参的 TP4 比；卡数也变了，见 7.3）</span>
    ...
    attention 权重虽然每张卡要存一份，但在 MoE 模型里只占几个百分点。注意上面的图画的是 V3（每卡 32 个专家），实测用的是 V4-Pro。<br>
...
''' + sec("s五", "五", "第四刀：切序列") + '''
  ...
  <ul>
    ...
    <li><b>推理</b>：KV cache 每个 token 都要存一份。V3 的 MLA 每 token 存一份压缩后的 KV（512 维）加一小段位置信息（64 维），(512 ＋ 64) × 61 层 × 2 字节（BF16）＝ 70,272 字节；再乘 131,072 个 token，
      <b>一个 128K 的请求就是约 8.58 GiB</b>，这还只是一个请求。</li>
  </ul>
...
__FIG_CP_ZIGZAG__
  ...
  <p>长上下文、高并发的推理（decode 阶段）里，KV 常常是最大的一块。TP 在注意力里是按头分卡的：V3 的查询头有 128 个，分得开；MLA 把 KV 压成所有头共用的一份，只剩一个头，切不开。
    一般地，TP 一旦超过 KV 头数 H，KV 就切不开了：每个头被复制 TP ÷ H 次（vLLM 文档原话：duplicated tp_size / H times）。
...
''' + sec("s七", "七", "摆到机器上") + '''
  ...
  <p>一个集群里的线不是一样快的。GPU 这边，GB300 NVL72 把 72 块卡连成一个 NVLink 域，域里每块卡 1.8 TB/s（收发合计，下同）；
    ...
    跨切片走数据中心网络，同口径比慢约 50 倍（DCN 每芯片单向 100 Gbps，折成双向约 25 GB/s；ICI 双向约 1,200 GB/s，1,200 ÷ 25 ≈ 48），比 GPU 那边的 9 倍悬殊得多。</p>
...
__FIG_FREQ__
  ...
  <p>于是有一条默认的摆法：<b>每一层都要说话的 TP、EP、FSDP、CP 先往最快的那一圈里放</b>；PP 只在段边界说话，DP 一步只说一次，它们去跨慢线。
    但光数次数不够，还要看<b>它能不能跟计算叠起来</b>。TP 叠不起来（每一块末尾那次 AllReduce 不做完，下一块就没法开始），所以必须待在快线里，度数上限就是那一圈的大小。
...
__FIG_TOPO__
  <p>TP4 decode 上能调的都调了：去掉 eager 模式（即打开 CUDA Graph）只多 2.6%，加 prefill 机器、调并发，总数从 14,563 涨到 21,100。
    ...
    真正属于「换切法」的那笔账，是 KV 不再在 4 张卡上各存一份（切序列那一节讲的毛病）；decode 从 4 张卡加到 8 张，也把专家摊薄了一半。两笔都换成了更大的 batch（并发是排队的请求数，batch 是同时在算的请求数）。
  ...
  <table>
    ...
    <tr><td>② 快线多大</td><td>256 芯片在同一个切片里，全是快线；小规模试 EP 吞吐掉了 71%，<b>不用 EP</b>（见表下注）</td>
  ...
  <p>衡量的数叫扩展效率：加卡后每卡的吞吐 ÷ 加卡前每卡的吞吐。下面是我们自己的两组实测，<b>只能组内比</b>。
    左边是一次 weak scaling：卡 ×4、global batch ×4。右边不是 strong scaling：卡数不变，只比 FSDP 组铺多宽。
    倒是同一个 DP 4 × FSDP 128，每个 device batch 12 时是 580、batch 8 时是 453：每张卡的活一少就吃亏，这就是 strong scaling 撞墙的样子。</p>
__FIG_SCALE__
