...
# 2026-08-31 现场判定那两张「都不重要」—— 这份材料是**自读复习和投屏两用**的，
...
#    红框落点，全部拿走。现在这张图只画两种出身。
#
# 起因是 现场看着它说：「对主线来说没啥用，应该尽快进入拆硬件那一段。」
# 去核了一遍，他是对的，而且这门课**已经因为同一条理由删过两次东西**：
#   · 图 A / 图 B（08-31）——「自读的人有目录，听课的人要的是立刻进主题」
#   · fig1-5（09-03）—— 那张图把 312.9 / 312.5 提前亮了，
#     而下一小节的标题还挂着「⭐ 压轴」，**自己剧透自己**
#
# ⛔ 下半犯的是 fig1-5 那条，只是规模大六倍：六个格子把六节的答案
#    全写在脸上 ——「312.9 对 312.5」「一边有 cache」「两边差 16 倍」，
...
q=[f'<svg viewBox="0 0 {W} {H}" width="100%" role="img" '
   'aria-label="GPU 与 TPU 的两种出身：一边不知道你要跑什么、处处留一手，'
   '一边早就知道你要跑什么、一手都不留">']
q.append('<text class="svglbl" x="0" y="16" fill="#202124" style="font-size:13.5px">'
         '整门课只有一个前提：两款芯片出身不同，于是每一层都不同</text>')
cols=[(0,   "#1a73e8","#e8f0fe","GPU","图形处理器",
      ...
      (512,"#1e8e3e","#e6f4ea","TPU","为矩阵乘定做的 ASIC",
       ["从第一天就只服务神经网络","形状规则、访问可预测、控制流静态",
        # ⛔ 2026-09-04 现场点破：原来这两行写「省下的面积全给计算单元 /
