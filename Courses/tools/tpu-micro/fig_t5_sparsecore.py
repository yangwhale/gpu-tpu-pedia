...
def _cards(f):
    ...
    para(f, RX + 16, CB_Y + 48, RW - 32,
         "要不要把 embedding 卸载到 SparseCore，判据是 <b>duplication factor</b> ＝ "
         "这一批取了多少个索引 ÷ 可选的行数。<b>它衡量的是「重复取同一行」的程度</b> —— "
         "重复得越厉害，专用通路省下的越多。", "xs", 16)
    para(f, RX + 16, CB_Y + 100, RW - 32,
         gate.IP(
             "推荐系统那边是几亿行词表里取几百个，duplication factor 很高，这是 SparseCore 的主场。"
             "而我们这次的语言模型是 131,072 ÷ 129,280 ≈ <r>1.01</r> —— 几乎每个索引都取不同的行，"
             "落在判据<b>最不划算的一端</b>，编译器于是根本没把它派过去。",
             "推荐系统那边是几亿行词表里取几百个，重复率极高，这是 SparseCore 的主场。"
             "而语言模型的一个 batch 里，几乎每个位置取的都是不同的词 —— "
             "重复率接近 1，落在判据<b>最不划算的一端</b>。",
             why="内部实测的 duplication factor"), "xs", 16)
...
def _band(f):
    ...
    para(f, 34, BAND_Y + 88, 1330,
         gate.IP(
             ...
             "而真实动机（最可能是反向传播里 scatter-add 在分片下难做）我没查实，不写成结论。",
             "值得注意的是<b>这不是一个「省时间」的选择</b>：one-hot 矩阵乘的绝对耗时"
             "比一次 gather 高出一个量级，只是相对于整步的分母小到可以忽略。"
             "常见的说法「反正 MXU 闲着，用算力换带宽很划算」<r>在算术上站不住</r>。"
             "真实动机最可能在反向传播（scatter-add 在分片下难做），但这一条<g>我没查实，不写成结论</g>。",
             why="内部实测耗时"), "xs", 18)
