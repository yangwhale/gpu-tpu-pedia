...
这一版改回真正的通路示意：
  ...
  · <b>底部那片 17×17 的格阵</b>把 289 倍从数字变成面积：
...
「第三方」「官方未公开」）。<b>289 倍是本图当场算的</b>：64 MiB ÷ 227 KiB ＝ 288.7 → 289。
⚠️ 2026-09-01 从 288 改成 289：288 是把 288.7 截断，而 §3.2b 那处写的是
四舍五入后的 289 —— 同一个量在同一份文档里出现两个数，读者一定会绊一下。
统一取 289 之后，格阵 17×17 ＝ 289 正好对上，原先那句「凑成方阵好数、
差的那一格不是误差」的免责说明随之删掉 —— 它道歉的那件事根本不存在。
"""
...
RATIO_Y, RATIO_H = SCH_Y + SCH_H + 24, 216  # 289 倍那片格阵
...
def _ratio(f):
    ...
    # ⛔⛔ 2026-09-05 补一句限定，否则这张画面会把 §3.2b 刚立的结论推翻。
    #    这里比的是「**一个 core** 的 VMEM」对「**一个 SM** 的共享内存」——
    #    是**每单元**，不是总量。按总量算方向恰好相反：§3.2b 已经算过，
    #    B200 片上 SRAM 合计 231 MiB，v7 是 134 MiB，**GPU 更多**。
    #    不写这半句，读者看完这片格阵会以为 3.2b 白讲了。
    # ⭐ 而这里该比的确实是每单元：决定 FlashAttention 块能开多大的，
    #    是一个 kernel 实例手上那块暂存，不是全片加起来有多少。
    f.t(tx + 18, RATIO_Y + 68, "绿框整片 ＝ TPU VMEM 64 MiB / core", "box", GN)
    y = para(f, tx + 18, RATIO_Y + 92, 504,
             "64 MiB ÷ 227 KiB ≈ <b>289 倍</b>。左边那 <b>289</b> 个小格里，"
             "<b>GPU 只占蓝色那一格</b>。", "xs", 18, max_lines=2)
    y = para(f, tx + 18, y + 6, 504,
             "⚠️ 这是<b>每单元</b>（一个 core 对一个 SM），<b>不是总量</b> —— "
             "按总量比方向相反，见 3.2b。", "xxs", 15, max_lines=2)
    para(f, tx + 18, y + 6, 504,
         "<b>而块能开多大，看的正是每单元这一格</b> —— 块越大，K/V 重复读的趟数越少。",
         "xs", 17, max_lines=2)
...
def _land(f):
    ...
    y = para(f, 58, LAND_Y + 54, 1284,
             ...
             # ⛔ 2026-09-05：原来写「其余三站」——&nbsp;**少数了一站**。
             #    图上是五站（①片外主存 ②硬件缓存 ③片上暂存 ④操作数缓冲 ⑤计算单元），
             #    只有 ② 是结构性的，所以「其余」是 **①③④⑤ 四站**。
             #    同一张图的图注写着「六行里只有两行是结构性的」（②和⑥）——
             #    六减二等于四，**图自己跟自己差一个**。
             #    ⑥ 不是「站」是「项」，多半就是数的时候把它跟站混在一起了。
             "<r>其余四站都是「同一件事，两边各有一个部件」，只是容量和名字不同。</r>",
             "xs", 19)
    para(f, 58, y + 2, 1284,
...
def _src(f):
    ...
    y = para(f, 58, SRC_Y + 48, 1284,
             ...
             "xs", 17)
    para(f, 58, y + 2, 1284,
         "<b>本图当场算的</b>：64 MiB ÷ 227 KiB ＝ 288.7 ≈ 289 倍。"
         "<g>格阵 17×17 恰好 289 格，一格不多一格不少。</g>",
         "xxs", 16)
