...
所以 256 卡省下的 ~39 GB **不足以吃下 MBS=4 需要的 +90 GB 激活**。
→ **MBS 上限仍是 2，与 64 卡相同。**

#### 突破口：用「几乎免费」的 recompute 换 MBS

64 卡 A11 实测：开 `selective recompute` + `moe_act` 后吞吐 **853.5 vs 854.0（−0.06%）**，
代价近乎为零 —— 因为 cutedsl 的融合 grouped MLP 已把重算吸收。

**这意味着可以用 recompute 大幅压激活，换取 MBS=4**。这是 256 卡最有希望超越 1,360 的路径，
也是 E 组的核心假设。

### 13.5 消融矩阵（E 组，256 卡）

基准 = 64 卡最优配置（C2）直接搬到 256 卡。

| # | 实验 | 假设 / 目的 |
|---|---|---|
| **E1** | C2 配置直接搬（FP8 / PP2 VPP8 EP32 / MBS2 / GBS4096） | **跨域基线**。与 64 卡 1,360.4 对比，量化跨域代价 |
| E2 | E1 + GBS 8192 | 微batch 翻倍摊薄固定开销（256 卡 DP=128，GBS 须被 MBS×DP 整除）|
| E3 | E1 + GBS 16384 | 继续放大，找拐点 |
| **E4** | E1 + **selective recompute + MBS4** | **核心假设：用免费 recompute 换更大 GEMM** |
| E5 | E4 + GBS 16384 | MBS4 下的 GBS 扫点 |
| E6 | E1 + EP16 | 64 卡上 EP16 曾 +3.3%，跨域是否仍成立 |
| E7 | E1 + EP64（用满整域） | 07e 在 DSV3 上 EP64 更差，Hy3 待验 |
| E8 | E1 + PP4 VPP4 | 跨域下 PP 增大是否更抗 RDMA 延迟 |
| E9 | E1 + BF16 | 256 卡的 BF16 对照（与 64 卡 854 比）|
| E10 | E1 + MTP=1 | 开 MTP 的代价（64 卡显存不够，256 卡可试）|
| E11 | E1 + seq_length 8192 | 长上下文场景，256 卡的余量能否支撑 |
| E12 | **`NCCL_MNNVL_ENABLE` 保持默认（不设 0）** | **负例验证**：确认跨域不关 MNNVL 确实报错 |

> E12 是刻意设计的**失败实验**，用来实证 §13.3 那条经验，而不是只在文档里写「据说会报错」。
