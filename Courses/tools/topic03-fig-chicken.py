...
def main():
    ...
    y0 = f.header(
        ...
        "⭐ <tspan font-weight=\"700\">三条破法，机制互不相同</tspan>"
        " ——&#160;不是同一个思路的三种写法",
    ...
    WAYS = [
        (BL, "① 师徒", "DSA", "teacher",
         ...
         # ⛔⛔ 2026-09-15：原来写「要专门训一个索引器（V3.2 花了 943.7B token）」——
         #    **数字安错了头**。按 V3.2 技术报告 §2.1.1：
         #      · 热身阶段：冻住除 indexer 外的全部参数，**2.1B token** ←「训索引器」
         #      · 稀疏阶段：**optimize all model parameters**，943.7B token ← 主模型一起重训
         #    ⭐ 所以真相跟原来那句正好相反：**徒弟极便宜，贵的是把师傅也重训一遍。**
         #    ⛔ 而本页 fig-dsa-why 早就写对了（「热身 2.1B → 稀疏期再训 943.7B」）——
         #      **同一页两处打架**。这是今晚第四次撞见同一个形状：
         #      **错的不是不知道，是知道了没传到每一处。**
         ["⚠️ 代价：训徒弟只要 2.1B token，",
          "可之后师傅得陪着再训 943.7B"]),
        (OR, "② 一份算两用", "NSA", "dual",
