...
⭐⭐⭐ 2026-09-14 R38：面板③右上那块「差在哪」原来是**写字板** ——&#160;
   整张图的题眼被写成三行带数字的文字，读者得自己在脑子里做除法。
   换成的装置：**一条按对数切成三段的横条**。
     log(56.88) ＝ log(7.08) ＋ log(4.685) ＋ log(1.714)
   所以三段**恰好铺满整条，不多不少** ——&#160;这不是凑的，
   是「乘法在对数里就是加法」的直接后果。
   ⭐⭐ 于是图形本身就在证明这个分解是**完备的**：没有第三个原因藏在别处。
      （瀑布图做不到 ——&#160;它靠视线连接，不靠长度守恒。）
   ⭐ 画出来才看见的一条：橙＋红两段比绿段还长，即
      **被吃掉的 8.03× 比到手的 7.08× 还多**。原来那三行文字里读不出来。
   ⚠️ 代价：段长是对数的，「看着占一半」≠「吃掉一半」。图上写了，
      讲义里也写了「别讲给台下」。

⚠️ 五条口径写在图上，一条都不能省，否则这张图会被当成实测：
   ① 这是**下界**：只算 HBM 读，不算计算、不算 MoE 的 all-to-all、
      不算 kernel 损耗。真机只会更慢。
   ② **batch = 1。** 批量一大，KV 那段按人头涨，画面会翻回 KV 主导。
      ⛔⛔ 2026-09-15 更正：这条原来写「权重那段被所有人摊薄」——&#160;
      **那是稠密模型的直觉，V3 是 MoE，不成立。** 256 个专家每 token 挑 8 个，
      人一多，挑中的**并集**逼近全部：B=64 时期望激活 222 个，
      权重从 34 涨到 545 GiB（15.8×），跟 KV 的 549 正好五五开。
      ⭐ 外部锚点（256×58×44.0M ＋ 17B ＝ 671B 官方总参）已在 main() 里做成断言。
   ③ 卡数按**装得下**算，没算算力够不够。
   ④ ⛔ 2026-09-15 补齐：DSA 那根**原来只算了挑中的 0.13 GiB，漏了索引器
      自己那份 key cache**（128 维 · FP8 · 61 层 · 128K ＝ **0.95 GiB**，
      是前者的 **7 倍**，而且每步必须整条读完）。现在两笔都算进去了：
      每步 34.59 →&#160;**35.55 GiB**，1.44 →&#160;**1.48 ms**，1.24× →&#160;**1.21×**。
      ⭐ 这门课的招牌是「每个数都能自己核」——&#160;漏一笔，这根柱子就核不出来。
...
def main():
    ...
    kv_sel = 8.58 * K / SEQ              # 挑中的那 2048 个
    # ⛔ 索引器自己那份 key cache：index_head_dim 128 · FP8 · 61 层 · 128K。
    #   它**每步都要整条读完**（不扫完就不知道该挑谁），所以必须进这笔账。
    idx_kv = 128 * 1 * 61 * SEQ / GiB
    assert 6.5 < idx_kv / kv_sel < 7.5, "索引器那份应是选中 KV 的 7 倍上下"
    kvr = kv_sel + idx_kv                                # DSA 每步真读的 KV
    tD = step(kvr, 7)
    gainD = mla[7] / (tD * 1e3)
    ...
    BW_ = 150
    GAP = 36
    bars = rows + [("DSA", kvr, BL, "挑中 0.13 ＋ 索引器 0.95", 7,
    ...
    #   （§五 那个「4.571 白送 × 12.44 赌出来」就是同一个手法）。
    #   ⭐ 拆出来正好是三段，每段有一个能指着说的原因：
    #        56.9×（显存之比）÷ 4.69（每步还要读共享的权重）÷ 1.71（卡多了）＝ 7.08×
    #   ⛔ 三个数必须互相除得上，所以这里全部算出来 + 断言，一个都不手抄。
    # ⚠️ R27 写的是「一条三级阶梯」，R38 已改成一条按对数切三段的横条 ——&#160;
    #   这行注释同步改了。**装置换了不改注释，下一个人就照着旧描述改讲义。**
    ...
    # ⭐⭐ 2026-09-14 R38：这三行原来是**写字板** ——&#160;整张图的题眼
    #   「56.9× 怎么变成 7.08×」被写成三行带数字的文字，读者得自己在脑子里做除法。
    # ⭐ 换成的装置：**一条按对数切成三段的横条**。
    #   因为 log(56.88) = log(7.08) + log(4.685) + log(1.714)，
    #   三段**恰好铺满整条，不多不少** ——&#160;而这不是凑出来的，
    #   是「乘法在对数里就是加法」的直接后果。
    # ⭐⭐ 所以图形本身就在证明这个分解是**完备的**：没有第三个原因藏在别处。
    #   （瀑布图做不到这一点 ——&#160;它靠视线连接，不靠长度守恒。）
    # ⚠️ 代价是段长是对数的，「看着占一半」≠「吃掉一半」，所以必须写在图上。
    # ⚠️ 这一块的竖向预算是**卡死的**：盒子 186 高，底下 top+204 还站着
    #   「所以省下的是那 5 张卡」那句。截图第一版三条说明就撞上了它。
    BARX, BARY, BARW, BARH = 732, top + 88, 600.0, 36
    import math
    segs = [(ratio, GR, "#e6f4ea", "%.2f×" % ratio),
            (d1, OR, "#feefe3", "÷%.2f" % d1),
            (d2, RD, "#fce8e6", "÷%.2f" % d2)]
    sc = BARW / math.log10(store)
    ws = [math.log10(v) * sc for v, _, _, _ in segs]
    # ⛔ 这两条断言是这个装置的**全部合法性** ——&#160;铺不满就不能这么画
    assert abs(sum(ws) - BARW) < 1e-6, sum(ws)
    assert all(w > 0 for w in ws), ws
    # ⭐⭐ 画出来才看见的一条：橙+红两段**比绿段还长**。
    #   也就是「被吃掉的 %.2f×」＞「到手的 %.2f×」——&#160;
    #   这条在原来那三行文字里完全读不出来，得自己把 4.69×1.71 乘一遍。
    eaten = d1 * d2
    assert abs(ratio * eaten - store) < 1e-9, (ratio, eaten, store)
    assert eaten > ratio, (eaten, ratio)          # 图上「右边比左边长」的依据
    assert abs((ws[1] + ws[2]) - (BARW - ws[0])) < 1e-9
    ...
    f.t(732, top + 82,
        ...
        "所以<tspan font-weight=\"700\">正好铺满</tspan> ——&#160;没有第三个原因。"
        % store, GY, size=15)
    xx = float(BARX)
    for (v, col, fill, lab), wseg in zip(segs, ws):
        f.box(xx, BARY, wseg, BARH, fill, col, 4, 1.4)
        f.t(xx + wseg / 2, BARY + 24, lab, col, bold=True, size=18,
            anchor="middle", mono=True)
        xx += wseg
    for j, (col, fill, head, why) in enumerate([
        (GR, "#e6f4ea", "%.2f× 真正到手" % ratio,
         "而右边两段合起来是 %.2f×，<tspan font-weight=\"700\">"
         "吃掉的比到手的还多</tspan>" % eaten),
        (OR, "#feefe3", "÷%.2f 被共享的权重吃掉" % d1,
         "每步还得把 %.2f GiB 权重读一遍，这段谁都省不掉" % ACT),
        (RD, "#fce8e6", "÷%.2f 被多出来的卡吃掉" % d2,
         "MHA 装不下，被迫用 %d 张而不是 %d 张（%d÷%d）"
         % (mha[4], mla[4], mha[4], mla[4])),
    ]):
        yj = top + 144 + j * 20
        f.box(732, yj - 11, 12, 12, fill, col, 2)
        f.t(752, yj, head, col, bold=True, size=15)
        f.t(752 + wpx(head, 15) + 14, yj, "——&#160;" + why, GY, size=15)
    f.t(732, top + 204,
    ...
    # ⛔⛔ 2026-09-15：这里原来写 `kvr * SEQ / K`，注释还得意地说「从 kvr 反推，
    #   这样它永远跟着口径走」——&#160;结果今天给 kvr 加上索引器那一笔，
    #   反推出来就成了 69.6，断言当场炸。
    # ⭐ 判据：**「从别的量反推」不等于「单一来源」。**
    #   真正的单一来源是去拿**原始定义**（PLAN 里 MLA 那一行），
    #   而不是从一个语义会变的派生量倒推回去。
    _KV_MLA = PLAN[3][1]                    # ＝ 8.58 GiB，MLA 一个人的整份 KV
    # ⛔⛔⛔ 2026-09-15 第二次改这一格 ——&#160;**第一次修的时候我修出了个新错。**
    #   第一版补的是「权重全员分摊，所以 batch=32 → 8.0×」。
    #   那是**稠密模型的直觉**：V3 是 MoE，256 个专家每 token 挑 8 个 ——
    #   人一多，挑中的专家**并集**迅速逼近全部，权重那一段自己就涨起来了。
    # ⭐ 外部锚点验过（这是判据，不是拍脑袋）：
    #     256 × 58 层 × 44.0M ＋ ~17B dense ＝ **671B**（官方总参）✓
    #     8 × 58 × 44.0M ＋ dense ＝ 激活 **37B**（官方口径）✓
    #   B=64 时期望激活 222 个专家 → 权重 545 GiB（15.8×），KV 549 GiB —— 正好五五开。
    # ⭐⭐ 所以真实的数比我第一版小了五倍：**32 人是 1.66×，不是 8×。**
    #   方向没错（稀疏确实随 batch 变值钱），错的是幅度 ——
    #   ⛔ 而错因正是本课第一原则点名的那一类：**听起来像常识的架构关系。**
    E_, K_, MI_, H_, LM_ = 256, 8, 2048, 7168, 58
    _per = 3 * MI_ * H_                       # 单个专家的参数量
    assert abs(E_ * LM_ * _per + 17e9 - 671e9) / 671e9 < 0.01, "总参对不上 671B"

    def _wgt(b):                              # batch=b 时要搬的权重（GiB，FP8）
        nE = E_ * (1 - (1 - K_ / E_) ** b)    # 激活专家并集的期望
        return (nE * LM_ * _per + 17e9) / GiB

    _B = 32
    _bN = (_wgt(_B) + _KV_MLA * _B) / (_wgt(_B) + kvr * _B)
    assert 1.5 < _bN < 1.9, "batch=%d 的倍数算歪了：%.2f" % (_B, _bN)
    f.box(712, top + 226, 664, 244, "none", BL, 9)
    f.t(732, top + 256,
    ...
    for r in wrap_rich(
        ...
        "KV 是<tspan font-weight=\"700\">每人一份</tspan>的，人一多它就重新长回来："
        "同时伺候 32 个人，这个数是 <tspan font-weight=\"700\">%.2f 倍</tspan>，"
        "64 个人 <tspan font-weight=\"700\">1.98 倍</tspan>。" % _bN,
            624, 16 * 1.12):
    ...
    for r in wrap_rich(
        "⛔⛔ 这里有个连专家都会踩的坑，值得单说："
        "<tspan font-weight=\"700\">「权重是全员分摊的」是稠密模型的直觉，V3 是 MoE，不成立。</tspan>"
        "256 个专家里每个字挑 8 个 ——&#160;人一多，"
        "<tspan font-weight=\"700\">挑中的专家凑在一起就逼近全部</tspan>："
        "64 个人时期望激活 222 个，权重那一段自己从 34 涨到 545 GiB。"
        "⭐ 所以稀疏确实<tspan font-weight=\"700\">随人数变值钱，但没那么夸张</tspan>。",
            624, 16 * 1.12):
        f.t(732, yj, r, GY, size=15)
        yj += 22
