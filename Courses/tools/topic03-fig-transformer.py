...
⭐⭐ **这张图凭什么能贯穿全篇。**
   ...
   把一层 Transformer 每一步的**张量形状**都标出来之后，会发现图上有
   **一个量是特殊的**：softmax 前后那个注意力矩阵，形状 `BTSKG` ——
   它的形状里**同时带着 T（query 长度）和 S（KV 长度）**，
   所以它是全图**唯一一个随序列长度平方长大**的东西。

   于是整个专题三可以收成一句：**都是在跟这一个张量较劲。**

     · 旋钮① 每个 token 存多少 → 动产生 K/V 的那两条支路（让存下来的变小）
     · 旋钮② 每个 query 看多少 → 动 mask 那一格（让矩阵里大部分格子不用算）
     · 旋钮③ 换一套数学       → **把这个矩阵整个删掉**（换乘法顺序）
     · FlashAttention        → **一处都没改**，只是不让它落到 HBM
...
VARIANTS = [
    dict(
        ...
        panel=("★ 先只看一件事：图上哪个量最大", RD, [
            ("★", "softmax 前后那个矩阵，形状 <b>BTSKG</b>", RD),
            ("", "它的形状里<b>同时带着 T（query 长度）和 S（KV 长度）</b>", GY),
            ("⭐", "所以它是<b>全图唯一一个随序列长度平方长大</b>的量", RD),
            ("", "", None),
            ("", "别的量都是线性的：Q/K/V 各带一个长度，X 带一个长度，", GY),
            ("", "权重<b>一个长度都不带</b>（W 那几个形状里没有 T 也没有 S）", GY),
            ("", "", None),
            ("⭐⭐", "<b>整个专题三，就是在跟这一个张量较劲。</b>", BL),
            ("", "四种较劲的方式，对应三个旋钮加 FlashAttention：", BL),
            ("①", "<b>不动它</b>，改产生 K/V 的支路 —— 让存下来的变小", BL),
            ("②", "<b>不动它的形状</b>，但让里面大部分格子不用算", OR),
            ("③", "<b>把它整个删掉</b> —— 换一套数学", PU),
            ("④", "<b>一处都不改</b>，只是不让它落到 HBM", GR),
        ]),
    ...
    dict(
        ...
        lead="点亮的两格就是这个旋钮的全部作用域。"
             "⭐ 注意：<b>那个平方大的矩阵一点没动</b> —— 这个旋钮省的是显存，不是算力。",
    ...
    dict(
        ...
        panel=("③ 不用听解释 —— 读输出形状就够了", PU, [
            ...
            ("", "chunk 化（分块并行）就是为了把并行度再找回来 —— 见 §5.4", OR),
            ("", "", None),
    ...
    dict(
        ...
        panel=("④ 它删掉的不是计算，是搬运", GR, [
            ("", "朴素写法要把中间那个 BTSKG <b>写进 HBM，再读回来</b>：", GY),
            ("", "写 S → 读 S → 写 P → 读 P，<b>四趟</b>", GY),
            ("⭐", "而 Q/K/V/O 加起来才 128 MiB，那个矩阵是 <b>32 GiB</b>", RD),
            ("", "（序列 128K 时的账，见 §2.2）", GY),
            ("", "", None),
...
def render(v):
    ...
    while i < len(stages):
        ...
        if kind == "io":
            ...
            if mark == "★":
                t(x + wbox // 2 + 10, y + 18,
                  "★ <tspan font-weight=\"700\">全图唯一随长度平方长大的量</tspan>",
                  fill=C(key, RD))
            pos[key] = (x, y, y + 26)
            y += 46
    ...
    #    坐标系里看"的效果。⛔ 别按变体改它。
    # 📌 数字沿用 §2.2 那一组（已在专题二核过），两处必须一致 ——
    #    同一组数在两个地方给出不同的值，比给错还糟。
    NY = PANEL_H + 16
    NUM = [
        ("", "取序列 <b>128K</b>、一层、BF16，把上面那些字母换成实际占多少：", GY),
        ("", "", None),
        ("Q/K/V/O", "四个加起来 —— <b>128 MiB</b>", BL),
        ("BTSKG", "中间那个矩阵 —— <b>32 GiB</b>", RD),
        ("", "", None),
        ("⭐", "<b>差 256 倍。</b>而它俩在图上只隔了两格。", RD),
        ("", "", None),
        ("", "⭐⭐ <b>这就是为什么整个专题都在跟它较劲</b> ——", BL),
        ("", "不是因为它难，是因为<b>别的项在它面前都是零头</b>。", BL),
    ]
    box(PX, NY, PW, 26 + len(NUM) * 19 + 18, "#fef7e0", OR, 8)
    t(PX + 14, NY + 20, "📐 把字母换成数字 ——&#160;它到底比别人大多少",
      "svglbl", BR, size=13)
