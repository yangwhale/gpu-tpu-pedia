...
BUSY, WAIT, IDLE, HOLE = BL, "#c6dafc", "#f1f3f4", RD
...
def build():
    ...
    f.legend([(BUSY, "这一拍在发指令 / 在干活"), (WAIT, "在等数据回来"),
              (HOLE, "气泡：没人能干活，这一拍白丢"), (GREY, "尚未参与")])
...
def _gpu(f):
    ...
    _lane(f, x, 6, "实际发出", "这个处理块", comp, BL)
...
def _tpu(f):
    ...
    _lane(f, x, 4, "实际发出", "这个 core", [("B", "")] * NCYC, GN)
