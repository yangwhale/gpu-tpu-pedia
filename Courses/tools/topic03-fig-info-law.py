...
⭐⭐⭐ 这一张可能是整个专题三**理论上最重要**的一张：
   ...
   而且这条规律**顺手回答了「为什么第三个旋钮的状态必须变大」** ——
   ⚠️ 注意是「状态变大」不是「必须混」：论文给的另一条出路是**按长度放大模型**。
...
def main():
    ...
    y0 = f.header(
        ...
        "⭐⭐ 这一张<tspan font-weight=\"700\">顺手回答了「为什么第三个旋钮的状态必须变大」</tspan>"
        "——&#160;那不是工程经验，是有理论下界的",
    ...
    yy = f.band(yy, "info", "⭐⭐ 「远处可以少看」这句直觉，现在有了严格版本", [
        ...
        "滑窗混合落在 1:1～6:1，线性混合落在 3:1～7:1</tspan>，"
        "两族<tspan font-weight=\"700\">重叠在 3:1～6:1</tspan> ——&#160;"
        "同一条理论一口气解释了两类看起来毫不相干的混合。",
        "⛔ <tspan font-weight=\"700\">但别说成「所有 SWA 模型都混」</tspan> ——&#160;"
        "同一张表里 <tspan font-weight=\"700\">Mistral 7B 是纯滑窗</tspan>，"
        "一层全注意力都没有，而且真的上过生产、用了两年。",
        "⭐⭐ 这个反例不是统计上的例外 ——&#160;它正好说明下面第二条口径："
        "<tspan font-weight=\"700\">L2M 是渐近命题，管不了 32K 这个尺度。</tspan>",
    ])
    ...
    yy = f.band(yy + 14, "warn", "口径，三条", [
        ...
        "⛔ 尤其旋钮②那一行：论文<tspan font-weight=\"700\">明确把稀疏注意力排除</tspan>在"
        "「Transformer 自动满足」那段分析之外（原文括注 excluding sparse attention），"
        "只说「可以照样分析」。",
        "⛔⛔ 还有一条最容易讲过头：<tspan font-weight=\"700\">定理要求的是「状态变大」，"
        "不是「必须混着用」</tspan>。",
        "论文自己给了两条出路：混合（每隔几层放一层状态随长度增长的），"
        "或者 <tspan font-weight=\"700\">model series ——&#160;按长度把模型整个放大</tspan>"
        "（Def 5.5 / Thm 5.6；它 §6 的 Mamba 实验正是这么验的）。",
        "⭐ <tspan font-weight=\"700\">混合是工程上选的那条，不是定理逼出来的那条</tspan> ——&#160;"
        "放大那条的代价是效率优势被抵消、且每个长度都得重训一个。",
