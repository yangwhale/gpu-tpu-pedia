...
| FP8（同硬件，**分母 4614**） | — | ❌ **1,014.8 已撤回**（漏算 189/192 专家，见 §3.4.10）<br>✅ 实测 **637.0 / MFU 13.8%**（补齐 all-gather 后）<br>**生产请用 677**（tokamax + QAG）<br>**7,308 tok/s/chip**（QAG + dvfs 7 + 18 个 tile 参数）<br><sub>2026-08-15 换 tile 入口，此前 674 / 4,854</sub> | 无 QAG 618 ⚠️ 未按新配方复测 | DSV3 官方 743.5 / 16.1% |
...
2. **v7 的 FP8 单芯片吞吐已经反超 GB300**（7,308 vs 6,242，+17%）。调优前只有 51.4%、2026-08-14 时是 77.8%。
   ⚠️ **口径提醒**：7,308 是 v7 的 FP8，6,242 是 GB300 那一列的数；tok/s 本身可比，但精度不同，别当成同精度对比。
...
| **v7 FP8 + QAG** | ⛔ **1,014.8 已撤回**（native 漏算 189/192 专家，补齐后 637.0，见 TUNING §3.4.10）。生产用 **677**（tokamax+QAG）。64 芯片开 QAG 后 **625**（vs 无 QAG 594，+5.3%）；256 experts 探索到 **645**。<br>⚠️ 原先记的「FP8 那条 kernel 的 tile 完全没调过、潜力 726」**已被实测证伪** —— FP8 内部仍走 tokamax，tile monkeypatch 一直生效。<br>2026-08-05 又扫了 tile / XLA flag / SparseCore 卸载 / 推 batch 共 8 格，**无一正收益**：调参空间已见底，再往上要加卡、改模型形状或写代码。见 [TUNING-v7 §4.6](TUNING-v7.md#46-什么能调什么不能调--一张总表) |
