...
for r in range(NR):
    ...
    p.append(f'<text class="svgsm" x="{gx+CW*4.5}" y="{gy+r*CH+12}" text-anchor="middle" fill="{PU}" style="font-size:10px">同一个数 × 127</text>')
...
# ── 大数字带
NUMS=[("50%","bf16 的 [1,128]：分到 2×128",BL),
      ("1,024 倍","补成 8×128×8×128",PU),
      ("50%","官方原话「50% or more」",RD)]
for d,(n,sub,c) in zip(PAN,NUMS):
    x=d["x"]
    p.append(f'<rect x="{x+12}" y="{NUM_T}" width="{PW-24}" height="{NUM_H}" rx="6" fill="{c}"/>')
    p.append(f'<text class="svgnum" x="{x+26}" y="{NUM_T+30}" fill="#fff" style="font-size:21px">{n}</text>')
    p.append(f'<text class="svgsm" x="{x+PW-26}" y="{NUM_T+29}" text-anchor="end" fill="#ffffffd8" style="font-size:10.5px">{sub}</text>')
