...
**这一轮的价值不在"跑出了同样的数"，而在跑之前和跑之后各抓到两个错。**
...
**MoE 兑现不了 FP8 的两倍峰值，稠密可以。** 原因跟 §九 开头那条一样：
...
§九 里我纠正过一次：`shard_exp_on_fsdp` 不是对 Hy3 封死，
...
| 10 | v7 调优到 MoE 合理水位 | 🔄 进行中 | 首跑 404.8 TFLOP/s/chip，目标见 §九 |
