...
# ⭐ 2026-09-13 只改字号，**立意和布局一个字不动**（现场：「编年史我挺满意的」）。
...
# ⛔⛔ 2026-09-14 R14：这两行原来是全书**第 3、4 行**，而且以「⭐ 一句话看懂
#    整段历史」开头。外部评审（麻瓜视角）的原话值得抄在这里：
#      「我在这两句里连续撞了六个不认识的东西：平方的计算量 / 并行度 / 往回赎 /
#        O(N) / 串行的状态 / chunk 化。而且它以『一句话看懂』开头 ——
#        **名字看不懂我不会自责，『一句话看懂』看不懂我会。**」
# ⭐ 两处改动：
#   ① 删掉第二行（线性注意力 / chunk 化那句）——&#160;它整句是第七章的剧透，
#      对开篇读者零价值，却贡献了六个陌生词里的三个。
#   ② 「一句话看懂」→「现在不用懂」。**承诺要跟兑现时间对齐** ——&#160;
#      这句话真正能被读懂的时刻在最后一章，那就在这里说清楚它会回来。
# ⛔ 第一行必须留：它是全书总纲，最后一章会把它原样抄回去收尾（首尾呼应靠它）。
t(0, 39, '⭐ 从 2014 年到今天，这一路在干的其实是一件事 ——&#160;<tspan fill="%s">现在不用懂，'
         '最后一章会回来收这句</tspan>：' % GY)
t(0, 58, '<tspan font-weight="700">Transformer 当初做的交易是'
         '「用平方的计算量，买来完全的并行度」——&#160;而这段历史，'
         '是在把那笔交易<tspan style="text-decoration:underline">往回赎</tspan>，'
         '但不能把并行度还回去。</tspan>'
         '　<tspan fill="%s">（信息截至 2026-09-07，全部现搜）</tspan>' % GY)
