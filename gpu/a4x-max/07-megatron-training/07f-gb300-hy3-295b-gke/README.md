...
直觉上 GB300 FP8 峰值 5,000 vs BF16 2,500 = 2×，但 **MoE 模型兑现不了**。本仓自己的实测：
...
MFU = Model TFLOP/s ÷ 硬件峰值（**BF16 按 2,500，FP8 按 5,000**）。

> ⚠️ **2026-08-31 更正**：本页此前一律用 **2,700 / 5,400** 作分母，那是把 GB200 的数字乘 1.2 推出来的，**NVIDIA 从未发布过这两个数**。官方 GB300 NVL72 规格表：`FP16/BF16 Tensor Core 360 PFLOPS`、`FP8/FP6 Tensor Core 720 PFLOPS`，脚注 1 写明「除非另有说明，所有 Tensor Core 规格均为 sparse」——
> dense 减半、再除以 72 颗 GPU → **BF16 2,500 / FP8 5,000 TFLOP/s per GPU**，与 GB200 NVL72 **完全相同**（Blackwell Ultra 提升的是 dense FP4 1.5× 与 attention 2×，不是 BF16/FP8）。
> 交叉验证：官方另列 `FP4 1440 | 1080²`（脚注 2 = without sparsity），1080 ÷ 72 = 15 PFLOPS/GPU dense FP4 ✓。
> **本页所有 MFU 已按新分母重算，TFLOP/s 与 tok/s 不受影响。**
...
|---|---|---|---|---|---|---|
| A1 Hy3 冠军 | BF16 | 1 | 64 | 854.0 | **34.2%** | 6,242 |
| C1 Hy3 | **FP8_MX** | 1 | 64 | 1,285.9 | 25.7% | 9,396 |
| **C2 Hy3** | **FP8_MX** | **2** | **64** | **1,360.4** | **27.2%** | **9,945** |
| *Qwen3-235B（官方对标）* | *MXFP8* | *2* | ***256*** | *1,360* | *27.2%* | *—* |
| *DSV3 671B（官方对标）* | *MXFP8* | *1* | *256* | *1,658* | *33.2%* | *—* |

**C2 的 1360.4 / MFU 27.2% 与 Qwen3-235B 在 256 卡上的 1360 / 27.2% 完全重合，而我们只用了 64 卡。**

### 两个成因，且相互耦合

**① 精度口径（主因，+50.6%）**
Model TFLOP/s 的分子是模型的数学 FLOPs（与精度无关），分母是**该精度下的硬件峰值**。
GB300 BF16 峰值 2,500、FP8 峰值 5,000。跑 BF16 时算力天花板本来就只有一半。
...
**BF16 场景下的最优**（若因精度对齐要求必须用 BF16）：**B4 = EP16，882.3 TFLOP/s / MFU 35.3%**。
...
## 十三、256 卡跨域测试方案（开跑前的设计文档）

> **状态：方案设计完成，待执行。** 本节先把拓扑、环境、显存预算、消融矩阵、判定标准全部写死，
> 再开跑 —— 避免像 64 卡那轮那样边跑边补设计。

### 13.1 目标

1. 在 **256 GPU / 4 个 NVLink 域**规模上重跑消融，找到该规模的最优配置。
2. 回答：**256 卡的 TFLOP/s 能否高于 64 卡的 1,360.4？** 还是只是总吞吐线性放大？
3. 量化**跨域通信的代价** —— 64 卡时 EP/PP/DP 全走域内 NVLink，256 卡时 PP/DP 必须跨域走 CX-8 RDMA。

### 13.2 拓扑：4 池 × 16 节点

实测集群状态（2026-07-26 10:12 HKT，8 个 GB300 池）：

| Pool | 节点 | 已占用 | 空闲 | kubelet | subblock |
|---|---|---|---|---|---|
| gb300-pool-0002 | 18 | 17 | 1 | 4447000 | e9e26a9c9da3 |
| gb300-pool-0006 | 18 | 17（infer 团队） | 1 | 4447000 | ee18edff617d |
...
| gb300-pool-0015 | 18 | 16（本项目 64 卡实验） | 2 | 4447000 | 10270c36001d |
| gb300-pool-0016 | 18 | 0 | **18** | 4447000 | d58388dca90c |
| gb300-pool-0017 | 18 | 0 | **18** | 4447000 | 572f6bfac269 |

**选定 4 个域**（每池取 16 台，留 2 台热备）：

| 域 | Pool | ComputeDomain | 说明 |
|---|---|---|---|
| A | **gb300-pool-0015** | `yw-cd-a` | 释放现有 64 卡实验后复用 |
| B | **gb300-pool-0016** | `yw-cd-b` | 全空，18 台 |
| C | **gb300-pool-0017** | `yw-cd-c` | 全空，18 台 |
| D | **gb300-pool-0013** | `yw-cd-d` | 全空，18 台（有 DRAM correctable ECC 告警，软错误不影响正确性）|

**备份池**：`0014`（16 台，零余量）、`0009`（18 台，**不在交付范围，动用前须问 Chris**）。

> **为什么每池只用 16 台而不是 18**：集群 **auto-repair 关闭**，坏节点不自愈。
> 留 2 台同 subblock 热备，坏一台换 label 即可顶上，避免 07e 那种「整池征用搬迁」。
>
> **pool = subblock = NVLink 域，一一对应**（8 个池的 subblock ID 各不相同，已实测确认）。
> **每个域必须一个独立 ComputeDomain**，绝不能用一个 CD 横跨 4 池 —— IMEX 通道只能在单个 NVLink 域内建立。

### 13.3 ⚠️ 跨域 MNNVL 环境变量（最关键的一条，来自 Chris 的既往经验）

**单域（64 卡）与跨域（256 卡）的 NCCL 配置不同，照搬会直接报错。**

| env | 单域（我们 64 卡用的） | **跨域（256 卡必须改）** | 作用 |
|---|---|---|---|
| `NCCL_MNNVL_ENABLE` | 未设（吃 GIB 默认 **2**） | **`0`** | **跨域必须关 MNNVL**，否则 NCCL 试图跨域走 NVLink 而失败 |
| `NCCL_CUMEM_ENABLE` | 未设（默认 1） | **`0`** | cuMem 仅服务于 MNNVL，跨域一并关 |
| `USE_MNNVL` | `1` | **`1`（保持不变）** | 这是给 **HybridEP dispatcher** 的，告诉它域内有 NVLink 可用 |
| `NVLINK_DOMAIN_SIZE` | `72` | `72` | GB300 NVL72 |
| `NUM_OF_HYBRID_EP_RANKS_PER_NVLINK_DOMAIN` | `= EP` | `= EP`（且 **EP ≤ 64**，不得跨域） | 改 EP 必须同步改这里 |
...
**EP 约束**：HybridEP 的 all-to-all 只能在单个 NVLink 域内。4 域 × 64 GPU，因此 **EP ≤ 64**，
且 EP rank 必须落在同一 subblock。64 卡时可选的 EP=16/32 在 256 卡下依然可用（域内子集）。

### 13.4 显存预算（256 卡 vs 64 卡）

用 [`mem_calc.py`](mem_calc.py) 的公式，按 full graph 的 **1.78×** 系数换算：

| N | PP | EP | MBS | 精度 | 权重 | 梯度 | 优化器 | 激活 | 朴素 | ×1.78 | 判定 |
|---|---|---|---|---|---|---|---|---|---|---|---|
| 64 | 2 | 32 | 1 | FP8 | 8.2 | 16.4 | **51.5** | 45.2 | 121.3 | 215.9 | ✅ |
| 64 | 2 | 32 | 2 | FP8 | 8.2 | 16.4 | **51.5** | 90.3 | 166.4 | 296.2 | ❌（实测 276 GB 可跑，公式偏保守）|
| **256** | 2 | 32 | 2 | FP8 | 8.2 | 16.4 | **12.9** | 90.3 | 127.8 | **227.5** | ✅ 宽裕 |
| **256** | 4 | 32 | 2 | FP8 | 4.1 | 8.2 | **12.9** | 93.0 | 118.2 | **210.3** | ✅ 最宽裕 |
| 256 | 2 | 32 | **4** | FP8 | 8.2 | 16.4 | 12.9 | **180.6** | 218.1 | 388.3 | ❌ OOM |

#### ⭐ 洞察一：加卡**不能**直接解锁更大的 MBS

**激活显存与 GPU 数无关。** 1F1B 稳态下每个 stage 持有 `L 层 × MBS` 份激活
（`L/PP` 层 × `PP` 个在飞 microbatch），既不随 PP 变，也不随 DP 变。
加卡只摊薄**优化器状态**（`294.9B × 12B / N`：64 卡 51.5 GB → 256 卡 **12.9 GB**）。
省下的 ~39 GB 不足以吃下 MBS=4 需要的 +90 GB 激活。

**两家参照实测印证了这一点**：

| 模型 | 卡数 | TP/PP/VPP/EP | MBS | GBS | TFLOP/s | 显存 |
|---|---|---|---|---|---|---|
| DeepSeek V3 671B | 256 | 1 / **2** / **8** / 32 | **1** | 4096–15360 | 1,658 | — |
| Qwen3-235B | 256 | 1 / **4** / **2** / 32 | **2** | **8192** | 1,360 | **277/288 (96%)** |
| **Hy3（我们）** | **64** | 1 / 2 / 8 / 32 | 2 | 4096 | **1,360** | 276 |

> Qwen3 在 256 卡上实测 **MBS=4 即便配 VPP=4 也 OOM**（276 GB 用满还差 288 MB），**VPP=8 直接 capture OOM**。
> 即：**两家在 256 卡上都卡在 MBS ≤ 2**，跟我们 64 卡的上限一样。
>
> **而我们已经用 64 卡打平了 Qwen3 用 256 卡拿到的 1,360。**

#### ⭐ 洞察二：GBS 必须随卡数放大，否则 bubble 恶化

`microbatch/DP-rank = GBS / (MBS × DP)`，这个数决定 pipeline bubble。**照搬 64 卡的 GBS 会让它掉 4 倍**：

| 卡数 | PP | DP | GBS | MBS | microbatch/rank | 评价 |
|---|---|---|---|---|---|---|
| 64 | 2 | 32 | 4,096 | 2 | **64** | 基线 |
| 256 | 2 | **128** | 4,096 | 2 | **16** | ⚠️ 直接搬 → bubble 涨 4× |
| 256 | 2 | 128 | 8,192 | 2 | 32 | 仍偏少 |
| **256** | **2** | 128 | **16,384** | 2 | **64** | ✅ 与基线等价 |
| **256** | **4** | **64** | **8,192** | 2 | **64** | ✅ **Qwen3 的解法** |

**Qwen3 用 PP=4 把 DP 压到 64**，从而只需 GBS=8192 就能保住 64 个 microbatch，
...
围绕上面三条洞察设计。**基准 E1 = Qwen3 在 256 卡的同款并行方案**（直接对标其 1,360）。
...
| E9 | EP64（用满整域） | E1 + EP64 | 07e 在 DSV3 上 EP64 更差，Hy3 待验 |
| E10 | VPP 扫点 | E1 + VPP4 / VPP8 | Qwen3 实测 VPP8 会 OOM，Hy3 层数更少（80 vs 94）或可行 |
| E11 | BF16 对照 | E1 + BF16 | 与 64 卡 854 / 882 对照 |
| E12 | MTP=1 | E1 + MTP | 64 卡显存不够，256 卡可试 |
| **E13** | **MNNVL 负例** | 不设 `NCCL_MNNVL_ENABLE=0` | **实证 §13.3 的跨域经验确实会报错** |

> **E4 是本轮的重头戏**：如果 TP2+MBS4 能把 TFLOP/s 推过 1,360，
> 那就证明 **256 卡的价值不只是 4× 总吞吐，还能提升单卡效率** —— 这正是「给你 256 卡干什么」的答案。
> 如果推不过去，也是有价值的负结论：说明 Hy3 在 GB300 上的单卡天花板由 hidden=4096 的 GEMM 形状决定，加卡只能换总量。

### 13.6 判定标准与对照口径

| 指标 | 采集方式 | 与 64 卡对照 |
|---|---|---|
| **Model TFLOP/s/GPU** | 稳态末 5 步中位数 | 直接可比（已按 GPU 归一化）|
| **MFU** | ÷ 2,500（BF16）或 5,000（FP8） | 直接可比 |
| **总吞吐** | `TFLOP/s × 256` vs `× 64` | 理想 4× 线性 |
| tokens/s/GPU | `GBS × seq / step / N` | 直接可比 |
| HBM 峰值 | 全程 `nvidia-smi` max | 应比 64 卡低 ~39 GB（优化器）|
| **跨域缩放效率** | `TFLOP/s@256 ÷ TFLOP/s@64` | **>95% 算优秀，<90% 说明跨域 RDMA 成瓶颈** |

### 13.7 风险与预案

| 风险 | 预案 |
|---|---|
| GIB 脚本覆盖 `NCCL_MNNVL_ENABLE` 回 2 | rank 0 日志 grep 实际值；必要时改用 `NCCL_CONF_FILE` 或在 GIB 脚本之后再 export |
| 4-CD IMEX clique 死结（07e 踩过）| 每池独立 CD；不反复 churn；卡住则整池换到备份池 0014/0009 |
| 僵尸 CUDA context 跨实验污染 | 沿用 `sweep.sh` 的 cleanup：验证显存归零，清不掉则重建 pod |
...
2. 给 4 池各打 16 台标签（`hy3a/hy3b/hy3c/hy3d`，避免与旧标签串台）
3. 应用 4-CD pod 池 YAML，确认 64/64 Running + 4 个 CD Ready
