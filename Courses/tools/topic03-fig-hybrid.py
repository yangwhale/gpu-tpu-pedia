...
def main():
    ...
    y0 = f.header(
        ...
        "⭐⭐ 「为什么必须混合」现在有理论版本了 ——&#160;"
        "§二 那条 L2M 条件说<tspan font-weight=\"700\">纯线性的状态必须变大</tspan>——&#160;⚠️ 而「变大」的办法不止混合一种",
        [(RD, "单用的短板"), (GR, "各家实际落点"),
         (BL, "建议区间"), (PU, "意外红利")])
    ...
    f.t(x + 40, yy + 112, "<tspan font-weight=\"700\">总存在一个长度，它必然兜不住。</tspan>", PU,
        True, 12.5)
    # ⛔ 2026-09-14：这里原来收在「L2M 把它直接判死」。定理判死的是
    #   「状态不增长」，**不是「不混合」** —— 论文自己给的另一条出路是
    #   按长度整个放大模型（model series, Def 5.5 / Thm 5.6）。
    # ⭐ 判据：**定理的结论要按定理的措辞抄，别顺着自己的叙事改一个词。**
    f.t(x + 40, yy + 132, "⚠️ 但定理要的是「状态变大」，办法不止混合一种", GY2,
        size=11, w=pw - 76)
    yy += 142
    ...
    f.t(x + 22, yy, "⭐⭐ 意外红利：全注意力层不加位置编码（NoPE）", PU,
        True, 13, cls="svglbl")
    yy += 24
    ...
    yy = f.band(yy + 14, "warn", "配比是超参，别背成常识", [
        ...
        "⭐ 三个口径别混：<tspan font-weight=\"700\">实测落点 1:1～7:1</tspan>（本课 44 行表全量 ——&#160;"
        "⚠️ 最低那两家 Gemma 2 与 gpt-oss 都是 1:1，别再写「无一例外 3:1 起」），"
    ...
    yy = f.src(yy + 16,
               ...
               "NoPE 见 <tspan font-weight=\"700\">Kimi Linear</tspan> arXiv 2510.26692（原文：对所有全注意力层用 NoPE）——&#160;⚠️ <tspan font-weight=\"700\">是它先做的，K3 是沿用</tspan>；75% / 6.3× 同文")
