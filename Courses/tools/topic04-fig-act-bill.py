...
def main():
    ...
    y0 = f.header(
        ...
        "水平的</tspan>，会把山形压扁",
        # ⛔ 2026-09-23 现场：「你这个橙色到底画的是什么？也没地方说一下。」
        #   ⭐ 查下来：图例里**只有蓝 / 红 / 绿三条，橙色根本没登记** ——&#160;
        #     它唯一一次被解释，是在主图**最右边**那行小字里，
        #     而读者是在**左下角**那个放大插图上第一次遇见它的。
        #   ⭐⭐ 判据：**一种颜色第一次出现的地方，就得能查到它是什么。**
        #     解释写在别处不算 ——&#160;读者不会为了一个颜色去扫全图。
    ...
    f.box(IX0 - 12, ITOP - 54, (IX1 - IX0) + 34, (IBOT - ITOP) + 80,
          "#fffaf2", OR, 8, 1.2)
    f.t(IX0 - 4, ITOP - 34,
        "🔍 <tspan font-weight=\"700\">橙色那条 ＝ 开了全量重算之后的同一笔账</tspan>",
        OR, True, 12.5)
    f.t(IX0 - 4, ITOP - 14,
        "同一条序列、同一个模型，只是<tspan font-weight=\"700\">中间结果不留，"
        "反向要用时现算一遍</tspan>　·　这里单独放大（纵轴换了一把尺）",
        GY, size=11.5)
