...
> **我们无 H100，全程 GB300。主线从 Stage 2（30B 4 节点 GB300）正式开始**；Stage 0/1 仅可选的 GB300 pipeline sanity。

| 阶段 | 模型 | 规模 | recipe | 目的 |
|---|---|---|---|---|
| 0 可选 | 小模型/1B | **1 GB300 节点(4 GPU)** | `grpo_smoke.yaml` / `grpo_math_1B.yaml` | pipeline sanity（非必须）|
| 1 可选 | Llama3.1-8B | 2 GB300 节点 | `grpo-llama3.1-8b-instruct-2n4g-async-1off.yaml` | 多节点 async 入门 |
| **2 ⭐ 主线入口** | Qwen3-30B-A3B (MoE) | **4 节点 GB300** | `grpo-qwen3-30ba3b-4n4g-async-1off.yaml` + `qwen3_30b_math_4n_4gpu.gb300.infra.yaml` | **正式起点**（EP8 + async 1-off + gen/train 分离）|
...
## 2b. 跨框架横向对比 + NeMo RL 定位

> 来源：SemiAnalysis《RL Systems Mind the Gap: Matching Trainer and Generator Throughput》(2026-06) + 《Anatomy of RL Frameworks》(hanifleo, 2025-09) + 各框架源码。

### 2b.1 RL 系统的核心矛盾（"Mind the Gap"）

一个开源 RL 训练系统有**三个角色**：**generator（生成 rollout）**、**RL environment（打分）**、**trainer（更新策略）**。verl 论文指出 **80–90% 的训练时间花在样本生成上**——这驱动了所有框架的架构选择。

SemiAnalysis 的核心论点：**要匹配 trainer 吞吐和 generator 生产率**（generator 生产率 = 并发 rollout 数 ÷ 端到端延迟）。两者不匹配就有一方饿着：
- generator 慢 → trainer 空等（GPU 闲置）
- trainer 慢 → rollout 堆积变陈旧（staleness 上升，需 off-policy 修正）

影响 generator 生产率的因素：推理引擎调优（并行策略、KV 量化、**PD 分离**、投机解码）、sandbox 延迟、reward model 类型（轻量 verifier 快 / LLM judge 慢）、reward shape（PRM 逐轮评 vs 最终评）。

### 2b.2 两条架构路线（gen↔train 如何共处）

| 路线 | 代表 | 机制 | 优 | 劣 |
|---|---|---|---|---|
| **A. Colocated（共卡引擎）** | verl 默认、OpenRLHF、NeMo RL 默认 | gen 和 train 同进程/同 placement group，权重经**显存/NCCL/CUDA IPC** 直传 | 延迟最低、无序列化、可零拷贝权重共享 | gen/train 抢同一批 GPU、难独立扩展 |
| **B. Disaggregated（分离服务）** | slime（唯一模式）、AReaL、verl agentic、NeMo RL async | rollout 作为独立 HTTP/RPC 服务，train 跨网络发请求 | 故障隔离、独立扩展、异构硬件 | RPC/网络开销、序列化成本 |

### 2b.3 权重同步（gen↔train 权重转换，你问的关键能力）

RL 每步训练完要把新权重推给 gen，这是**吞吐关键**。三种做法：

- **Resharding / Device-Mesh（verl 3D-HybridEngine）**：colocated 下用 `torch.distributed.redistribute` 做 **3D 张量原地重分布**（训练的 TP/PP/EP mesh → 生成的 mesh），`FSDPVLLMShardingManager` 实现，**近零显存开销**（in-place 变换单个模型）。**只在 colocated engine 模式可行**。这就是你说的 verl "colocated 3D HybridFlow 快速权重转换"。
- **CUDA IPC handles（slime）**：Megatron 先 gather 分布式张量 → 导出 **CUDA IPC handle** → SGLang 经 `/update_weights_from_tensor` 映射重载，**免 CPU↔GPU 拷贝**。配合 Tensor Flattening/Bucketing/MoE 快路径，**Qwen3-30B-A3B 权重转换仅 7 秒（8×H100）**。
- **Direct Broadcast（OpenRLHF）**：简单广播新权重。

### 2b.4 异步 / off-policy 能力对比

| 框架 | 控制器模型 | Colocated | Disaggregated | Async RL | Off-Policy | 权重同步 | 训练后端 | GB300 官方 recipe |
|---|---|---|---|---|---|---|---|---|
| **verl** | 单控制器 + HybridFlow | ✅ 默认(3D-HybridEngine) | ✅ agentic(RPC) | ❌(server 模式有 req-level) | 1-step off | **Device-mesh 原地 resharding**(零拷贝) | FSDP/Megatron | ❌(Blackwell-ready) |
| **slime** | 3 模块分离 | ✅(`--colocate`) | ✅ 主打 | ✅ 原生 | 1-step off | **CUDA IPC**(30B 7s) | Megatron | 🟡 docker 有 GB300 处理 |
| **AReaL** | 全异步 | — | ✅ 核心 | ✅ 核心 | **✅ 全 staleness-aware PPO** | via SGLang | — | ❌ |
| **OpenRLHF** | 角色分离 actor | ✅ hybrid engine | ✅ | ❌ | ❌ | Direct broadcast | DeepSpeed | ❌ |
| **NeMo RL** | Ray + 单驱动 | **✅ 默认** | **✅ async 1-off** | **✅** | 1-step off + 重要性采样 | **Megatron→vLLM refit** + in-flight | **Megatron-Bridge**(TP/PP/EP/CP) | **✅ 专属 gb300 YAML** |

### 2b.5 ⭐ NeMo RL 到底强在哪、弱在哪（回答核心问题）

**NeMo RL 同时具备两大流派的能力——这是它的核心优势：**
1. **有 verl 的 colocated 能力**：默认 colocated 模式（gen/train 共卡），权重经 Megatron→vLLM **refit** + `in_flight_weight_updates` 直传。**但注意**：NeMo RL 的权重同步走 **refit**（Megatron gather → vLLM 重载，类似 slime 的 IPC 路线），**不是 verl 那种 3D device-mesh 原地零拷贝 resharding**。两者都快，verl 的原地 reshard 在 colocated 下理论显存开销更低。
2. **有 slime 的 async 1-off 分离能力**：`async_grpo` + `colocated=false`，**one-step off-policy + 重要性采样修正**——与 slime 同档（1-off）。
3. **独有优势**：**Megatron-Bridge 训练后端**（TP/PP/EP/CP 全并行，大 MoE/长上下文训练最强）+ **NVIDIA 亲自维护的 GB300 NVL72 recipe + k8s/KAI/DRA 基建** + **FP8/MXFP8 rollout**。一个框架里既能 colocated 又能 disaggregated async。

**NeMo RL 相对弱的地方：**
1. **off-policy 只到 1-off**：不像 **AReaL 的全 staleness-aware PPO**（任意 staleness η + 解耦目标 + staleness 正则 + 可中断 rollout）。极端异步/大集群下 AReaL 的 off-policy 更激进、吞吐上限更高。
2. **权重转换速度未见公开数字**：slime 明确报了 Qwen3-30B **7s**，NeMo RL 的 refit 速度需自己实测（本文 §6 待补）。
3. **生态/社区成熟度**：verl 社区最大、recipe 最多；NeMo RL 偏 NVIDIA 官方栈、绑 k8s/KAI 较重（但正好适合我们的 GB300 集群）。

**一句话定位**：**NeMo RL = verl 的 colocated + slime 的 async 1-off 分离 + Megatron-Bridge 大模型训练 + NVIDIA 官方 GB300 基建**，是「全能型 + GB300 原生」；代价是 off-policy 不如 AReaL 激进、绑 NVIDIA k8s 栈较重。对我们（有 GB300 集群、要官方 recipe、跑大 MoE）是**最合适**的起点。
...
> **⚠️ 我们没有 H100，最小测试也在 GB300 上做。所以本阶梯【直接从适合 GB300 的规模起步】——主线入口 = Stage 2 的 Qwen3-30B-A3B 4 节点 GB300。** 下面 Stage 0/1 仅作**可选**的「单/双 GB300 节点 pipeline sanity」（用小模型快速验证 pipeline 正确、不烧大集群），不是必须。若直接从 30B 4 节点起也完全可以。

### Stage 0（可选）— 单 GB300 节点 pipeline sanity（1 节点 4 GPU）

不为性能，只为**快速验证 pipeline 正确 + 环境通**（HF 下载、Ray、vLLM refit、GRPO loss）。在**一个 GB300 节点**（4 GPU）上跑小模型：
```bash
# 冒烟(最快):
uv run examples/run_grpo.py --config examples/configs/grpo_smoke.yaml
# 小模型完整 GRPO (把 cluster 改到 1 节点 4 GPU):
uv run examples/run_grpo.py --config examples/configs/grpo_math_1B.yaml cluster.gpus_per_node=4
```
> **正确性参考 baseline**（不必在 GB300 复刻全程，作为「收敛长什么样」的参照）：官方 DeepScaleR（DeepSeek-R1-Distill-Qwen-1.5B）三阶段 8K→16K→24K，~400 步 average reward **0.65**，AIME24 pass@1 上升（`docs/guides/grpo-deepscaler.md`，原在 8×H100）。我们无 H100，把它当收敛形状的参照即可。

### Stage 1（可选）— 双 GB300 节点 async 入门（Llama3.1-8B，2 节点）

验证多节点 Ray + NCCL + async gen/train 分离（为 30B 铺路）：
```bash
RECIPE=examples/configs/recipes/llm/performance/grpo-llama3.1-8b-instruct-2n4g-async-1off.yaml
# 配套一个 2 节点 GB300 infra（参照 4n infra 改 num_nodes=2）
```

### Stage 2 — ⭐⭐ GB300 主线入口（Qwen3-30B-A3B MoE，4 节点 16 GPU）——【从这里正式开始】
...
**复现 checklist（GB300-first，无 H100）：**
1. ☐ 前置：Lustre PVC 绑定 + repo clone 到 `/mnt/rl-workspace/${user}/nemo-rl` + secrets + KAI 拓扑
2. ☐ （可选）单 GB300 节点小模型 pipeline sanity（Stage 0），确认 HF/Ray/vLLM refit/loss 通
3. ☐ **主线从 Stage 2 起**：`nrl-k8s run $RECIPE --infra $INFRA --wait` 起 GB300 **4 节点 Qwen3-30B**，先 `grpo.max_num_steps=5` 冒烟
4. ☐ 冒烟过 → 去掉 step 限制正式训练，wandb 记 step time / MFU / reward + **实测 refit 权重转换耗时**（对标 slime 30B 7s）
5. ☐ 逐级扩：4n(30B) → 8n → 16-32n(235B) → 64n(DeepSeek-V3)
