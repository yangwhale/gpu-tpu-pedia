...
def main():
    ...
    for r, (lab, alive, col, note) in enumerate((
        ...
        yy = py2 + 76 + r * 110
        f.t(80, yy - 12, lab, INK, True, 15)
        f.box(80, yy, BARW, 40, "#f1f3f4", GY2, 4)
        if alive < 1.0:
            f.box(80, yy, BARW * alive, 40, "#fef7e0", OR, 4, sw=1.8)
        f.t(80 + BARW + 16, yy + 26, note, GY, size=13)
        f.t(80, yy + 66,
    ...
    f.box(80, py2 + 300, 1240, 62, "#fff8e1", OR, 8)
    f.t(104, py2 + 326,
    ...
    BASE = py3 + 190                      # 柱子的基线
    bx = 150
    f.t(bx + 110, py3 + 62, "真实分布　（%s 根柱子）" % format(VOCAB, ","),
        INK, True, 14.5, "middle")
    f.t(bx + 110, py3 + 86, "高低极不均匀", GY, size=12.5, anchor="middle")
    for k, v in enumerate(REAL):
        h = 88 * v / 0.5
        f.box(bx + k * 48, BASE - h, 34, h, "#fef7e0" if k == 0 else "#f1f3f4",
              OR if k == 0 else GY2, 3)
    f.t(bx + 130, BASE + 26, "…… 其余 %s 个的概率几乎是 0"
        % format(VOCAB - len(REAL), ","), GY2, size=12)

    # ⛔ 这里原来用的是 ⇄ ——&#160;渲染环境缺这个字形，出来是一个空框。
    #   ⭐ 判据：**装饰性符号一律画出来，不要靠字体里有没有。**
    f.line(668, BASE - 48, 622, BASE - 48, GY, 2.2)
    f.line(732, BASE - 30, 778, BASE - 30, GY, 2.2)
    f.t(700, BASE + 2, "一样难猜", GY, True, 14, "middle")

    cx = 880
    f.t(cx + 155, py3 + 62, "等概率的 7 个", INK, True, 14.5, "middle")
    f.t(cx + 155, py3 + 86, "每个都是 1/7", GY, size=12.5, anchor="middle")
    for k in range(7):
        f.box(cx + k * 48, BASE - 56, 34, 56, "#e6f4ea", GR, 3)
    f.t(cx + 155, BASE + 26, "这就是「困惑度 ＝ 7」那句话的全部含义",
        GR, True, 13, "middle")
