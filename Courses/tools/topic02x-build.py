...
<header class="hero">
  __FAMNAV__
  <div class="kicker">专题二 · 外传　·　L100　·　18 分钟</div>
...
a('  <p>这个 560 是从什么样的硅片布局里长出来的？<b>把两颗按同一种画法拆开看。</b></p>')
a(fig("figx-2",
...
a(fig("figx-6",
      "<b>图 X-6</b>　左边是权重体积（回答「装不装得下」），"
      "<b>右边是我们真跑过的配置</b>（回答「实际用了几颗」）。"
      "<em>⭐ 实测下来：<b>小的单颗，主力清一色 8 卡</b> —— 一台主机的量级就够，"
      "没有一个需要跨主机。</em>"))
a(fold('⛔ <b>「装得下」和「该用几颗」是两个不同的问题</b> ——&nbsp;'
       '两处按体积猜会猜错的地方',
...
a('  <p>打开仓库里任何一个扩散模型的目录，都会看到<b>两份例子</b>：'
  '一个一体化脚本，和一个 <code>*_staged/</code> 目录。'
  '<b>后者存在的理由不是「跑得更快」，是它把切口露在外面。</b></p>')
a(fig("figx-10",
