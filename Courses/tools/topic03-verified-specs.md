...
## D. ⭐ 用一个能被验证的模型，定位「官方数字对不上」的那一处

模型：`峰值 FLOPS = 时钟 × TensorCore数 × 每核MXU数 × MXU边长² × 2`

| 芯片 | 公布峰值 BF16/chip | core/chip | MXU/core | MXU 边长 | **反推时钟** |
|---|---|---|---|---|---|
| v2 | 46 TFLOPS | 2 | 1 | 128 | **0.702 GHz** |
| v3 | 140 TFLOPS | 2 | 2 | 128 | **1.068 GHz** |
| v4i | 137 TFLOPS | 1 | 4 | 128 | **1.045 GHz** |
| v4 | 275 TFLOPS | 2 | 4 | 128 | **1.049 GHz** |
| v5e | 197 TFLOPS | 1 | 4 | 128 | **1.503 GHz** |
| v5p | 459 TFLOPS | 2 | 4 | 128 | **1.751 GHz** |
| v6e | 918 TFLOPS | 1 | 2 | 256 | **3.502 GHz** |
| v7 | 2,307 TFLOPS | 2 | 2 | 256 | **4.400 GHz** |

**v2–v5p 全部落在可信区间，而且有两个能对上公开时钟的锚点**：
v2 反推 **0.702 GHz**（公开 700 MHz）、v4 反推 **1.049 GHz**（公开 1050 MHz）、
v5p 反推 **1.751 GHz**（公开 1.75 GHz）。**模型成立。**

**v6e 反推 3.51 GHz、v7 反推 4.41 GHz —— 对 TPU 明显不可能。**
说明这两代的三个公开数字（峰值 / MXU 数 / MXU 边长）**不可能同时为真**。
若把每核 MXU 数按 v4/v5 的 4 个算（或等价地假设每个 PE 每周期做 2 次 bf16 乘加），
则 v6e = **1.755 GHz**、v7 = **2.203 GHz** —— 正好接在 v5p 的 1.75 GHz 后面，趋势合理。

> ⚠️ **但这只是「哪种解释自洽」，不是查到的事实。**
> 课件里**不许**写「v7 时钟 2.2 GHz」或「v7 每核 4 个 MXU」。
> 能写的是：**这三个公开数字互相矛盾，且矛盾能被一个在前五代上验证过的模型定位出来。**
