...
**FP8 + QAG 叠加 `dvfs=7` 试过一轮但没拿到数** —— 2026-08-11 16:51 起跑，卡在
`BACKEND_PASSES` 编译阶段（FP8 编译比 BF16 明显慢）时节点被回收，`exit 137`。
**不是配置报错**，配方本身没被证伪，下次有卡时是第一顺位。外推值 625 × 1.086 ≈ 679。

**未测：** 256 芯片上是否等幅；FP8 + QAG 叠加的实测值；
