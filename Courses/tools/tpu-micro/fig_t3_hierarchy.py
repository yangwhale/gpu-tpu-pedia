...
ROWS = [
    ...
    dict(q="不换协议能连到多大", qn="超出这个规模就得换一套互联",
         ...
         tpu=_cell_text("9,216 颗", "3D 环面一路铺到整个 pod，"
                        "<b>全程同一套 ICI</b>。这是<b>互联那一节</b>的落点。", GN, "ICI 4.0")),
    # ⛔ 2026-09-05：原来写「这是 §8 的落点」。§8 在 L300 是「这一课的数我是怎么核的」，
    #    在 L200 整节没收 —— **两份文档里都指错了**。这张图两边共用，
    #    所以不写号，按内容指。
]
...
def _band(f):
    ...
    para(f, 34, BAND_Y + 88, 1330,
         ...
         "两边的调优直觉<r>不能互相搬运</r> —— 这也是为什么<b>收缩维那一节</b>里的 <code>head_dim=128</code> "
         "打 TPU 却不打 GPU。", "xs", 18)
