...
def _bundle(f):
    ...
    for c, name, n in SLOTS:
        ...
        x += 14
    # 6 个立即数
    f.rect(x, BUN_Y, 152, 42, FILL[SUB], SUB, 1.6, 5, "4,3")
    f.t(x + 76, BUN_Y + 26, "立即数 ×6", "lbl", SUB, "middle")
    x += 168

    f.rect(x, BUN_Y, 176, 42, "#fff", INK, 1.6, 5)
    f.t(x + 88, BUN_Y + 20, "合计 322 bit", "box", INK, "middle")
    f.t(x + 88, BUN_Y + 35, "TPU v2 / v3，公开论文", "xxs", None, "middle")
...
def _cards(f):
    ...
    items = [
        ("① 形状一不规则，性能就掉得很难看",
         ...
         "「后一步紧跟着前一步」，空拍没东西填。"),
        ("② 编译很慢，而且慢得有道理",
         gate.IP("v7 上一次 XLA 编译常见 <b>10–17 分钟</b>。",
