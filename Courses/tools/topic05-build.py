...
__FIG_FREQ__
  ...
  <p>于是有一条默认的摆法：<b>每一层都要说话的 TP、EP、FSDP、CP 先往最快的那一圈里放</b>；PP 只在段边界说话，DP 一步只说一次，它们去跨慢线。
    ...
    EP 和 FSDP 能靠提前发、边算边传藏住一部分，就有人让它们跨出去：V3 的 EP 64 就横跨 8 台机器（H800 机内外只差 3 到 4 倍，远没有 TPU 悬殊，见台账），Llama 3 把 FSDP 放在了最外层。
...
__FIG_TOPO__
  <p>TP4 decode 上能调的都调了：去掉 eager 模式（即打开 CUDA Graph）只多 2.6%，加 prefill 机器、调并发，总数从 14,563 涨到 21,100。
    可这 45% 是拿多一倍的卡换来的，出字间隔始终钉在 46.8–53 ms。换成 DEP8 那一步，同样并发 512 下出字间隔从 46.8 ms 降到 11.8 ms，首字延迟从 55.8 秒降到 22.8 秒（同一套在并发 256 时首字只要 7.8 秒，并发 512 下多出来的大部分是排队）。
    真正属于「换切法」的那笔账，是 KV 不再在 4 张卡上各存一份（5.4）；decode 从 4 张卡加到 8 张，也把专家摊薄了一半。两笔都换成了更大的 batch（并发是排队的请求数，batch 是同时在算的请求数）。
  ...
  <p>衡量的数叫扩展效率：加卡后每卡的吞吐 ÷ 加卡前每卡的吞吐。下面是我们自己的两组实测，<b>只能组内比</b>。
    左边是一次 weak scaling：卡 ×4、global batch ×4。右边比的是同样的卡、FSDP 铺多宽。
