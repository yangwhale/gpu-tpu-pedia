# DeepSeek-V4 (Flash / Pro) · GB300 NVL72 · SGLang / vLLM NVFP4 推理
...
V4-Pro 1.6T ≈ 851G FP4，**内存盘（tmpfs）放不下**（模型 + 运行时 > 942G 节点 RAM），且 RAM 要留给 KV cache。所有权重放 **Local SSD RAID `/mnt/disks/raid/0`**（读 14GB/s，加载 <1min）。GCS 备份走 **ADC + python SDK**（节点 OAuth scope 只读、org 禁 SA key，唯一可写法，见 benchmark 文档 §3.9 Step 3）。

- 模型 GCS 备份：`gs://<bucket>/DeepSeek-V4-Flash-NVFP4`（168G）/ `DeepSeek-V4-Pro-NVFP4`（913G）。
