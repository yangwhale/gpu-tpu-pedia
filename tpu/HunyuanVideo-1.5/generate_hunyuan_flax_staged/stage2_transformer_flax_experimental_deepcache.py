...
class TPUDeepCache:
    ...
    核心思想（参考 deepcache_explained.md）：
    ...
    与 angelslim 的区别：
    - angelslim 使用 PyTorch hook 机制，与 JAX JIT 不兼容
    - 本实现在 forward 函数级别实现，JIT 友好
    
    加速原理：
    - HunyuanVideo 有 20 个 double_blocks + 40 个 single_blocks
    - 跳过 20 层，只计算 40 + 1 = 41 层
    - 理论加速比: 61/41 ≈ 1.49x（考虑其他开销，实测可能 1.2x-1.4x）
    """
    ...
    def __init__(self, cache_start_step, cache_end_step, cache_step_interval, total_steps):
        ...
        # 缓存状态：double_blocks 的输出
        self.cached_img = None  # 进入 single_blocks 前的 img 状态
        self.cached_txt = None  # 进入 single_blocks 前的 txt 状态
    ...
    def get_cache(self):
        ...
        return self.cached_img, self.cached_txt
...
class FullForwardModule(torch.nn.Module):
    """
    封装完整 transformer forward 的 Module
    通过 Module 封装确保 torchax.compile 正确工作
    """
    ...
    def forward(
        ...
        """
        完整的 transformer forward，额外返回 double_blocks 后的中间状态
        """
...
class CachedForwardModule(torch.nn.Module):
    ...
    def forward(
        ...
        """
        使用缓存的 forward，跳过 double_blocks，只执行 single_blocks + final_layer
        """
        transformer = self.transformer
        ...
        # 从 attn_param 获取缓存的 thw（在 FullForwardModule.forward 中已设置）
...
def create_cached_transformer_modules(transformer, mask_type, extra_kwargs):
    """
    创建支持 DeepCache 的 transformer forward 模块
    
    返回两个 Module：
    1. full_forward_module: 完整执行 transformer
    2. cached_forward_module: 跳过 double_blocks，只执行 single_blocks + final_layer
    """
    full_forward_module = FullForwardModule(transformer, mask_type, extra_kwargs)
    cached_forward_module = CachedForwardModule(transformer)
    return full_forward_module, cached_forward_module
...
def main():
    ...
    # ========================================================================
    # 设置 TPU DeepCache（如果启用）
    # TPU 版本不使用 hooks，而是在 forward 级别实现条件分支
    # ========================================================================
    deep_cache = None
    ...
    if args.enable_cache:
        ...
        print_rank0(f"  cache_end_step: {args.cache_end_step}")
        print_rank0(f"  cache_step_interval: {args.cache_step_interval}")
        print_rank0(f"  no_cache_steps 数量: {len(deep_cache.no_cache_steps)} / {args.num_inference_steps}")
        print_rank0(f"  实际 cache 步数: {args.num_inference_steps - len(deep_cache.no_cache_steps)}")
        print_rank0(f"  (forward 函数将在 embeddings 准备后创建)")
    ...
    # === 编译 Transformer ===
    # 注意：当使用 DeepCache 时，我们需要编译两个独立的函数
    # 而不是编译整个 transformer
    print_rank0("\n编译 Transformer...")
    with env:
        if not args.enable_cache:
            # 不使用 cache 时，直接编译 transformer
            ...
            print_rank0("  DeepCache 模式：forward 函数将在准备 embeddings 后编译")
    ...
    with env:
        ...
        if args.enable_cache:
            ...
            timesteps_r_warmup = None
            if use_meanflow:
                timesteps_r_warmup = timesteps[1].repeat(latent_model_input_warmup.shape[0])
            
            # 预热 full_forward
            print_rank0("- 预热 full_forward（完整 transformer）...")
            warmup_start = time.perf_counter()
            with torch.no_grad():
                result_warmup = full_forward_fn(
                    latent_model_input_warmup,
                    t_expand_warmup,
                    prompt_embeds,
            ...
            warmup_full_time = time.perf_counter() - warmup_start
            print_rank0(f"  full_forward 预热完成: {warmup_full_time:.2f}秒")
            
            # 预热 cached_forward（使用 full_forward 的中间结果）
            ...
            warmup_cached_time = time.perf_counter() - warmup_start
            print_rank0(f"  cached_forward 预热完成: {warmup_cached_time:.2f}秒")
            print_rank0(f"  ✓ DeepCache 两条路径预热完成")
    ...
    def run_denoising_loop(
        ...
        with mesh, env:
            ...
            # 如果启用 DeepCache，在 denoising 开始前清除缓存状态
            if deep_cache is not None:
                deep_cache.clear()
                if not is_warmup:
                    print_rank0(f"  ✓ DeepCache 状态已清除")
        ...
        if deep_cache is not None and not is_warmup:
            ...
            print_rank0(f"\n[DeepCache 统计]")
            print_rank0(f"  缓存命中: {stats['cache_hit']}")
            print_rank0(f"  缓存未命中: {stats['cache_miss']}")
            print_rank0(f"  命中率: {stats['hit_rate']:.1%}")
