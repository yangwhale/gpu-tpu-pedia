...
def _band(f):
    ...
    para(f, 34, BAND_Y + 88, 1330,
         # ⛔⛔ 2026-09-05：这一句原来写「省下的面积去了两个地方：更大的乘加阵列
         #    和更大的软件可控暂存」。**这条断言全课已经撤回过，但只撤在正文里，
         #    图注原样活着。** 两个数都不支持它：
         #      · 每周期乘加数 —— B200 606,208 对 v7 524,288，**GPU 多 15.6%**
         #      · 片上 SRAM —— B200 231 MiB 对 v7 134 MiB，**GPU 多 73%**
         #    而「省了多少面积」这件事公开信息根本答不了（Google 不公布 die 尺寸
         #    和晶体管数），所以连「省下的面积去哪了」这个问法本身都立不住。
         # ⭐ 形状：**撤回一条断言，要连它的所有副本一起撤。**
         #    正文改了、图注没改，而图注是投到屏幕上的那一份。
         "<b>省下的不是「面积换算力」，是这套机构整个不做了</b> —— "
         "电路更简单、功耗更低。<r>它没有变成更多算力，也没有变成更多片上存储</r>"
         "（两项 B200 都更多）。"
