...
def _tpu(f):
    ...
    rows = [
        ...
        ("线程块 ＋ 共享内存",
         "一个 TensorCore ＋ 它私有的 <b>64 MB VMEM</b>"
         "<g>（Google 开发者社区 Ironwood 调优文；一颗 chip 两个核 ＝ 128 MB）</g>",
         "容量差 <r>289 倍</r>（227 KiB vs 64 MiB）——&#160;<b>但这是上限比上限</b>："
         "227 是<b>一个线程块</b>的上限（同一个 SM 上 4 个 Tensor Core 抢它），"
         "64 也要跟<b>权重预取</b>分"),
        ("cluster",
