...
def main():
    ...
    ROWS = [
        ("朴素线性注意力", "2020", "I", "eye",
         "—— 起点：只加不减", "最容易", GY),
        # ⛔ 2026-09-12 二轮学生审稿：原来 RetNet 和 GLA 挤在同一格，
    ...
    f.t(0, y0 + 6, "⚠️ 先说怎么读这八张小图："
        ...
        "（越矮擦得越狠）；", INK, size=17, w=1396)
    f.t(0, y0 + 32, "<tspan font-weight=\"700\">盖在上面的斜纹 ＝ 这一整块都被动过</tspan>"
        "（也就是那个 −βkkᵀ）。八张图的差别全在这两样上。", INK, size=17, w=1396)
    y0 += 52
    ...
    for idx, (name, year, form, kind, fix, par, col) in enumerate(ROWS):
        ...
        # ⛔⛔ 2026-09-12 二轮学生量了 WCAG 对比度：这八格原来靠**深浅**区分，
    ...
    yy = f.band(yy, "info", "⭐⭐ 这张图真正的结论：表达力和可算性是一起设计的", [
        ...
        # ⛔ 2026-09-12：这里原来写「没有高效的并行扫描 —— 并行度直接归零」。
