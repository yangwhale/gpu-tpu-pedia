...
def main():
    ...
    # ══════════ ② 三个旋钮 ══════════════════════════════════════
    y1 = y0 + PH + 18
    # ⛔⛔ 2026-09-14 现场指出：每张卡里「每个 token 存多少」和下面那行
    #   「拧过去之后：…」**压在一起**（实测垂直重叠 9px）。
    #   原来是 what 的基线 by+192（20px）对上 by+206（18px），中间只有 14px ——
    #   ⭐ 20px 的字**需要 28px 以上的行距**，14px 是必然相撞，不是偶然。
    # ⛔⛔ 更要紧的是：**版面体检那条撞车 lint 看着这三处，报了 0。**
    #   它的判据是「垂直重叠超过一半才算同一行」，而这三处 frac 只有 0.36。
    #   那个 0.5 是为了不把**正常行距**误报成撞车而设的，方向没错，
    #   但它把「**两行离得太近、糊在一起**」整类漏了个干净 ——
    #   ⭐⭐ 判据：**汉字的墨迹几乎填满 em 盒**，所以对中文来说
    #     「重叠 36%」根本不是擦边，是实打实地压字。
    #     用「占各自高度的比例」当阈值，天然对中文过宽。已一并改掉那条 lint。
    PH2 = 404
    ...
    for i, (col, no, what, who, effect, kind) in enumerate(KNOBS):
        bx = 96 + i * 420
        f.box(bx, by + 20, 356, 316, "#fff", col, 12)
        # 旋钮本体：一个圆 ＋ 一根指针
        ...
        # 拧过去之后那一格长什么样
        gy_ = by + 232          # ⭐ 原来 212 —— 见上面 PH2 那段，两行只差 14px
