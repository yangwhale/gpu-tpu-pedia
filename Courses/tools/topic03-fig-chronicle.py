...
⭐⭐⭐ **2026-09-07 第四轮，一条追问换来整张表的重心。** 原话：
   ...
   **做到 1M 以上的十二家，无一例外都动了旋钮②或③；纯全注意力那一档最高只到 288K（Mistral Large 3）。**
   ⚠️ 口径见 `topic03_models.over_1m`：声明值 ≥1M；RWKV-7 的「无限（理论）」
   不计入（算上它是 13 家，结论不变）——&nbsp;**这门课自己在教口径，就不能自己有两个数。**
...
# ⛔ 2026-09-07：W 从 1560 收到 1460。原因不是排版，是**字号**——
#    图在版心里按 (壳宽 ÷ W) 缩放，W 越大字被缩得越小。1560 时正文渲染出来
#    只有 11.9px，比专题二 42 张图的地板（12.2px）还小一点点。
#    ⭐ 收窄 W ＝ 放大缩放比 ＝ 字变大，**一个字都不用改**。
#    📌 横轴是按内容宽度分配的，所以收窄之后它自己重新分配，不会撞车。
W = 1460
p = []
...
LANES = (
    ...
    ("① 每个 token 存多少（KV 怎么小）", BL, "#e8f0fe", [
        (2019, "MQA"), (2023, "GQA"), (2024, "MLA"),
        # ⛔ 2026-09-07 补：这条泳道原先 2024 之后就只剩一个 Gated MLA，
        #   看起来像「旋钮① 早就到头了」——&nbsp;**不是**。2025-2026 它又活了：
        #   MFA（Step-3，低秩分解把头数和头维一起放大）、
        #   K=V 共享（Gemma 4 全局层）、跨层共享 KV（Gemma 4 E2B、CLA 一族）。
        # ⭐ 漏掉它们会让读者以为「省 KV 这条路 2024 年就走完了」。
        (2024, "CLA 跨层共享"),
        (2025, "MFA 低秩分解"),
        (2026, "Gated MLA · K=V 共享"),
    ]),
...
Y0, Y1 = 2014, 2026
# ⛔ AX0 从 150 抬到 182：泳道标题写在 x=14，最长那条「前史 · 注意力是 RNN 的补丁」
#    量出来约 166px，而 2014 那一格的标签从 AX0+18 起 —— **正好压在「丁」字上**。
#    ⭐ 版面 lint 报「图内文字无撞车」却没抓到它：它比的是**格子**，
#      而这里撞的是**两段自由文字**。别把 lint 全绿当成没问题。
AX0, AXW = 182, W - 182 - 30
...
for y in range(Y0, Y1 + 1):
    ...
    p.append('<line x1="%d" y1="%d" x2="%d" y2="%d" stroke="%s" stroke-width="1"/>'
             % (x, AXY - 4, x, AXY + 4, _c))
    t(x, AXY - 9, "'%s" % str(y)[2:], fill=_c, anchor="middle", size=11)
