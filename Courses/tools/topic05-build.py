...
<figcaption>整层只在用的那一刻出现；平时每张卡只拿着每层的四分之一。
  ...
  <p>通信是 3Ψ，<b>数据并行的 1.5 倍</b>；换来的是每卡常驻从 9.76 TiB 降到 9.76 GiB（约 10 GB，还不含激活）。</p>
...
''' + sec("s三", "三", "第二刀：切权重") + '''
  ...
  <p>判断一刀会不会被通信拖住，只看一个比值：<b>每在网络上搬一个字节，能换来多少次计算</b>。它要高过硬件自己的比值（每秒算多少次 ÷ 每秒搬多少字节），否则卡就在干等。</p>
  <p>FSDP 这笔账很干净（⚠️ 推导，按稠密模型算）：一步搬约 6Ψ 字节（前向、反向各拼一次 bf16 权重，再分一次梯度，各约 2Ψ），
    算 6ΨT 次（专题四「每个 token 约 6 倍参数量次运算」，T 是每张卡分到的 token 数）。<b>两者一除，正好等于 T</b>，跟模型多大没关系。</p>
__FIG_INTENSITY__
  <p>红虚线 3,845 ＝ v7 每芯片 2,307 TFLOP/s（bf16）÷ 每芯片发出约 0.6 TB/s（常说的 1.2 TB/s 是收发合计；⚠️ 这个拆分是推算，见台账）。
    换句话说，<b>每颗芯片一步分不到三千八百多个 token，FSDP 就被拖住</b>。这还是最乐观的线：只走一根轴，高约 3 倍；
    MoE 只靠 FSDP、不配专家并行，搬的是全部参数、算的只有被选中的那部分，再乘约 18 倍（V3 的 6,710 亿 ÷ 370 亿）。</p>
...
__FIG_TP_ORDER__
  ...
  <p>TP 切不到的是逐个 token 的小运算（归一化、dropout、残差相加），那几段每张卡都存着一整份激活。
    ...
    就是第一节那个等式，<b>通信量不变</b>，省下的是激活显存。别被名字骗了：它是 TP 的搭档，不是第四刀（§8.8）。</p>
  ...
  <p>TP 送的是「半成品」（激活）：token 多一倍，要送的和要算的都多一倍，<b>batch 在账里约掉了</b>。
    ...
    V3 的隐藏维 7,168，TP 8 路约 4,032，按最乐观的线刚好贴线（7,168 × 4.5 ÷ 3,845 ≈ 8.4），实际已经在线下。</p>
  ...
  <h3>3.5　PP：按层切</h3>
  <p>要横跨很多台机器、走慢线，又不想像 FSDP 那样每层搬权重，就按层切：每张卡负责连续的几层，段与段之间只在边界上用一对一收发传激活。
    这是所有刀里通信最少的，所以它<b>能跨到慢线上</b>。代价是<b>气泡</b>：</p>
__FIG_PP_BUBBLE__
...
<figcaption>micro-batch 从 4 个加到 8 个，灰色的气泡跟着缩一半。
  ...
  <p>每一行空白的总长都一样，是 p−1 个前向加 p−1 个反向。在最早的 GPipe 调度里，每一份的激活都要攒着等反向，份数越多攒得越多；
    1F1B 调度把同时在路上的份数限制在 p 以内，气泡一样大，省的是激活显存。</p>
