...
ROWS=[  # (层名, 副注, TPU 内容, GPU 内容, TPU空?, GPU空?)
 ...
 #    TPU 侧有没有独立的超越函数硬件 —— **只能写「公开资料没说」，不能写「没有」**。
 #    这一条 2026-08-31 收紧过措辞：原来写的是「没有单列的单元」，那是在替对方
 #    宣布不存在。查不到只证明查不到。初代 TPU 明确有过这类硬件（见正文脚注 §），
 #    所以「没披露」比「没有」是概率高得多的解释。
 ("零碎活的小单元","矩阵乘和逐元素之外","XLU 跨 lane 单元 × 2（公开资料，未标代次）"
  "<TAB>转置 · 跨 lane 归约 · shuffle —— 公开资料明写「慢且贵」"
  "<TAB>⚠️ 超越函数（exp / tanh / sin）归哪个部件，公开资料没说",
