...
而语言模型的取行重复度不够高 ——&nbsp;<b>注意不是「没有重复」</b>：
一条十几万 token 的序列里不同的词只有几千个，绝大部分命中最高频那一两千行。
重复确实有，<b>只是跟搜广推差着量级</b>，落在<b>最不划算的那一端</b>，
编译器于是根本没把它派过去。''' +
gate.IKO("doc.dupfactor", why="判据公式出自内部资料") + '''
...
<td>不是「几乎不重复」——&nbsp;十几万 token 里不同的词只有几千个，重复确实有；
但<b>集中度跟搜广推差着量级</b>，所以落在 SparseCore <b>最不划算</b>的那一端。⚠️ 本页只给这个观察，<b>不给判据的具体算式</b></td></tr>
<tr><td>XLA 编译耗时<br>（§7）</td><td>10–17 分钟</td>
<td>v7 上大模型一次编译的常见区间。<b>它和硬件规格无关，跟模型大小、
切片规模、XLA 版本都有关</b>，所以只当量级读，不要当基准</td></tr>
</tbody></table></div>
''' + gate.IKO("doc.internal_sources", why="内部条目清单") + '''
</section>''')
