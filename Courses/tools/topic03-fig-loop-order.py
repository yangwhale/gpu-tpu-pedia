...
"""
from topic03_draw import (Fig, BL, OR, GR, RD, GY, PU, CY, INK, GY2,
                          LINE, LINE2)
...
# ⭐⭐ R36 加的装置：迭代集合 ⊥ 依赖集合
# ══════════════════════════════════════════════════════════════════
# 原来这一格是**用散文说的**：「下一列还会碰到同一个 Q 块，这块的输出没算完，
# 只能先存回去」——&#160;话是对的，但读者只能信，图上看不出来。
#
# 网上能找到最接近的画法是知乎 @猛猿 那两张：同一张 S 网格，一张用红圈圈住
# **一行**（O_0 要等齐的），另一张圈住**一列**（一趟外层循环碰到的）。
# ⛔ 但那是**前后两张图**，隔着几屏 ——&#160;「两个集合互不重合」这个真正的原因，
#    要读者自己在脑子里把两张图叠起来才看得见。**没人把它们并排画过。**
#
# ⭐ 本图把两个集合叠进同一张网格：青色＝这一趟外层循环会碰到的格子，
#    红框＝某一个输出要等齐的格子。于是
#      FA1：一列 × 一行，**只交于 1 格** → 这一趟凑不齐任何一个输出 → 必须落盘
#      FA2：两者**是同一行**            → 走完这一趟，这个输出直接定稿
# ⭐⭐ 而这跟面板 ③ 那条 warp 判据是**同一句话在三个尺度上**：
#    求和号跑在 K/V 这一维上 ——&#160;循环外层走 K/V，一趟就凑不齐任何输出；
#    thread block 按 K/V 分，块之间就得通信；warp 切 K/V，一个 warp 也凑不齐。
#    ⛔ 加这条之后 ③ 的 band 标题从「两个尺度」改成了「三个尺度」——&#160;
#      标题和正文分头改是本仓库反复出过的错，改一个就回头对另一个。
ISEL, JSEL = 5, 2                 # 拿来举例的那一行 / 那一列

_dep = {(ISEL, j) for j in range(NB) if j <= ISEL}       # O_ISEL 要等齐的
_it1 = {(i, JSEL) for i in range(NB) if JSEL <= i}       # FA1 一趟外层（走 K/V）
_it2 = {(ISEL, j) for j in range(NB) if j <= ISEL}       # FA2 一趟外层（走 Q）
assert len(_it1 & _dep) == 1, "举例的行列必须相交，否则「只交于 1 格」是空话"
assert _it2 == _dep, "⭐ FA2 的迭代集合就是依赖集合 —— 这是整个装置的落点"
assert len(_dep) == len(_it1) == 6, "两个集合一样大，比较才干净"
FRAC = len(_it1 & _dep), len(_dep)                       # 这一趟给 O_ISEL 凑齐了多少
assert FRAC == (1, 6)
...
def main():
    ...
    def grid(x, y, outer_q, col, title, sub):
        ...
        for i in range(NB):
            for j in range(NB):
                ...
                if j > i:
                    ...
                    continue
                # ⭐ 青色 ＝ 这一趟外层循环会碰到的格子（迭代集合）。
                #   FA1 外层走 K/V → 是一整列；FA2 外层走 Q → 是一整行。
                #   **标签贴在哪，本身就是内容**：列头 vs 行尾，见下面两个 tag。
                hot = (i == ISEL) if outer_q else (j == JSEL)
                f.box(cx, cy, G - 2, G - 2,
                      "#d0e9ec" if hot else "#fbfcfe",
                      CY if hot else "#dde4ec", 2, 1.3 if hot else 1)
        ...
        # ── 依赖集合：O_ISEL 要等齐的那些格子 ────────────────────────
        # causal 下就是第 ISEL 行的 j = 0..ISEL。**两版完全一样** ——
        # 它由数学决定（softmax 要整行归一），跟循环怎么写没有关系。
        # 所以红框在两张图里画在同一个位置，是**对照的锚**。
        f.box(x - 4, y + ISEL * G - 4, (ISEL + 1) * G + 6, G + 6,
              "none", RD, 5, 1.8)
        f.t(x - 12, y + ISEL * G + 20, "O_%d 要等齐" % ISEL, RD, True, 13,
            "end")

        # ── 迭代集合的名字，贴在它自己的「末端」上 ───────────────────
        # ⭐ 两张图这个标签**落点不同**，而那个不同正是本格要讲的事：
        #     FA1 → 列头（这一趟顺着列往下走）
        #     FA2 → 行尾（这一趟顺着行往右走，终点正好是红框的另一端）
        #   位置都落在 causal 空出来的灰三角里，不占新地方。
        if outer_q:
            f.t(x + 6 * G + 31, y + ISEL * G + 20, "这一趟", CY, True, 13,
                "middle")
        else:
            f.t(x + JSEL * G + 15, y + 21, "这一趟", CY, True, 13, "middle")
            f.t(x + JSEL * G + 15, y + 44, "↓", CY, True, 14, "middle")
        return y + NB * G
    ...
    def setline(a, b, tail):
        """第三行图例：两个集合各自是什么形状，以及它们的关系。

        ⭐ 颜色跟网格上一致（青＝迭代集合、红＝依赖集合），所以这行不是
           在**重复**图，是在**读**图 ——&#160;读者能把字直接贴回格子上。
        """
        return ('<tspan fill="%s" font-weight="700">这一趟 ＝ %s</tspan>，'
                '<tspan fill="%s" font-weight="700">要等齐的 ＝ %s</tspan>'
                ' ——&#160;%s' % (CY, a, RD, b, tail))

    for gx, s1, s2, s3 in (
            (GX1, "K/V 每列只载一次（%d 次）" % TC,
             "O 每一格都要读回来再写回去",
             setline("一列", "一行",
                     "只交于 1 格，这一趟只给 O_%d 凑齐 %d/%d"
                     % ((ISEL, ) + FRAC))),
            (GX2, "K/V 每一格都要重新载一次",
             "O 每行只写一次（%d 次）" % TR,
             setline("一行", "同一行",
                     "完全重合，走完这一趟 O_%d 就定稿" % ISEL))):
        ...
        f.t(gx + 4, bot + 64, s3, GY, False, 14)

    f.lines(60, bot + 92, 1280, [
        "⭐ 判据就长在这两个集合上："
        "<tspan font-weight=\"700\">一趟循环碰到的格子，"
        "跟一个输出需要的格子，是不是同一批。</tspan>"
        "不是 → 这一趟凑不齐任何一个成品，",
        "中间结果只能先落回 HBM（FA1 那 %d 次 O 读写就是这么来的）；"
        "是 → 算完即定稿。⭐ 这跟面板 ③ 的 warp 判据"
        "<tspan font-weight=\"700\">是同一句话</tspan> ——&#160;"
        "求和号跑在 K/V 这一维上。" % C1["O"],
    ], size=15, lh=25)
    ...
    yy = f.band(y2 + PH3 + 2, "ok", "一条判据，三个尺度通用", [
        "<tspan font-weight=\"700\">切「要被累加的那一维」就得合；"
        "切「各自独立出结果的那一维」就不用合。</tspan>"
        "attention 里前者是 K／V（它们在求和号里面），后者是 Q（每行输出各管各的）。",
        "⭐ 所以面板 ① 那句「这一趟碰到的，是不是这个输出要的」，"
        "跟这里的 warp 分工，是<tspan font-weight=\"700\">同一条判据"
        "在三个尺度上</tspan>：循环、thread block、warp ——&#160;"
        "动作都是<tspan font-weight=\"700\">把外圈让给 Q</tspan>。"
