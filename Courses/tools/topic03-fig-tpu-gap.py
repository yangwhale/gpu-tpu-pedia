...
def main():
    ...
    f.t(80, ay + 246, "⭐ 出餐极快 ——　代价是临时改单很贵", BL, True, 17)
    ...
    f.t(748, ay + 246, "⭐ 灵活 ——　代价是每道菜都要现开火", OR, True, 17)
    ...
    for i, (t, what, where) in enumerate([
        ...
        f.t(bx + 22, by + 66, t, OR, True, 17, w=356)
    ...
    f.t(56, by + 220, "⛔ 这三样<tspan font-weight=\"700\">全是在 GPU 上先长出来的</tspan>"
        " ——&#160;它们默认了一台「随手跑腿不太贵」的机器。", RD, True, 17)
    ...
    f.t(760, ey + 142, "⛔ 六次搬运，地址还是跑起来才算出来的", RD, True, 17)

    f.box(56, ey + 168, 1304, 104, "#fce8e6", RD, 10)
    f.t(80, ey + 208, "⭐ 论文原话：这让<tspan font-weight=\"700\">"
        "DMA 调度</tspan>变得困难", RD, True, 17)
    f.t(80, ey + 246, "——&#160;DMA 就是那台推车：它最擅长「一趟拉一整排」，"
        "最怕「这一趟拉哪几个，得先算一下」。", GY, size=17)

    # ══════════ 落点 ════════════════════════════════════════════
    yy = y2 + PH3 + 20
    yy = f.band(yy, "bad", "⛔ 这句话值得原样读一遍 —— 它出自 2026 年 4 月，不是五年前", [
