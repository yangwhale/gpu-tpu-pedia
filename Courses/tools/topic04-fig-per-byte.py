...
⭐ 图上每一个数都能当场验：
  ...
  · 交点 1.25·S = 16384 →&#160;S ≈ 13,107（16,384 ＝ o_proj 的输入宽，不是 d_model）
  · GPT-3 那一竖（S=2048）落在 **2,048**（它的斜率是 1.00，不是 V3 的 1.25）
    ——&#160;**比所有线性层都低**
    ⛔ 这一行早先写的是 2,560，那正是本文件下面记的那个 bug 的产物；
      **改了代码没改文件头**，于是错的那个数在注释里又活了几天。
  · V3 预训练那一竖（S=4096）落在 5,120 ——&#160;**比最宽的线性层 16,384 还低**
  · V3 扩训那一竖（S=131072）落在 163,840 ——&#160;**比它高 10 倍**
...
# ⛔⛔⛔ 2026-09-19 再修一次，栽在**同一条判据的另一侧**：
#   这里原来写 7168，注释是「gate/up 的输入宽 ＝ d_model（MoE 专家很窄）」——
#   只查了 MoE 专家那一侧（2,048 < 7,168 ✅），**漏了 attention 的出口**。
#   V3 的 o_proj 把 128 头 × 128 ＝ 16,384 投回 7,168，输入宽是 d_model 的两倍。
#   ⭐ 也就是说：下面那段关于 GPT-3 的教训写对了，却没有回头用在自己身上。
#   判据补一句：**「最宽的线性层」要把每一个矩阵乘的输入都数一遍，不能只数 FFN 那一侧。**
V3_WIDEST = 128 * 128        # ＝ 16,384，o_proj 的输入宽（全层最宽）
...
def main():
    ...
    f.t(cx, cy - 60, "⭐⭐ 交点　S ≈ 13,107", INK, True, 16, "middle")
    f.t(cx, cy - 38, "左边 attention 还不是最贵的，右边它就是最贵的",
    ...
    yy = f.band(py + PH + 22, "bad", "同一条判据，结论翻转 ——　而两个模型的线<tspan text-decoration=\"underline\">都不一样</tspan>", [
        ...
        "<tspan font-weight=\"700\">163,840</tspan>，越过交点 13,107 <tspan font-weight=\"700\">很远</tspan>，"
    ...
    yy = f.src(yy + 24,
               ...
               " ——&#160;这才是要记的东西",
               "⛔ 交点 13,107 <tspan font-weight=\"700\">不是工程阈值</tspan>，"
