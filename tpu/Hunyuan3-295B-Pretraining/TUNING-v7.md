...
| BF16 445 → 599 每一步 | [§3 调优故事线](#3-调优故事线从-445-到-599) |
...
> ⭐ **64 芯片当前最优是 FP8+QAG 的 625**，比同规模无 QAG 的 594 高 5.3%，
> 而且 batch 更小（7 vs 10）。代价：FSDP 只能取 64（[§5.4.2](#542-qag先量化再通信一条被专家数卡死的路)）。
...
> **v7 每芯片吞吐是 GB300 单卡的 69.1%**（调优前 51.4%）。
> 对 v5p 的「4.16 倍」因序列长度不同而**偏高**，只作量级参考。

**BF16 目标 600–630 TFLOP/s/chip（26–27% MFU）：当前 599，差 0.2%，已达标。**
**FP8 当前 625（192e）/ 645（256e）；调参空间已见底，见 [§4.6](#46-什么能调什么不能调--一张总表)。**
